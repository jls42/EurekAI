...
// ─────────────────────────────────────────────────────────────────────────────
// Méthodes extraites de quizVocalComponent — `const = function` pour éviter
// l'agglomération Lizard CCN (cf. CLAUDE.md piège connu).
// ─────────────────────────────────────────────────────────────────────────────

const questions = function (this: QuizVocalContext): QuizQuestion[] {
  return this.items();
};
...
const currentAudioUrl = function (this: QuizVocalContext): string {
  const idx = this.currentIndex();
  if (idx === undefined) return '';
  return (this.gen as QuizVocalGen).audioUrls?.[idx] || '';
};

const questionAudio = function (this: QuizVocalContext): HTMLAudioElement | null {
  return (this.$refs.questionAudio as HTMLAudioElement | undefined) ?? null;
};

const stopQuestion = function (this: QuizVocalContext) {
...
const playQuestion = function (this: QuizVocalContext) {
  const audio = this.questionAudio();
...
const startVocalRecording = async function (this: QuizVocalContext) {
  try {
    this.stopQuestion();
...
const stopVocalRecording = function (this: QuizVocalContext) {
...
const isCurrentAnswered = function (this: QuizVocalContext): boolean {
  const idx = this.currentIndex();
...
const nextQuestion = function (this: QuizVocalContext) {
  this.stopQuestion();
  this.feedback = null;
  this.currentQ++;
...
const prevQuestion = function (this: QuizVocalContext) {
...
const onNextReady = function (this: QuizVocalContext) {
  this.restoreOrPlay();
};

const onPrevReady = function (this: QuizVocalContext) {
  this.restoreOrPlay();
};

const restoreOrPlay = function (this: QuizVocalContext) {
  const idx = this.currentIndex();
