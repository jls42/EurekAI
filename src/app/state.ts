import { UI_LANGUAGES } from '../i18n/languages';
import type {
  Consigne,
  Generation,
  Profile,
  ProjectData,
  ProjectMeta,
  Source,
  ChatMessage,
  PendingTrackerEntry,
} from '../../types';
import type { EventKey } from '../../helpers/event-bus';
import type { Toast } from './toast';

const PROFILE_AVATARS = Array.from({ length: 20 }, (_, i) => String(i));

// Catégories de navigation/génération — extraites pour limiter la longueur
// de createState() (warning Codacy `Method has X lines of code`).
const CATEGORIES_DEFAULT: ReadonlyArray<{
  key: string;
  labelKey: string;
  icon: string;
  color: string;
}> = [
  {
    key: 'dashboard',
    labelKey: 'nav.dashboard',
    icon: 'layout-grid',
    color: 'var(--color-primary)',
  },
  { key: 'sources', labelKey: 'nav.sources', icon: 'upload-cloud', color: 'var(--color-accent)' },
  { key: 'chat', labelKey: 'nav.chat', icon: 'message-circle', color: 'var(--color-primary)' },
  { key: 'summary', labelKey: 'nav.summary', icon: 'file-text', color: 'var(--color-gen-summary)' },
  {
    key: 'flashcards',
    labelKey: 'nav.flashcards',
    icon: 'layers',
    color: 'var(--color-gen-flashcards)',
  },
  { key: 'quiz', labelKey: 'nav.quiz', icon: 'brain', color: 'var(--color-gen-quiz)' },
  {
    key: 'quiz-vocal',
    labelKey: 'nav.quiz-vocal',
    icon: 'mic',
    color: 'var(--color-gen-quizvocal)',
  },
  {
    key: 'podcast',
    labelKey: 'nav.podcast',
    icon: 'headphones',
    color: 'var(--color-gen-podcast)',
  },
  {
    key: 'fill-blank',
    labelKey: 'nav.fill-blank',
    icon: 'pencil-line',
    color: 'var(--color-gen-fillblank)',
  },
  {
    key: 'dictation',
    labelKey: 'nav.dictation',
    icon: 'spell-check',
    color: 'var(--color-gen-dictation)',
  },
  { key: 'image', labelKey: 'nav.image', icon: 'image', color: 'var(--color-gen-image)' },
];

const initialTheme = (): 'dark' | 'light' => {
  const t = localStorage.getItem('sf-theme');
  if (t === 'dark' || t === 'light') return t;
  return globalThis.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
};

// Sections du state Alpine extraites en helpers pour rester sous le seuil
// Codacy "Method has X lines of code". Le getter `uploading` reste inline dans
// createState() car le spread `{...initSourceState()}` perd la sémantique
// getter (copie la valeur retournée, pas le descripteur).

const initProfileState = () => ({
  profiles: [] as Profile[],
  currentProfile: null as Profile | null,
  showProfilePicker: false,
  showProfileForm: false,
  editingProfile: null as Profile | null,
  newProfileName: '',
  newProfileAge: '',
  newProfileAvatar: '0',
  newProfileLocale: 'fr',
  profileAvatars: PROFILE_AVATARS,
});

const initProjectState = () => ({
  projects: [] as ProjectMeta[],
  currentProjectId: null as string | null,
  currentProject: null as ProjectData | null,
  newProjectName: '',
  showNewProject: false,
});

type UploadSession = {
  id: string;
  projectId: string;
  cleanupScheduled: boolean;
  files: Array<{
    id: string;
    name: string;
    file: File | null;
    status: 'pending' | 'hashing' | 'uploading' | 'done' | 'duplicate' | 'error';
    errorMsg: string | null;
  }>;
};

const initSourceState = () => ({
  sources: [] as Source[],
  selectedIds: [] as string[],
  uploadSessions: [] as UploadSession[],
  dragging: false,
  viewSource: null as Source | null,
  viewSourceMode: 'ocr' as string,
  viewSourceZoom: 1,
  viewSourceRotation: 0,
  viewSourceRotations: {} as Record<string, number>,
  viewSourceDragging: false,
  viewSourceDragStart: { x: 0, y: 0 },
  viewSourcePanX: 0,
  viewSourcePanY: 0,
  viewSourcePanStart: { x: 0, y: 0 },
  viewSourceCompareVertical: true,
  textInput: '',
  webQuery: '',
  scrapeMode: 'auto' as 'auto' | 'readability' | 'lightpanda',
  showTextInput: false,
  showWebInput: false,
  _metaPopoverPos: null as DOMRect | null,
  _metaPopoverAnchor: null as HTMLElement | null,
  _metaPopoverShownAt: 0,
  _metaPopoverTitle: '',
  _metaPopoverLines: [] as string[],
  _metaPopoverLineClass: 'text-text-secondary',
  _metaPopoverFooter: '',
  _metaPopoverFooterClass: 'text-text-primary',
});

const initVoiceState = () => ({
  recording: false,
  recorder: null as MediaRecorder | null,
  recordingDuration: 0,
  recordingTimer: null as ReturnType<typeof setInterval> | null,
});

const initConsigneState = () => ({
  consigne: null as Consigne | null,
  consigneLoading: false,
  useConsigne: true,
});

const initGenerationState = () => ({
  generateCount: 10,
  countOptions: [10, 20, 30, 42, 50],
  generations: [] as Generation[],
  openGens: {} as Record<string, boolean>,
  editingTitle: null as string | null,
  editTitleValue: '',
  loading: {
    summary: false,
    flashcards: false,
    quiz: false,
    podcast: false,
    'quiz-vocal': false,
    image: false,
    'fill-blank': false,
    dictation: false,
    auto: false,
    all: false,
    voice: false,
    websearch: false,
  } as Record<string, boolean>,
  // abortControllers indexés par type (legacy) coexiste avec abortControllersByGid
  // (par gid UUID v4) qui permet le cancel ciblé d'un pending précis quand plusieurs
  // pendings du même type coexistent.
  abortControllers: {} as Record<string, AbortController>,
});

// --- Pending lifecycle ---
// pendingById = source de vérité pour les 7 Generation persistées (multi-pendings
// même type possibles : multi-onglets, /generate/auto parallel). Hydraté depuis
// project.results.pendingTracker au selectProject + via events SSE.
// shownToastEventKeys = dédup toast UI per-tab (mémoire onglet, non persistée).
// Combiné avec appendNotification (dédup persistée localStorage), garantit qu'un
// même eventKey produit max 1 toast UI par onglet ET max 1 notif persistée cross-tabs.
// crossTabSyncBroken = flag UI public si le bridge cross-tab basé sur l'API
// privée Alpine `_x_dataStack` casse après upgrade. Posé via event DOM.
// notificationsVersion = compteur incrémenté à chaque appendNotification réussi
// pour déclencher la reactivity Alpine sur la cloche header.
const initPendingLifecycleState = () => ({
  pendingById: {} as Record<string, PendingTrackerEntry>,
  abortControllersByGid: {} as Record<string, AbortController>,
  shownToastEventKeys: new Set<EventKey>(),
  crossTabSyncBroken: false,
  notificationsVersion: 0,
});

type MistralVoice = {
  id: string;
  name: string;
  languages: string[];
  gender?: string;
  tags?: string[];
  speaker: string;
  emotion: string;
  lang: string;
  langFull: string;
};

const initSettingsState = () => ({
  showSettings: false,
  apiStatus: { mistral: false, ttsAvailable: false, voiceCacheReady: false, requireUserKey: false },
  // Clé Mistral navigateur (cf. src/app/api-key.ts). Champs RÉACTIFS : les gates
  // mistralReady()/ttsReady() en dépendent, mutés au boot/save/clear/switch profil.
  hasMistralKey: false,
  keyStorageDegraded: false,
  showApiKeyDialog: false,
  apiKeyInput: '',
  apiKeyScope: 'global' as 'global' | 'profile',
  apiKeyConsentClear: false,
  keyTestStatus: '' as '' | 'testing' | 'ok' | 'invalid' | 'quota' | 'network' | 'missing',
  allModerationCategories: [] as string[],
  moderationDefaults: {} as Record<string, string[]>,
  mistralVoicesList: [] as MistralVoice[],
  configDraft: {
    models: {
      summary: '',
      flashcards: '',
      quiz: '',
      podcast: '',
      translate: '',
      ocr: '',
      quizVerify: '',
      chat: '',
    },
    ttsModel: '',
  },
});

const initNavigationState = () => ({
  sidebarOpen: false,
  sidebarCollapsed: false,
  mobileTab: 'magic',
  isMobile: false,
  activeView: 'dashboard',
});

const initChatState = () => ({
  chatMessages: [] as ChatMessage[],
  chatInput: '',
  chatLoading: false,
});

const initDialogState = () => ({
  toasts: [] as Toast[],
  toastCounter: 0,
  confirmCallback: null as (() => void | Promise<void>) | null,
  confirmTarget: '',
  confirmMessage: '',
  confirmCta: '',
  confirmTrigger: null as HTMLElement | null,
  newProfilePin: '',
  newProfilePinConfirm: '',
  pinVerifyInput: '',
  pinVerifyCallback: null as ((pin: string) => void) | null,
  showPinDialog: false,
});

export function createState() {
  return {
    uiLanguages: UI_LANGUAGES,
    ...initProfileState(),
    ...initProjectState(),
    ...initSourceState(),
    get uploading(): boolean {
      // 'hashing' = pré-check en cours → encore "en upload". 'duplicate' = en attente de décision
      // utilisateur (Importer quand même / Ignorer) → PAS bloquant, exclu pour ne pas figer l'UI.
      return this.uploadSessions.some((s) =>
        s.files.some(
          (f) => f.status === 'pending' || f.status === 'hashing' || f.status === 'uploading',
        ),
      );
    },
    ...initVoiceState(),
    ...initConsigneState(),
    ...initGenerationState(),
    ...initPendingLifecycleState(),
    ...initSettingsState(),
    theme: initialTheme(),
    ...initNavigationState(),
    ...initChatState(),
    categories: CATEGORIES_DEFAULT.map((c) => ({ ...c })),
    lightboxUrl: '',
    ...initDialogState(),
  };
}
