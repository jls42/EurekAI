...
|---|---|---|
| 📷 | **Importazione file** | Importa le tue lezioni — foto, PDF (tramite Mistral OCR con punteggio di confidenza medio, terzo `high`/`medium`/`low`) o file di testo (TXT, MD). Sessioni di upload con retry per file e avanzamento individuale |
...
<p align="center">
  <img src="public/assets/model-map.webp" alt="Mappatura IA da modello a attività" width="800" />
</p>
...
- **Importazione file** — File JPG, PNG o PDF trattati da `mistral-ocr-latest` (testo stampato, tabelle, scrittura a mano), oppure file di testo (TXT, MD) importati direttamente. Gli upload multi-file utilizzano un sistema di **sessioni di upload**: avanzamento individuale per file, retry del file in errore senza reinviare gli altri, dismiss della sessione quando terminata. L'OCR espone un **punteggio di confidenza** medio (`average`, clampato in `[0,1]`, calcolato a partire da `averagePageConfidenceScore` restituiti da Mistral), visualizzato nell'UI sotto forma di badge tier `high` / `medium` / `low` (soglie ~0.9 / ~0.7) — avvisa senza bloccare se la scansione è di bassa qualità.
...
Il router utilizza `mistral-small-latest` per analizzare il contenuto delle sorgenti e proporre i generatori più pertinenti tra i 7 disponibili. L'interfaccia mostra l'avanzamento in tempo reale: prima una fase di analisi, poi le generazioni individuali con possibilità di annullamento.
...
- **Rilevamento delle istruzioni**: rileva le istruzioni di revisione ("So la mia lezione se so...") e le prioritizza nei generatori testuali compatibili (scheda, flashcard, quiz, testi con spazi vuoti)
...
Ogni chiamata Mistral (chat, OCR, STT, TTS, moderazione, agenti) è strumentata per fornire una stima € **trasparente** all'utente — nessuna sorpresa nella fatturazione.
...
- **Unità supportate**: `tokens`, `characters` (TTS), `pages` (OCR), `audio-seconds` (STT) — conversione pilotata da `helpers/cost-calc.ts`
...
|---|---|---|
| `mistral-large-latest` | Scheda di ripasso, flashcard, podcast, quiz, testi a riempimento, chat, verifica quiz vocale, agente immagini, agente di ricerca web, rilevamento consegne | Miglior supporto multilingue + rispetto delle istruzioni |
...
| `mistral-moderation-latest` | Moderazione dei contenuti | 5 categorie bloccate per bambini/adolescenti (+ jailbreak) |
...
| Variabile | Richiesta | Predefinito | Ruolo |
|---|---|---|---|
...
**Strumenti esterni richiesti (opzionali ma utilizzati da `pretest` / `npm run security`)**:
...
Senza questi strumenti, `npm test` fallisce in `pretest` (lizard assente) e `npm run security` fallisce (opengrep assente). Gli hook husky bloccano quindi il commit/push.
...
> **Per i contributori IA**: consultare [`CLAUDE.md`](CLAUDE.md) per il contesto architetturale dettagliato, le regole obbligatorie (anti-leak prompt, codici di errore, cost tracking) e le insidie note (Lizard CCN, Opengrep, migrazione Codacy/Semgrep).
...
| `GET` | `/api/config/status` | Stato delle API: `mistral` (chiave Mistral impostata), `ttsAvailable` (alias di `mistral`, Mistral Voxtral è l'unico provider TTS) |
...
| `POST` | `/api/projects/:pid/sources/websearch` | Scraping URL o ricerca web `{query}` — restituisce una tabella di sorgenti |
...
| `POST` | `/api/projects/:pid/generate/fill-blank` | Testi a riempimento |
...
| `POST` | `/api/projects/:pid/generate/route` | Analisi di routing (piano dei generatori da avviare) — restituisce `{plan, costDelta}` (solo costo del routing) |
| `POST` | `/api/projects/:pid/generate/auto` | Generazione automatica backend (routing + 7 tipi: summary, flashcards, quiz, fill-blank, podcast, quiz-vocale, image). Esecuzione in parallelo — presuppone un tier Mistral con rate-limit ≥ 7 richieste simultanee; altrimenti più errori 429 possono risalire in `failedSteps`. |
...
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | Inviare le risposte dei testi a riempimento `{answers}` |
...
| **Persistenza in file JSON** | Zero dipendenze, avvio immediato. Nessun database da configurare — si parte e via. |
| **Vite + Handlebars** | Il meglio di entrambi i mondi: HMR veloce per lo sviluppo, partial HTML per organizzare il codice, Tailwind JIT. |
| **Prompt centralizzati** | Tutti i prompt IA in `prompts.ts` — facile iterare, testare e adattare per lingua/gruppo di età. |
...
| **Funzionalità basate sugli Agenti** | La generazione di immagini e la ricerca web usano Agenti Mistral temporanei — ciclo di vita pulito con pulizia automatica. |
| **Scraping intelligente di URL** | Un campo unico accetta URL e parole chiave mescolate — gli URL vengono scrapati via Readability (pagine statiche) con fallback Lightpanda (pagine JS/SPA), le parole chiave attivano un Agente Mistral web_search. Ogni risultato crea una sorgente indipendente. |
