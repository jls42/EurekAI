...
**EurekAI** è nato durante il [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([sito ufficiale](https://worldwide-hackathon.mistral.ai/)) (marzo 2026). Mi serviva un tema — e l'idea è arrivata da qualcosa di molto concreto: preparo regolarmente i compiti in classe con mia figlia, e mi sono detto che doveva essere possibile rendere tutto questo più ludico e interattivo grazie all'IA.

L'obiettivo: prendere **qualsiasi input** — una foto della lezione, un testo copiato e incollato, una registrazione vocale, una ricerca web — e trasformarlo in **schede di ripasso, flashcards, quiz, podcast, testi a completamento, illustrazioni e molto altro**. Il tutto alimentato dai modelli francesi di Mistral AI, il che lo rende una soluzione naturalmente adatta agli studenti francofoni.

Il [prototipo iniziale](https://github.com/jls42/worldwide-hackathon.mistral.ai) è stato realizzato in 48 ore durante l'hackathon come proof of concept attorno ai servizi Mistral — già funzionante, ma limitato. Da allora, EurekAI è diventato un vero progetto: testi a completamento, navigazione negli esercizi, scraping web, moderazione parentale configurabile, revisione approfondita del codice e molto altro. L'intero codice è generato dall'IA — principalmente [Claude Code](https://code.claude.com/), con alcuni contributi tramite [Codex](https://openai.com/codex/) e [Gemini CLI](https://geminicli.com/).
...
|---|---|---|
| 📷 | **Importazione file** | Importa le tue lezioni — foto, PDF (tramite Mistral OCR con punteggio di confidenza medio, livelli `high`/`medium`/`low`) oppure file di testo (TXT, MD). Sessioni di upload con retry per file e avanzamento individuale |
...
| 🃏 | **Flashcards** | Carte Q/R interattive, lettura audio dialogata |
...
| 🔤 | **Dettato** | Parole dette in audio (Voxtral TTS) da una lista importata, digitazione da tastiera, correzione rigorosa lettera per lettera con regola ortografica spiegata |
...
| 💬 | **Tutor IA** | Chat contestuale con i documenti delle tue lezioni, con chiamata di strumenti |
...
EurekAI accetta 4 tipi di sorgenti, moderati in base al profilo (attivata di default per bambino e adolescente) :

- **Importazione file** — File JPG, PNG o PDF elaborati da OCR Mistral — **OCR 4 (`mistral-ocr-4-0`) di default** (migliore qualità), **OCR 3 (`mistral-ocr-2512`) opzionale** nelle Impostazioni (più economico, ~½ del costo) — per testo stampato, tabelle e scrittura a mano; oppure file di testo (TXT, MD) importati direttamente. Gli upload multi-file usano un sistema di **sessioni di upload**: avanzamento individuale per file, retry del file in errore senza reinviare gli altri, dismiss della sessione una volta terminata. L'OCR espone un **punteggio di confidenza** medio (`average`, limitato in `[0,1]`, calcolato a partire da `averagePageConfidenceScore` restituiti da Mistral), visualizzato nell'UI come badge di livello `high` / `medium` / `low` (soglie ~0.9 / ~0.7) — avvisa senza bloccare se la scansione è di scarsa qualità.
...
| **Dettato** | `mistral-large-latest` + Voxtral TTS | Parole chiave dette in audio (1 MP3/parola) → digitazione da tastiera → correzione rigorosa (accenti) con regola spiegata |
...
Un tutor conversazionale con accesso completo ai documenti delle lezioni:
...
- Cronologia di 50 messaggi per lezione
- Moderazione dei contenuti se attivata per il profilo

### Router automatico

Il router utilizza `mistral-small-latest` per analizzare il contenuto delle sorgenti e proporre i generatori più pertinenti tra gli 8 disponibili. L'interfaccia mostra l'avanzamento in tempo reale: prima una fase di analisi, poi le generazioni individuali con possibilità di annullamento.
...
- **Rilevamento delle istruzioni**: rileva le istruzioni di ripasso ("So la mia lezione se so...") e le prioritizza nei generatori testuali compatibili (scheda, flashcards, quiz, testi a completamento)

### Sicurezza e controllo parentale

- **4 gruppi di età**: bambino (≤10 anni), adolescente (11-15), studente (16-25), adulto (26+)
- **Moderazione dei contenuti**: `mistral-moderation-2603` (Mistral Moderation 2) con 10 categorie disponibili, 5 bloccate di default per bambino/adolescente (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Categorie personalizzabili per profilo nelle impostazioni. L'alias `-latest` è volutamente evitato (punta ancora a una versione deprecata).
...
- **Voci per profilo** (`Profile.mistralVoices?: { host, guest }`) — ogni bambino può avere la propria coppia di voci per podcast/quiz vocali
...
- **Voci personalizzate**: i genitori possono creare le proprie voci tramite l'API Mistral Voices (a partire da un campione audio) e assegnarle ai ruoli host/guest — i podcast e i quiz vocali vengono così letti con la voce di un genitore, rendendo l'esperienza ancora più immersiva per il bambino
...
| **Headless browser** | Lightpanda | Browser headless ultraleggero (Zig + V8) per le pagine JS/SPA — scraping di fallback |
...
| **Test** | Vitest | Test unitari — coverage misurata da SonarCloud |
...
|---|---|---|
| `mistral-large-latest` | Scheda, Flashcards, Podcast, Quiz, Testi a completamento, Chat, Verifica quiz vocale, Agent Immagine, Agent Web Search, Rilevamento istruzioni | Migliore multilingua + rispetto delle istruzioni |
| `mistral-ocr-4-0` (OCR 4, default) | OCR di documenti — qualità superiore | Testo stampato, tabelle, scrittura a mano ($4 / 1000 pagine) |
...
| `mistral-moderation-2603` | Moderazione dei contenuti | 5 categorie bloccate per bambino/adolescente (tra cui `jailbreaking`) |
...
| `HTTPS_KEY` / `HTTPS_CERT` | opzionale | — | Percorsi chiave/cert TLS (cfr. `scripts/gen-cert.sh`) → Express e Vite servono in HTTPS (contesto sicuro LAN/tablet) |
...
> **Per i contributori IA** : consultare [`CLAUDE.md`](CLAUDE.md) per il contesto architetturale dettagliato, le regole obbligatorie (anti-leak prompts, codici di errore, cost tracking) e le insidie note (Lizard CCN, Opengrep, migrazione Codacy/Semgrep).
...
| `PUT` | `/api/config` | Modifica la config (modelli, voce, modello TTS) |
...
| `POST` | `/api/config/reset` | Reimposta la config predefinita |
...
| `POST` | `/api/providers/mistral/validate` | Valida una chiave Mistral inserita dall’utente — sempre 200 `{status}` (`ok`/`invalid`/`quota`/`network`/`missing`), nessun fallback env |
...
| `DELETE` | `/api/profiles/:id` | Elimina un profilo + cascata progetti `{pin?}` → `{ok, deletedProjects}` |
...
| `POST` | `/api/projects/:pid/sources/websearch` | Scraping di URL o ricerca web `{query}` — restituisce un array di fonti |
| `DELETE` | `/api/projects/:pid/sources/:sid` | Elimina una fonte |
| `POST` | `/api/projects/:pid/moderate` | Modera `{text}` |
| `POST` | `/api/projects/:pid/detect-consigne` | Rileva le istruzioni di revisione |
...
| `POST` | `/api/projects/:pid/generate/remediation-summary` | Scheda di promemoria mirata sulle domande sbagliate di un quiz `{generationId, weakQuestions}` — chiamata in parallelo a `quiz-review` dal pulsante « Allenarmi sui miei errori » |
...
| `POST` | `/api/projects/:pid/generate/auto` | Generazione automatica backend (routing + 8 tipi: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image, dictation). Esecuzione in parallelo — presuppone un tier Mistral con rate-limit ≥ 8 richieste simultanee ; altrimenti più 429 possono risalire in `failedSteps`. |
...
| `POST` | `/api/projects/:pid/generations/:gid/dictation-attempt` | Invia le risposte del dettato `{answers}` (punteggio server rigoroso) |
...
| `POST` | `/api/projects/:pid/generations/:gid/cancel` | Annulla una generazione in corso (unico percorso di annullamento di un pending) |
...
| **Scraping intelligente di URL** | Un campo unico accetta URL e parole chiave mescolati — gli URL vengono scrapati tramite Readability (pagine statiche) con fallback Lightpanda (pagine JS/SPA), le parole chiave attivano un Agent Mistral web_search. Ogni risultato crea una fonte indipendente. |
| **TTS 100% Mistral** | Mistral Voxtral TTS (nessuna chiave aggiuntiva oltre a `MISTRAL_API_KEY`) — sintesi vocale integrata nella catena di costo e nella risoluzione voce per lingua. |
...
- **[Vite](https://vitejs.dev)** — Tool di build frontend
...
- **[Lightpanda](https://lightpanda.io)** — Navigatore headless ultra-leggero per lo scraping di pagine JS/SPA
