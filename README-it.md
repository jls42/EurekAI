...
**EurekAI** è nato durante l'[Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([sito ufficiale](https://worldwide-hackathon.mistral.ai/)) (marzo 2026). Avevo bisogno di un tema — e l'idea è venuta da qualcosa di molto concreto: preparo regolarmente le verifiche con mia figlia, e ho pensato che fosse possibile rendere tutto più divertente e interattivo grazie all'IA.

L'obiettivo: prendere **qualsiasi input** — una foto della lezione, un testo copiato e incollato, una registrazione vocale, una ricerca web — e trasformarlo in **schede di revisione, flashcard, quiz, podcast, testi con spazi vuoti, illustrazioni e altro ancora**. Il tutto alimentato dai modelli francesi di Mistral AI, il che lo rende una soluzione naturalmente adatta agli studenti francofoni.

Il [prototipo iniziale](https://github.com/jls42/worldwide-hackathon.mistral.ai) è stato progettato in 48 ore durante l'hackathon come proof of concept attorno ai servizi Mistral — già funzionante, ma limitato. Da allora, EurekAI è diventato un vero progetto: testi con spazi vuoti, navigazione negli esercizi, scraping web, moderazione parentale configurabile, revisione del codice approfondita e molto altro. L'intero codice è generato dall'IA — principalmente [Claude Code](https://code.claude.com/), con alcuni contributi tramite [Codex](https://openai.com/codex/) e [Gemini CLI](https://geminicli.com/).
...
<p align="center">
  <img src="public/assets/architecture-overview.webp" alt="Panoramica architettura" width="800" />
</p>
...
<p align="center">
  <img src="public/assets/model-map.webp" alt="Mappatura Modello→Compito AI" width="800" />
</p>
...
- **Importazione file** — File JPG, PNG o PDF elaborati da `mistral-ocr-latest` (testo stampato, tabelle, scrittura manuale), o file di testo (TXT, MD) importati direttamente.
...
- **Web / URL** — Incolla una o più URL per eseguire lo scraping del contenuto direttamente (Readability + Lightpanda per le pagine JS), o digita parole chiave per una ricerca web tramite Agent Mistral. Il campo unico accetta entrambe le tipologie — URL e parole chiave sono separate automaticamente, ogni risultato crea una sorgente indipendente.
...
- **Rilevamento di istruzioni**: rileva le istruzioni di revisione ("So la mia lezione se so...") e le priorizza nei generatori testuali compatibili (scheda, flashcard, quiz, testi con spazi vuoti)
...
- **Restrizioni della chat**: la chat IA è disattivata di default per i minori di 16 anni, attivabile dai genitori
...
- **Mistral Voxtral TTS** (predefinito): `voxtral-mini-tts-latest`, nessuna chiave aggiuntiva richiesta
...
| **Browser headless** | Lightpanda | Browser headless ultra-leggero (Zig + V8) per le pagine JS/SPA — fallback scraping |
...
|---|---|---|
| `mistral-large-latest` | Scheda, Flashcards, Podcast, Quiz, Testi con spazi vuoti, Chat, Verifica quiz vocale, Agent Image, Agent Web Search, Rilevamento istruzioni | Migliore multilingua + seguito delle istruzioni |
| `mistral-ocr-latest` | OCR di documenti | Testo stampato, tabelle, scrittura manuale |
...
| `POST` | `/api/config/reset` | Reimposta la config di default |
| `GET` | `/api/config/voices` | Lista delle voci Mistral TTS (opzionale `?lang=fr`) |
...
| `DELETE` | `/api/projects/:pid` | Elimina il progetto | ### Sorgenti
| Metodo | Endpoint | Descrizione |
|---|---|---|
...
| `POST` | `/api/projects/:pid/generate/quiz` | Quiz a scelta multipla (QCM) |
...
| `POST` | `/api/projects/:pid/generate/route` | Analisi di instradamento (piano dei generatori da avviare) |
| `POST` | `/api/projects/:pid/generate/auto` | Generazione auto backend (routage + 5 types : summary, flashcards, quiz, fill-blank, podcast) |
...
| **Prompt adattati per età** | 4 gruppi d'età con vocabolario, complessità e tono diversi — lo stesso contenuto insegna in modo differente a seconda dell'apprendente. |
| **Funzionalità basate sugli Agent** | La generazione di immagini e la ricerca web utilizzano Agent Mistral temporanei — ciclo di vita pulito con pulizia automatica. |
| **Scraping intelligente di URL** | Un campo unico accetta URL e parole chiave miste — le URL vengono scrapate tramite Readability (pagine statiche) con fallback Lightpanda (pagine JS/SPA), le parole chiave attivano un Agent Mistral web_search. Ogni risultato crea una sorgente indipendente. |
...
**Questo documento è stato tradotto dalla versione fr alla lingua it utilizzando il modello gpt-5-mini. Per ulteriori informazioni sul processo di traduzione, consultate https://gitlab.com/jls42/ai-powered-markdown-translator**
