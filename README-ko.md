...
<p align="center">
  <strong>어떤 콘텐츠든 인터랙티브한 학습 경험으로 바꿉니다 — <a href="https://mistral.ai">Mistral AI</a> 기반.</strong>
</p>
...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI"><img src="https://img.shields.io/badge/▶️_데모_보기-YouTube-red?style=for-the-badge&logo=youtube" alt="YouTube 데모"></a>
</p>
...
**EurekAI**는 [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([공식 사이트](https://worldwide-hackathon.mistral.ai/))(2026년 3월) 중에 탄생했습니다. 주제가 필요했고, 아이디어는 아주 구체적인 것에서 떠올랐습니다. 저는 딸과 함께 정기적으로 시험 대비를 하는데, AI를 활용하면 이를 더 재미있고 인터랙티브하게 만들 수 있겠다고 생각했습니다.

목표는 **어떤 입력이든** — 수업 사진, 복사해 넣은 텍스트, 음성 녹음, 웹 검색 — **복습 노트, 플래시카드, 퀴즈, 팟캐스트, 빈칸 채우기, 삽화 등**으로 바꾸는 것입니다. 모두 Mistral AI의 프랑스 모델로 구동되어, 프랑스어권 학생들에게 자연스럽게 적합한 솔루션이 됩니다.

[초기 프로토타입](https://github.com/jls42/worldwide-hackathon.mistral.ai)은 해커톤 동안 Mistral 서비스에 대한 개념 증명으로 48시간 만에 만들어졌습니다 — 이미 작동했지만 제한적이었습니다. 이후 EurekAI는 진짜 프로젝트가 되었습니다: 빈칸 채우기, 연습문제 탐색, 웹 스크래핑, 구성 가능한 부모용 모더레이션, 심층 코드 리뷰 등등. 코드 전체는 AI로 생성되며 — 주로 [Claude Code](https://code.claude.com/)를 사용하고, 일부는 [Codex](https://openai.com/codex/)와 [Gemini CLI](https://geminicli.com/)의 도움을 받았습니다.
...
| | 기능 | 설명 |
|---|---|---|
| 📷 | **파일 가져오기** | 수업 자료를 가져오세요 — 사진, PDF(`high`/`medium`/`low`의 평균 신뢰도 점수를 사용하는 Mistral OCR 기반) 또는 텍스트 파일(TXT, MD). 파일별 재시도와 개별 진행률이 있는 업로드 세션 |
| 📝 | **텍스트 입력** | 어떤 텍스트든 직접 입력하거나 붙여넣기 |
| 🎤 | **음성 입력** | 녹음하세요 — Voxtral STT가 음성을 전사합니다 |
| 🌐 | **웹 / URL** | URL을 붙여넣거나(Readability + Lightpanda로 직접 스크래핑) 검색어를 입력하세요(Mistral web_search 에이전트) |
| 📄 | **복습 노트** | 핵심 포인트, 어휘, 인용문, 일화를 담은 구조화된 노트 |
| 🃏 | **플래시카드** | 인터랙티브한 Q/A 카드, 대화형 음성 읽기 |
| ❓ | **객관식 퀴즈** | 오류에 대한 적응형 복습이 있는 객관식 문제(개수 설정 가능) |
| ✏️ | **빈칸 채우기** | 힌트와 관대한 검증이 있는 완성형 연습문제 |
| 🎙️ | **팟캐스트** | 2인 목소리의 짧은 오디오 팟캐스트 — 기본은 Mistral 음성 또는 사용자 지정 음성(부모용!) |
| 🖼️ | **삽화** | Mistral 에이전트가 생성한 교육용 이미지 |
| 🗣️ | **음성 퀴즈** | 소리 내어 읽는 문제(사용자 지정 음성 가능), 음성 답변, AI 검증 |
| 💬 | **AI 튜터** | 도구 호출이 가능한, 수업 자료와의 맥락형 채팅 |
| 🧠 | **자동 라우터** | `mistral-small-latest` 기반 라우터가 콘텐츠를 분석하고 사용 가능한 7가지 유형 중 생성기 조합을 제안 |
| 🔒 | **부모 통제** | 프로필별 구성 가능한 모더레이션(카테고리 사용자 지정 가능), 부모 PIN, 채팅 제한 |
| 🌍 | **다국어 지원** | 인터페이스는 9개 언어로 제공되며; AI 생성은 프롬프트를 통해 15개 언어로 제어 가능 |
| 🔊 | **음성 읽기** | Mistral Voxtral TTS로 노트와 플래시카드(질문/응답 대화)를 들을 수 있음 |
| 💶 | **API 비용 추적** | 각 생성과 출처에 대한 € 비용을 투명하게 추정(토큰 / 문자 / 페이지 / 오디오 초). 카드별 배지 + 프로젝트별 총합, 대시보드에서 확인 가능 |
...
EurekAI는 프로필에 따라 조정된 4가지 소스 유형을 지원합니다(어린이와 청소년은 기본 활성화) :

- **파일 가져오기** — JPG, PNG 또는 PDF 파일은 `mistral-ocr-latest`(인쇄 텍스트, 표, 손글씨)로 처리되며, 텍스트 파일(TXT, MD)은 직접 가져옵니다. 다중 파일 업로드는 **업로드 세션** 시스템을 사용합니다: 파일별 개별 진행률, 다른 파일을 다시 제출하지 않고 실패한 파일만 재시도, 완료 시 세션 닫기. OCR은 Mistral이 반환한 `averagePageConfidenceScore`를 기반으로 계산된 **평균 신뢰도 점수**(`average`, `[0,1]`로 제한됨)를 노출하며, UI에는 임계값 ~0.9 / ~0.7에 해당하는 `high` / `medium` / `low` 배지로 표시됩니다 — 품질이 좋지 않은 스캔은 차단하지 않고 경고만 표시합니다.
- **자유 텍스트** — 어떤 콘텐츠든 입력하거나 붙여넣으세요. 모더레이션이 활성화되어 있으면 저장 전에 검열됩니다.
- **음성 입력** — 브라우저에서 오디오를 녹음합니다. `voxtral-mini-latest`로 전사됩니다. `language="fr"` 파라미터가 인식을 최적화합니다.
- **웹 / URL** — 하나 이상의 URL을 붙여넣어 콘텐츠를 직접 스크래핑하거나(Readability + Lightpanda, JS 페이지용), 키워드를 입력해 Mistral Agent를 통한 웹 검색을 수행할 수 있습니다. 하나의 입력 필드에서 둘 다 허용되며 — URL과 키워드는 자동으로 분리되고, 각 결과는 독립된 소스로 생성됩니다.
...
| **팟캐스트** | `mistral-large-latest` + Voxtral TTS | 2인용 스크립트 → MP3 오디오 |
| **삽화** | `mistral-large-latest` 에이전트 | `image_generation` 도구를 통한 교육용 이미지 |
| **음성 퀴즈** | `mistral-large-latest` + Voxtral TTS + STT | TTS 문제 → STT 답변 → AI 검증 |

### 채팅형 AI 튜터

수업 자료에 완전히 접근할 수 있는 대화형 튜터:
...
라우터는 `mistral-small-latest`를 사용해 소스 콘텐츠를 분석하고, 7가지 사용 가능한 생성기 중 가장 관련성 높은 것을 제안합니다. 인터페이스는 실시간 진행 상황을 보여줍니다: 먼저 분석 단계, سپس 개별 생성이 진행되며 취소도 가능합니다.

### 적응형 학습

- **퀴즈 통계**: 문제별 시도 횟수와 정답률 추적
- **퀴즈 복습**: 약한 개념을 겨냥한 5~10개의 새 문제 생성
- **지시문 감지**: 복습 지시문("나는 ...를 알면 수업을 안다")을 감지해 호환되는 텍스트 생성기(노트, 플래시카드, 퀴즈, 빈칸 채우기)에서 우선 처리

### 보안 및 부모 통제

- **4개 연령 그룹**: 어린이(≤10세), 청소년(11-15), 학생(16-25), 성인(26+)
- **콘텐츠 모더레이션**: 10개 카테고리를 제공하는 `mistral-moderation-latest`, 그중 5개는 어린이/청소년에게 기본 차단(`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). 카테고리는 설정에서 프로필별로 사용자 지정 가능.
- **부모 PIN**: SHA-256 해시, 15세 미만 프로필에 필요. 프로덕션 배포에서는 느린 솔트 해시(Argon2id, bcrypt)를 사용하는 것이 좋습니다.
- **채팅 제한**: 16세 미만은 AI 채팅이 기본 비활성화되며, 부모가 활성화할 수 있음

### 다중 프로필 시스템

- 이름, 나이, 아바타, 언어 환경설정을 가진 여러 프로필
...
- 계단식 삭제: 프로필을 삭제하면 그 프로필의 모든 프로젝트가 삭제됨

### API 비용 추적

각 Mistral 호출(chat, OCR, STT, TTS, moderation, agents)은 사용자에게 **투명한** € 추정을 제공하도록 계측됩니다 — 청구에 대한 놀라움이 없습니다.

- **기준 소스**: `helpers/pricing.ts` — 모델 prefix별 `MODEL_PRICING`(예: `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), 주기적 재스크래핑을 위한 Mistral 문서 URL이 포함된 `PRICING_SOURCES`
- **지원 단위**: `tokens`, `characters`(TTS), `pages`(OCR), `audio-seconds`(STT) — `helpers/cost-calc.ts`가 주도하는 변환
...
- **UI**: 생성별(`src/partials/cost-badge-gen.html`), 소스별(`cost-badge-src.html`), 대시보드의 누적 총합(`Project.totalCost`) 비용 배지
- **엔드포인트**: `/generate/*` 및 `/sources/*` 응답은 반환된 객체(Generation / Source)에 `estimatedCost`, `usage`, `costBreakdown`를 덧붙입니다. `POST /generate/auto/route`는 라우팅 전용 비용을 위한 `costDelta: number` 필드를 추가합니다. `GET /projects/:pid`는 `costLog[]`에서 계산한 합계와 전체 기록이 포함된 `totalCost`로 보강된 프로젝트를 반환합니다

### 멀티 공급자 TTS 및 사용자 지정 음성

- **Mistral Voxtral TTS**: `voxtral-mini-tts-latest`, 100% Mistral 음성 합성, 추가 키 불필요
- **사용자 지정 음성**: 부모는 Mistral Voices API(오디오 샘플 기반)를 통해 자신만의 음성을 만들고 호스트/게스트 역할에 할당할 수 있으며 — 팟캐스트와 음성 퀴즈는 부모의 목소리로 읽혀 아이에게 더욱 몰입감 있는 경험을 제공합니다
- 두 개의 구성 가능한 음성 역할: **호스트**(주 내레이터)와 **게스트**(팟캐스트의 두 번째 목소리)
- Mistral의 전체 음성 카탈로그는 설정에서 확인 가능하며, 언어로 필터링 가능
...
| **템플릿** | vite-plugin-handlebars | partials를 통한 HTML 구성 |
...
| **테스트** | Vitest | 단위 테스트 — SonarCloud로 측정된 커버리지 |
| **저장소** | JSON 파일 | 종속성 없는 저장 |

---

## 모델 참조 | 모델 | 용도 | 이유 |
|---|---|---|
| `mistral-large-latest` | 요약 노트, 플래시카드, 팟캐스트, 퀴즈, 빈칸 채우기, 채팅, 음성 퀴즈 검증, 이미지 에이전트, 웹 검색 에이전트, 지시 감지 | 최고의 다국어 성능 + 지시 사항 준수 |
...
| `mistral-moderation-latest` | 콘텐츠 모더레이션 | 아동/청소년용 차단 카테고리 5개(+ 탈옥) |
...
> **참고** : Mistral Voxtral TTS는 유일한 TTS 제공업체입니다. `MISTRAL_API_KEY` 외에 추가 키는 필요하지 않습니다.
...
| `NODE_ENV` | 선택 사항 | `development` | `production`이면 Express가 `dist/`에서 프런트엔드를 제공(그렇지 않으면 `public/`) |
...
**Git 훅(Husky)**: `pre-commit`는 `npm test`를 실행하고, `pre-push`는 `npm run security`를 실행합니다. 둘 다 실패 시 커밋/푸시를 차단합니다.
...
> **AI 기여자용**: 상세한 아키텍처 맥락, 필수 규칙(프롬프트 유출 방지, 오류 코드, 비용 추적) 및 알려진 함정(Lizard CCN, Opengrep, Codacy/Semgrep 마이그레이션)은 [`CLAUDE.md`](CLAUDE.md)를 참조하세요.
...
| `PUT` | `/api/config` | 설정 변경(모델, 음성, TTS 모델) |
| `GET` | `/api/config/status` | API 상태: `mistral`(Mistral 키가 설정됨), `ttsAvailable`(`mistral`의 별칭, Mistral Voxtral은 유일한 TTS 제공업체) |
...
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `GET` | `/api/profiles` | 모든 프로필 सूची |
| `POST` | `/api/profiles` | 프로필 생성 |
| `PUT` | `/api/profiles/:id` | 프로필 수정(< 15세는 PIN 필요) |
| `DELETE` | `/api/profiles/:id` | 프로필 삭제 + 프로젝트 `{pin?}` → `{ok, deletedProjects}` 연쇄 삭제 |

### 프로젝트
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `GET` | `/api/projects` | 프로젝트 목록(`?profileId=` 선택 사항) |
...
| `POST` | `/api/projects/:pid/generate/auto` | 백엔드 자동 생성(라우팅 + 7개 유형: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). 병렬 실행 — 초당 동시 요청 7개 이상인 Mistral 티어를 전제로 함; 그렇지 않으면 여러 429가 `failedSteps`에 표시될 수 있음. |

모든 생성 라우트는 `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}`를 허용합니다. `quiz-review`는 추가로 `{generationId, weakQuestions}`를 요구합니다.
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS 음성 읽기(노트/플래시카드) |
...
| 결정 | 근거 |
|---|---|
| **React/Vue 대신 Alpine.js** | 최소한의 용량, Vite로 컴파일되는 TypeScript와 가벼운 반응성. 속도가 중요한 해커톤에 적합합니다. |
| **JSON 파일로 영속성 유지** | 의존성 없음, 즉시 시작. 설정할 데이터베이스가 없습니다 — 시작하고 바로 사용 가능합니다. |
| **Vite + Handlebars** | 개발용 빠른 HMR, 코드 구성을 위한 HTML partials, Tailwind JIT 등 두 세계의 장점을 모두 제공합니다. |
| **중앙화된 프롬프트** | 모든 AI 프롬프트가 `prompts.ts`에 있어 언어/연령대별로 쉽게 반복, 테스트, 조정할 수 있습니다. |
| **다중 생성 시스템** | 각 생성은 자체 ID를 가진 독립 객체이며, 하나의 수업에서 여러 노트, 퀴즈 등을 허용합니다. |
| **연령별 맞춤 프롬프트** | 4개의 연령 그룹에 대해 어휘, 복잡성, 톤이 다름 — 같은 콘텐츠라도 학습자에 따라 다르게 가르칩니다. |
| **에이전트 기반 기능** | 이미지 생성과 웹 검색은 임시 Mistral 에이전트를 사용하며, 자동 정리되는 깔끔한 생명 주기를 가집니다. |
| **지능형 URL 스크래핑** | 하나의 입력 필드에서 URL과 키워드를 혼합해 받을 수 있습니다 — URL은 Readability(정적 페이지)로 스크래핑하고 Lightpanda(JS/SPA 페이지)로 폴백하며, 키워드는 Mistral web_search 에이전트를 트리거합니다. 각 결과는 독립된 소스를 생성합니다. |
| **100% Mistral TTS** | Mistral Voxtral TTS(`MISTRAL_API_KEY` 외 추가 키 없음) — 비용 체인과 언어별 음성 해석에 통합된 음성 합성입니다. |
...
Mistral AI Worldwide Hackathon(2026년 3월)에서 시작되어, [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) 및 [Gemini CLI](https://geminicli.com/)로 전적으로 AI가 개발했습니다.
