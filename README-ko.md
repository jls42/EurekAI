...
<p align="center">
  ...
  <a href="https://sonarcloud.io/summary/new_code?id=jls42_EurekAI"><img src="https://sonarcloud.io/api/project_badges/measure?project=jls42_EurekAI&metric=code_smells&token=c8f6207cabba0e2841b2ec04126d7fd6687397e8" alt="코드 스멜"></a>
...
**EurekAI**는 [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([공식 사이트](https://worldwide-hackathon.mistral.ai/))에서 탄생했습니다 (2026년 3월). 주제가 필요했는데, 아이디어는 매우 실용적인 상황에서 나왔습니다: 저는 딸과 함께 자주 시험을 준비하는데, 인공지능으로 이를 더 재미있고 대화형으로 만들 수 있겠다고 생각했습니다.

목표는 다음과 같습니다: **어떤 입력이든** 받아들여 — 교재 사진, 복사해 붙여넣은 텍스트, 음성 녹음, 웹 검색 등 — 이를 **요약 노트, 플래시카드, 퀴즈, 팟캐스트, 빈칸 채우기 문제, 삽화 등**으로 변환하는 것입니다. 모든 것은 Mistral AI의 프랑스계 모델로 구동되어 프랑스어 사용 학습자에게 자연스럽게 적합합니다.

프로젝트는 해커톤 동안 시작되어 이후 확장 및 개선되었습니다. 전체 코드의 대부분은 AI로 생성되었으며 — 주로 [Claude Code](https://docs.anthropic.com/en/docs/claude-code)로 작성되었고, 일부는 [Codex](https://openai.com/index/introducing-codex/)를 통해 기여되었습니다.
...
| 🎤 | **음성 입력** | 녹음하면 Voxtral STT가 음성을 텍스트로 변환 |
| 🌐 | **웹 검색** | 질문을 입력하면 Mistral 에이전트가 웹에서 답을 찾습니다 |
| 📄 | **요약 노트** | 핵심 포인트, 어휘, 인용, 일화가 포함된 구조화된 노트 |
| 🃏 | **플래시카드** | 출처 참조가 포함된 5-50장 Q/A 카드로 능동적 암기 지원 |
| ❓ | **객관식 퀴즈** | 5-50문항의 선다형 문제와 실수에 대한 적응형 복습 |
| ✏️ | **빈칸 채우기** | 힌트와 관용적 검증이 포함된 완성형 연습문제 |
| 🎙️ | **팟캐스트** | Mistral Voxtral TTS로 변환되는 2인 구성의 미니 팟캐스트 |
| 🖼️ | **삽화** | Mistral 에이전트가 생성한 교육용 이미지 |
| 🗣️ | **음성 퀴즈** | 질문을 음성으로 재생하고 구두 답변을 제출하면 AI가 검증 |
| 💬 | **AI 튜터** | 수업 자료에 맥락적으로 접근 가능한 대화형 튜터, 도구 호출 포함 |
| 🧠 | **자동 라우터** | `mistral-small-latest` 기반 라우터가 콘텐츠를 분석하고 7가지 생성기 중 조합을 제안 |
| 🔒 | **부모 통제** | 연령별 검열, 부모 PIN, 채팅 제한 |
| 🌍 | **다국어 지원** | 인터페이스 9개 언어 제공; 프롬프트로 15개 언어의 AI 생성 제어 가능 |
| 🔊 | **음성 재생** | Mistral Voxtral TTS 또는 ElevenLabs로 노트 및 플래시카드 음성 재생 |
...
EurekAI는 프로필에 따라(기본적으로 어린이와 청소년에 활성화) 4가지 소스 타입을 허용합니다:

- **OCR 업로드** — JPG, PNG 또는 PDF 파일을 `mistral-ocr-latest`로 처리합니다. 인쇄된 텍스트, 표, 손글씨를 처리합니다.
- **자유 텍스트** — 어떤 콘텐츠든 입력하거나 붙여넣을 수 있습니다. 저장 전 검열이 활성화되어 있으면 검열을 거칩니다.
- **음성 입력** — 브라우저에서 오디오를 녹음합니다. `voxtral-mini-latest`로 전사됩니다. `language="fr"` 설정이 인식률을 최적화합니다.
- **웹 검색** — 쿼리를 입력하면 `web_search` 도구를 사용하는 임시 Mistral 에이전트가 결과를 수집하고 요약합니다.
...
| 생성기 | 모델 | 출력 |
|---|---|---|
| **요약 노트** | `mistral-large-latest` | 제목, 요약, 10-25 핵심 포인트, 어휘, 인용, 일화 |
| **플래시카드** | `mistral-large-latest` | 출처 참조가 포함된 5-50장 Q/A 카드 |
| **객관식 퀴즈** | `mistral-large-latest` | 5-50문항, 각 4지선다, 해설, 적응형 복습 |
| **빈칸 채우기** | `mistral-large-latest` | 힌트가 있는 완성형 문장, 관용적 검증(Levenshtein) |
| **팟캐스트** | `mistral-large-latest` + Voxtral TTS | 2인 스크립트 → MP3 오디오 |
| **삽화** | 에이전트 `mistral-large-latest` | 도구 `image_generation`을 통한 교육용 이미지 |
| **음성 퀴즈** | `mistral-large-latest` + Voxtral TTS + STT | TTS로 질문 제시 → STT로 답변 수집 → AI 검증 |
...
- **도구 호출** : 대화 중에 노트, 플래시카드, 퀴즈 또는 빈칸 문제를 생성할 수 있음
- 코스당 50개 메시지의 히스토리 보관
- 프로필에 따라 검열 적용 가능

### 자동 라우터

라우터는 `mistral-small-latest`를 사용해 소스의 콘텐츠를 분석하고 7가지 생성기 중 가장 적합한 것을 제안합니다. 인터페이스는 실시간 진행 상황을 표시합니다: 먼저 분석 단계가 있고, 이후 개별 생성이 진행되며 취소 가능.

### 적응형 학습

- **퀴즈 통계** : 문제별 시도 및 정확도 추적
- **퀴즈 복습** : 약한 개념을 겨냥한 5-10개의 새로운 질문 생성
- **지시사항 탐지** : 복습 지시("Je sais ma leçon si je sais...")를 감지하고 호환되는 텍스트 생성기(노트, 플래시카드, 퀴즈, 빈칸)에 우선 반영

### 보안 및 부모 통제

- **4개 연령 그룹** : 어린이(≤10세), 청소년(11-15세), 학생(16-25세), 성인(26세+)
- **콘텐츠 검열** : `mistral-moderation-latest`로 어린이/청소년에게 5개 카테고리(성적, 증오, 폭력, 자해, 탈옥 시도)를 차단, 학생/성인에는 제한 없음
- **부모 PIN** : SHA-256 해시, 15세 미만 프로필에 필요. 운영 환경에서는 소금(salt)이 있는 느린 해시(Argon2id, bcrypt)를 권장.
- **채팅 제한** : 기본적으로 16세 미만은 AI 채팅 비활성화, 부모가 활성화 가능

### 멀티 프로필 시스템

- 여러 프로필: 이름, 나이, 아바타, 언어 설정
- 프로젝트는 `profileId`를 통해 프로필에 연결
- 연쇄 삭제: 프로필 삭제 시 모든 관련 프로젝트 삭제
...
| 레이어 | 기술 | 역할 |
|---|---|---|
| **런타임** | Node.js + TypeScript 5.x | 서버 및 타입 안전성 |
| **백엔드** | Express 4.x | REST API |
| **개발 서버** | Vite 7.x + tsx | HMR, Handlebars partials, 프록시 |
...
| **TTS (대안)** | ElevenLabs SDK 2.x | `eleven_v3`, 자연스러운 음성 제공 |
...
| 모델 | 사용처 | 이유 |
|---|---|---|
| `mistral-large-latest` | 노트, 플래시카드, 팟캐스트, 퀴즈, 빈칸, 채팅, 음성 퀴즈 검증, 이미지 에이전트, 웹 검색 에이전트, 지시 탐지 | 다국어 우수 + 지시 따름 성능 우수 |
| `mistral-ocr-latest` | 문서 OCR | 인쇄 텍스트, 표, 손글씨 처리 |
| `voxtral-mini-latest` | 음성 인식 (STT) | 다국어 STT, `language="fr"`로 최적화 |
| `voxtral-mini-tts-latest` | 음성 합성 (TTS) | 팟캐스트, 음성 퀴즈, 읽어주기 용도 |
| `mistral-moderation-latest` | 콘텐츠 검열 | 어린이/청소년용 5개 차단 카테고리(+ 탈옥 시도) |
| `mistral-small-latest` | 자동 라우터 | 라우팅 결정을 위한 빠른 콘텐츠 분석 |
| `eleven_v3` (ElevenLabs) | 음성 합성 (대체 TTS) | 자연스러운 음성, 설정에서 대안으로 사용 가능 |
...
> **참고** : Mistral Voxtral TTS는 기본 제공자이며 — `MISTRAL_API_KEY` 외의 추가 키가 필요 없습니다. ElevenLabs는 설정에서 구성 가능한 대체 TTS 제공자입니다.
...
| `PUT` | `/api/profiles/:id` | 프로필 수정 (15세 미만 PIN 필요) |
| `DELETE` | `/api/profiles/:id` | 프로필 삭제 + 프로젝트 연쇄 삭제 `{pin?}` → `{ok, deletedProjects}` |

### 프로젝트
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `GET` | `/api/projects` | 프로젝트 목록 (`?profileId=` 선택적) |
...
| `POST` | `/api/projects/:pid/detect-consigne` | 복습 지시 탐지 |
...
모든 생성 라우트는 `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}`을 허용합니다. `quiz-review`은 추가로 `{generationId, weakQuestions}`을 요구합니다.
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS로 읽어주기 (노트/플래시카드) |
...
| 결정 | 정당성 |
|---|---|
| **Alpine.js 선택 (React/Vue 대신)** | 경량, 최소한의 오버헤드, Vite로 컴파일된 TypeScript와의 가벼운 반응성. 해커톤처럼 속도가 중요한 상황에 적합. |
| **JSON 파일로 영속성 유지** | 종속성 없음, 즉시 시작 가능. 데이터베이스 설정 불필요 — 바로 시작할 수 있음. |
| **Vite + Handlebars** | 두 세계의 장점: 개발을 위한 빠른 HMR, 코드 정리를 위한 HTML 부분 템플릿(partials), Tailwind JIT. |
| **Prompts centralisés** | 모든 AI 프롬프트가 `prompts.ts`에 있음 — 언어/연령대별로 반복, 테스트 및 조정하기 쉬움. |
| **Système multi-générations** | 각 생성물은 고유 ID를 가진 독립 객체 — 수업당 여러 학습지, 퀴즈 등을 허용. |
| **Prompts adaptés par âge** | 4개의 연령대 그룹으로 어휘, 복잡도 및 어조가 다름 — 동일한 내용이 학습자에 따라 다르게 가르쳐짐. |
| **Fonctionnalités basées sur les Agents** | 이미지 생성 및 웹 검색은 임시 Mistral 에이전트를 사용 — 자체 수명 주기와 자동 정리. |
| **TTS multi-provider** | 기본 TTS는 Mistral Voxtral TTS(추가 키 불필요), 대안으로 ElevenLabs — 재시작 없이 구성 가능. |
...
- **[Alpine.js](https://alpinejs.dev)** — 가벼운 리액티브 프레임워크
...
Mistral AI Worldwide Hackathon(2026년 3월) 동안 시작되었으며, Claude Code와 Codex를 사용하여 전적으로 AI에 의해 개발되었습니다.
