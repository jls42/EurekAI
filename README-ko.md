...
**EurekAI**는 [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([공식 사이트](https://worldwide-hackathon.mistral.ai/))(2026년 3월) 기간에 탄생했습니다. 주제가 하나 필요했는데, 아주 현실적인 아이디어에서 출발했습니다. 저는 딸과 함께 시험공부를 자주 하는데, AI를 활용하면 이를 더 재미있고 인터랙티브하게 만들 수 있겠다고 생각했습니다.

목표는 **어떤 입력이든** — 수업 사진, 복사해 붙여넣은 텍스트, 음성 녹음, 웹 검색 — **복습 노트, 플래시카드, 퀴즈, 팟캐스트, 빈칸 채우기, 일러스트 등**으로 바꾸는 것입니다. 이 모든 것은 Mistral AI의 프랑스 모델로 구동되며, 프랑스어 사용 학생들에게 자연스럽게 잘 맞는 솔루션입니다.

[초기 프로토타입](https://github.com/jls42/worldwide-hackathon.mistral.ai)은 해커톤 동안 Mistral 서비스 중심의 개념 증명으로 48시간 만에 만들어졌습니다. 이미 동작했지만 제한적이었습니다. 그 이후 EurekAI는 진짜 프로젝트가 되었습니다: 빈칸 채우기, 연습문제 탐색, 웹 스크래핑, 구성 가능한 부모 모더레이션, 심층 코드 리뷰 등 훨씬 더 많은 기능이 추가되었습니다. 코드 전체는 AI가 생성하며 — 주로 [Claude Code](https://code.claude.com/)를 사용하고, 일부는 [Codex](https://openai.com/codex/)와 [Gemini CLI](https://geminicli.com/)의 기여를 받았습니다.
...
| | 기능 | 설명 |
|---|---|---|
| 📷 | **파일 가져오기** | 수업 자료를 가져오세요 — 사진, PDF(`high`/`medium`/`low`의 평균 신뢰도 점수를 사용하는 Mistral OCR) 또는 텍스트 파일(TXT, MD). 파일별 재시도와 개별 진행률이 있는 업로드 세션 |
| 📝 | **텍스트 입력** | 어떤 텍스트든 직접 입력하거나 붙여넣기 |
| 🎤 | **음성 입력** | 직접 녹음하세요 — Voxtral STT가 음성을 전사합니다 |
| 🌐 | **웹 / URL** | URL을 붙여넣으세요(Readability + Lightpanda로 직접 스크래핑) 또는 검색어를 입력하세요(Mistral web_search Agent) |
| 📄 | **복습 노트** | 핵심 포인트, 어휘, 인용문, 일화가 포함된 구조화된 노트 |
| 🃏 | **플래시카드** | 인터랙티브한 Q/A 카드, 대화형 음성 읽기 |
| ❓ | **객관식 퀴즈** | 오답의 적응형 복습(구성 가능한 개수)이 포함된 객관식 문제 |
| ✏️ | **빈칸 채우기** | 힌트와 관대한 채점이 있는 완성형 연습문제 |
| 🎙️ | **팟캐스트** | 오디오로 된 2인 대화 미니 팟캐스트 — 기본 Mistral 음성 또는 맞춤 음성(부모님!) |
| 🖼️ | **일러스트** | Mistral Agent가 생성하는 교육용 이미지 |
| 🗣️ | **음성 퀴즈** | 소리 내어 읽는 문제(맞춤 음성 가능), 음성 답변, AI 검증 |
| 💬 | **AI 튜터** | 도구 호출이 가능한, 수업 자료와 문맥을 공유하는 채팅 |
| 🧠 | **자동 라우터** | `mistral-small-latest` 기반 라우터가 콘텐츠를 분석하고 사용 가능한 7개 유형 중 생성기 조합을 제안 |
| 🔒 | **부모 통제** | 프로필별 구성 가능한 모더레이션(맞춤 카테고리), 부모 PIN, 채팅 제한 |
| 🌍 | **다국어** | 인터페이스는 9개 언어로 제공되며, AI 생성은 프롬프트를 통해 15개 언어로 제어 가능 |
| 🔊 | **소리 내어 읽기** | Mistral Voxtral TTS 또는 ElevenLabs로 노트와 플래시카드(질문/답변 대화)를 들을 수 있음 |
| 💶 | **API 비용 추적** | 각 생성과 소스의 비용(€)을 토큰/문자/페이지/오디오 초 단위로 투명하게 추정. 카드별 배지 + 프로젝트별 총합이 대시보드에 표시됨 |
| 🎨 | **프로필별 테마** | 각 프로필은 `dark` 또는 `light` 테마를 선택하며, 프로필 전환 시 유지됨 |
...
EurekAI는 프로필에 따라 관리되는 4가지 소스 유형을 स्वीकार합니다(어린이 및 청소년은 기본 활성화됨):

- **파일 가져오기** — JPG, PNG 또는 PDF 파일은 `mistral-ocr-latest`(인쇄 텍스트, 표, 손글씨)로 처리되며, 텍스트 파일(TXT, MD)은 직접 가져옵니다. 여러 파일 업로드는 **업로드 세션** 시스템을 사용합니다: 파일별 개별 진행률, 다른 파일을 다시 제출하지 않고 실패한 파일만 재시도, 완료 시 세션 닫기. OCR은 평균화된 **신뢰도 점수**(`average`, `[0,1]`로 clamp, Mistral이 반환한 `averagePageConfidenceScore`로 계산)를 노출하며, UI에서 `high` / `medium` / `low` 배지(임계값 ~0.9 / ~0.7)로 표시되어 품질이 낮은 스캔은 차단하지 않고 경고합니다.
- **자유 텍스트** — 어떤 내용이든 입력하거나 붙여넣기. 모더레이션이 활성화되어 있으면 저장 전에 검사됩니다.
- **음성 입력** — 브라우저에서 오디오를 녹음합니다. `voxtral-mini-latest`가 전사합니다. `language="fr"` 파라미터가 인식을 최적화합니다.
- **웹 / URL** — 하나 이상의 URL을 붙여넣어 콘텐츠를 직접 스크래핑하거나(Readability + Lightpanda는 JS 페이지용), 키워드를 입력해 Mistral Agent를 통한 웹 검색을 수행합니다. 단일 필드는 두 가지를 모두 허용하며, URL과 키워드는 자동으로 분리되고 각 결과는 독립적인 소스를 생성합니다.
...
| **플래시카드** | `mistral-large-latest` | 소스 참조가 포함된 Q/A 카드(구성 가능한 개수) |
| **객관식 퀴즈** | `mistral-large-latest` | 객관식 문제, 해설, 적응형 복습(구성 가능한 개수) |
| **빈칸 채우기** | `mistral-large-latest` | 힌트가 있는 완성형 문장, 관대한 검증(Levenshtein) |
...
- **도구 호출**: 대화 중에 복습 노트, 플래시카드, 퀴즈 또는 빈칸 채우기를 생성할 수 있음
- 과목당 50개 메시지 기록
- 프로필에 대해 활성화된 경우 콘텐츠 모더레이션

### 자동 라우터

라우터는 `mistral-small-latest`를 사용해 소스 콘텐츠를 분석하고, 사용 가능한 7개 중 가장 적절한 생성기를 제안합니다. 인터페이스는 실시간 진행률을 표시합니다: 먼저 분석 단계가 있고, 그다음 개별 생성이 진행되며 취소도 가능합니다.

### 적응형 학습

- **퀴즈 통계**: 문제별 시도와 정확도 추적
- **퀴즈 복습**: 약한 개념을 겨냥한 5~10개의 새 문제 생성
- **지시문 감지**: 복습 지시문("Je sais ma leçon si je sais...")을 감지하고 호환되는 텍스트 생성기(노트, 플래시카드, 퀴즈, 빈칸 채우기)에서 우선 적용

### 보안 및 부모 통제

- **4개 연령 그룹**: 어린이(≤10세), 청소년(11-15), 학생(16-25), 성인(26+)
- **콘텐츠 모더레이션**: `mistral-moderation-latest`에 10개 카테고리가 있으며, 어린이/청소년은 기본적으로 5개가 차단됨(`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). 카테고리는 설정에서 프로필별로 맞춤 구성 가능.
- **부모 PIN**: SHA-256 해시, 15세 미만 프로필에 필요. 프로덕션 배포에서는 솔트가 있는 느린 해시(Argon2id, bcrypt)를 사용할 것.
- **채팅 제한**: 16세 미만은 기본적으로 AI 채팅이 비활성화되며, 부모가 활성화할 수 있음

### 다중 프로필 시스템

- 이름, 나이, 아바타, 언어 선호를 가진 여러 프로필
- **프로필별 음성** (`Profile.mistralVoices?: { host, guest }`) — 각 자녀는 팟캐스트/음성 퀴즈용 음성 쌍을 가질 수 있음
- **프로필별 테마** (`Profile.theme: 'dark' | 'light'`) — 프로필 변경 시 자동 전환, 백엔드에 저장됨
- `profileId`를 통해 프로필과 연결된 프로젝트
- 연쇄 삭제: 프로필을 삭제하면 해당 프로필의 모든 프로젝트도 삭제됨

### API 비용 추적

각 Mistral 호출(채팅, OCR, STT, TTS, 모더레이션, 에이전트)은 사용자에게 **투명한** € 추정을 제공하도록 계측되어 — 청구에 대한 놀라움이 없습니다.

- **단일 진실 원천**: `helpers/pricing.ts` — 모델 prefix별 `MODEL_PRICING`(예: `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), 주기적 재스크래핑을 위한 Mistral 문서 URL이 포함된 `PRICING_SOURCES`
...
- **엔드포인트**: `/generate/*` 및 `/sources/*` 응답은 반환된 객체(Generation / Source)에 `estimatedCost`, `usage` 및 `costBreakdown`를 추가합니다. `POST /generate/auto/route`는 라우팅 자체 비용을 위한 `costDelta: number` 필드를 추가합니다. `GET /projects/:pid`는 `totalCost`(계산된 합계는 `costLog[]`에서 산출)와 전체 히스토리가 포함된 프로젝트를 반환합니다

### 다중 제공자 TTS 및 맞춤 음성

- **Mistral Voxtral TTS**(기본) : `voxtral-mini-tts-latest`, 추가 키 불필요
- **ElevenLabs**(대안) : `eleven_v3`, 자연스러운 음성, `ELEVENLABS_API_KEY` 필요
- 제공자는 애플리케이션 설정에서 구성 가능
- **맞춤 음성**: 부모는 Mistral Voices API를 통해(오디오 샘플 기반) 자신만의 음성을 만들고 이를 호스트/게스트 역할에 할당할 수 있으며 — 그러면 팟캐스트와 음성 퀴즈가 부모의 목소리로 읽혀 어린이에게 더 몰입감 있는 경험을 제공합니다
- 두 개의 구성 가능한 음성 역할: **호스트**(주요 내레이터) 및 **게스트**(팟캐스트의 두 번째 목소리)
- Mistral 전체 음성 카탈로그는 설정에서 확인 가능하며, 언어별 필터링 지원
...
| **TTS(기본)** | Mistral Voxtral TTS | `voxtral-mini-tts-latest`, 내장 음성 합성 |
| **TTS(대안)** | ElevenLabs SDK 2.x | `eleven_v3`, 자연스러운 음성 |
| **아이콘** | Lucide 1.x | SVG 아이콘 라이브러리 |
| **웹 스크래핑** | Readability + linkedom | 웹 페이지의 मुख्य 콘텐츠 추출(Firefox Reader View 기술) |
| **헤드리스 브라우저** | Lightpanda | JS/SPA 페이지용 초경량 헤드리스 브라우저(Zig + V8) — 스크래핑 대체 경로 |
| **Markdown** | Marked | 채팅에서 마크다운 렌더링 |
| **파일 업로드** | Multer 2.x | multipart 폼 처리 |
| **오디오** | ffmpeg-static | 오디오 세그먼트 연결 |
| **테스트** | Vitest | 단위 테스트 — SonarCloud로 측정되는 커버리지 |
...
## 모델 참조 | 모델 | 사용 용도 | 이유 |
|---|---|---|
| `mistral-large-latest` | 학습지, 플래시카드, 팟캐스트, 퀴즈, 빈칸 채우기 텍스트, 채팅, 음성 퀴즈 검증, 이미지 에이전트, 웹 검색 에이전트, 지시 감지 | 최고의 다국어 지원 + 지시사항 준수 |
| `mistral-ocr-latest` | 문서 OCR | 인쇄된 텍스트, 표, 손글씨 |
| `voxtral-mini-latest` | 음성 인식(STT) | 다국어 STT, `language="fr"`로 최적화됨 |
| `voxtral-mini-tts-latest` | 음성 합성(TTS) | 팟캐스트, 음성 퀴즈, 음성 읽기 |
| `mistral-moderation-latest` | 콘텐츠 모더레이션 | 아동/청소년용 차단 카테고리 5개(+ 탈옥 방지) |
...
> **참고**: Mistral Voxtral TTS가 기본 제공자입니다 — `MISTRAL_API_KEY` 외에 추가 키는 필요하지 않습니다. ElevenLabs는 설정에서 구성할 수 있는 대체 TTS 제공자입니다.

### 환경 변수

| 변수 | 필수 | 기본값 | 역할 |
|---|---|---|---|
| `MISTRAL_API_KEY` | ✅ | — | Mistral API 키(chat, OCR, STT, Voxtral TTS, agents, moderation) |
| `ELEVENLABS_API_KEY` | ⚠ 선택 사항 | — | ElevenLabs 키; TTS provider = ElevenLabs인 경우에만 필요 |
| `PORT` | 선택 사항 | `3000` | Express 백엔드의 HTTP 포트 |
| `NODE_ENV` | 선택 사항 | `development` | `production`이면 Express가 `dist/`에서 프런트엔드를 제공함(그렇지 않으면 `public/`) |
...
**Git 훅(Husky)**: `pre-commit`는 `npm test`를 실행하고, `pre-push`는 `npm run security`를 실행합니다. 둘 중 하나라도 실패하면 커밋/푸시가 차단됩니다.
...
이 도구들이 없으면 `npm test`는 `pretest`에서 실패하고(lizard 없음), `npm run security`는 실패합니다(opengrep 없음). 그러면 husky 훅이 커밋/푸시를 차단합니다.
...
> **`ELEVENLABS_API_KEY`**는 선택 사항입니다(TTS 대체).
...
> **AI 기여자를 위한 안내**: 자세한 아키텍처 맥락, 필수 규칙(프롬프트 누출 방지, 오류 코드, 비용 추적) 및 알려진 함정(Lizard CCN, Opengrep, Codacy/Semgrep 마이그레이션)은 [`CLAUDE.md`](CLAUDE.md)를 참조하세요.
...
| `PUT` | `/api/config` | 설정 변경(모델, 음성, TTS provider) |
| `GET` | `/api/config/status` | API 상태: `mistral`(Mistral 키 설정됨), `elevenlabs`(ElevenLabs 키 설정됨), `ttsAvailable`(구성된 TTS provider의 키가 있으면 true) |
| `POST` | `/api/config/reset` | 기본 설정으로 재설정 |
| `GET` | `/api/config/voices` | Mistral TTS 음성 목록 표시(선택 사항 `?lang=fr`) |
...
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `GET` | `/api/projects` | 프로젝트 목록 표시(`?profileId=` 선택 사항) |
...
| `POST` | `/api/projects/:pid/detect-consigne` | 복습 지시사항 감지 |

### 생성
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `POST` | `/api/projects/:pid/generate/summary` | 학습지 |
...
| `POST` | `/api/projects/:pid/generate/auto` | 백엔드 자동 생성(라우팅 + 7개 유형: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). 병렬 실행 — 초당 요청 제한이 7개 동시 요청 이상인 Mistral 등급을 가정함; 그렇지 않으면 여러 429가 `failedSteps`에 반환될 수 있음. |

모든 생성 라우트는 `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}`을 허용합니다. `quiz-review`는 추가로 `{generationId, weakQuestions}`가 필요합니다.
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS 음성 읽기(학습지/플래시카드) |
...
| 결정 | 근거 |
|---|---|
| **React/Vue 대신 Alpine.js** | 최소한의 용량, Vite가 컴파일한 TypeScript와 함께 가벼운 반응성. 속도가 중요한 해커톤에 적합합니다. |
| **JSON 파일 기반 영속성** | 의존성 없음, 즉시 시작. 구성해야 할 데이터베이스가 없습니다 — 바로 실행하면 됩니다. |
| **Vite + Handlebars** | 두 세계의 장점: 개발용 빠른 HMR, 코드 구성을 위한 HTML partials, Tailwind JIT. |
| **중앙 집중식 프롬프트** | 모든 AI 프롬프트가 `prompts.ts`에 있어 언어/연령 그룹별로 쉽게 반복, 테스트, 조정 가능. |
| **다중 생성 시스템** | 각 생성은 자체 ID를 가진 독립 객체 — 강의당 여러 학습지, 퀴즈 등을 가능하게 함. |
| **연령별 맞춤 프롬프트** | 어휘, 복잡성, 어조가 다른 4개 연령 그룹 — 같은 내용도 학습자에 따라 다르게 가르칩니다. |
| **에이전트 기반 기능** | 이미지 생성과 웹 검색은 임시 Mistral 에이전트를 사용 — 자동 정리로 깔끔한 생명 주기. |
| **지능형 URL 스크래핑** | 단일 입력 필드가 URL과 키워드 혼합을 허용 — URL은 Readability(정적 페이지)로 스크랩되고 Lightpanda(동적/SPA 페이지)로 폴백하며, 키워드는 Mistral web_search 에이전트를 트리거합니다. 각 결과는 독립적인 소스를 생성합니다. |
...
Mistral AI Worldwide Hackathon(2026년 3월)에서 시작되었으며, [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) 및 [Gemini CLI](https://geminicli.com/)를 사용해 AI에 의해 전적으로 개발되었습니다.
