...
<p align="center">
  <strong>모든 콘텐츠를 AI 기반의 인터랙티브 학습 경험으로 변환합니다.</strong>
</p>
...
**EurekAI**은 [Mistral AI Worldwide 해커톤](https://worldwide-hackathon.mistral.ai/) (2026년 3월) 동안 탄생했습니다. 주제가 필요했는데 — 아이디어는 아주 현실적인 상황에서 나왔습니다: 저는 딸과 함께 정기적으로 시험 준비를 하며, AI를 통해 이것을 더 재미있고 인터랙티브하게 만들 수 있지 않을까 생각했습니다.

목표: **어떤 입력이든** — 교재 사진, 복사·붙여넣은 텍스트, 음성 녹음, 웹 검색 — 를 가져와서 **요약 노트, 플래시카드, 퀴즈, 팟캐스트, 빈칸 채우기 문제, 일러스트레이션 등**으로 변환하는 것입니다. 모든 것은 Mistral AI의 모델로 구동되어 불어권 학생들에게 자연스럽게 적합한 솔루션이 됩니다.

모든 코드 라인은 해커톤 기간 중에 작성되었습니다. 모든 API와 오픈소스 라이브러리는 해커톤 규정에 맞게 사용되었습니다.
...
| 📝 | **텍스트 입력** | 원하는 텍스트를 입력하거나 붙여넣으세요 |
| 🎤 | **음성 입력** | 브라우저에서 녹음하세요 — Voxtral STT가 음성을 전사합니다 |
| 🌐 | **웹 검색** | 질문을 입력하세요 — Mistral 에이전트가 웹에서 답을 찾습니다 |
| 📄 | **요약 노트** | 핵심 포인트, 용어, 인용구, 일화 등이 포함된 구조화된 노트 |
| 🃏 | **플래시카드** | 출처가 참조된 5-50장의 질문/답 카드로 능동적 암기 지원 |
| ❓ | **객관식 퀴즈** | 5-50문제의 선택형 문제와 오답에 대한 적응형 복습 |
| ✏️ | **빈칸 채우기 텍스트** | 힌트와 관용적 검증을 갖춘 채우기 문제 |
| 🎙️ | **팟캐스트** | 2인 구성의 미니 팟캐스트를 Mistral Voxtral TTS로 오디오 변환 |
| 🖼️ | **일러스트레이션** | Mistral 에이전트가 생성한 교육용 이미지 |
| 🗣️ | **음성 퀴즈** | 질문을 음성으로 읽어주고, 구두 응답을 AI가 확인 |
| 💬 | **AI 튜터** | 수업 자료와 맥락을 아는 대화형 챗, 도구 호출 가능 |
| 🧠 | **스마트 라우터** | AI가 콘텐츠를 분석해 7가지 생성기 중 가장 적합한 것을 추천 |
| 🔒 | **부모 통제** | 연령별 검열, 부모용 PIN, 채팅 제한 |
| 🌍 | **다국어** | 인터페이스 및 AI 콘텐츠가 프랑스어와 영어로 제공 |
| 🔊 | **음성 낭독** | Mistral Voxtral TTS 또는 ElevenLabs로 노트와 플래시카드 읽기 |
...
EurekAI는 프로필에 따라 중재되는 4가지 소스 타입을 받습니다(어린이 및 청소년에 대해 기본 활성화):

- **OCR 업로드** — JPG, PNG 또는 PDF 파일을 `mistral-ocr-latest`로 처리합니다. 인쇄된 텍스트, 표 및 필기체를 처리합니다.
- **자유 텍스트** — 원하는 내용을 입력하거나 붙여넣으세요. 저장 전에 중재가 활성화되어 있으면 검열됩니다.
- **음성 입력** — 브라우저에서 오디오를 녹음합니다. `voxtral-mini-latest`이 전사합니다. `language="fr"` 설정은 인식 성능을 최적화합니다.
- **웹 검색** — 쿼리를 입력하세요. 임시 Mistral 에이전트가 `web_search` 도구로 결과를 수집하고 요약합니다.

### AI 콘텐츠 생성

생성되는 학습 자료는 일곱 가지 유형입니다:

| 생성기 | 모델 | 출력 |
|---|---|---|
| **요약 노트** | `mistral-large-latest` | 제목, 요약, 10-25개의 핵심 포인트, 용어, 인용구, 일화 |
| **플래시카드** | `mistral-large-latest` | 출처가 참조된 5-50장의 질문/답 카드로 능동적 암기 지원 |
| **객관식 퀴즈** | `mistral-large-latest` | 5-50문제, 각 문제당 4개 선택지, 해설, 적응형 복습 |
| **빈칸 채우기 텍스트** | `mistral-large-latest` | 힌트가 있는 완성 문장, 관용적 검증(Levenshtein) |
| **팟캐스트** | `mistral-large-latest` + Voxtral TTS | 2인 스크립트 → MP3 오디오 |
| **일러스트레이션** | 에이전트 `mistral-large-latest` | 도구 `image_generation`을 사용한 교육용 이미지 |
| **음성 퀴즈** | `mistral-large-latest` + Voxtral TTS + STT | TTS로 질문을 읽고 → STT로 응답 → AI가 검증 |
...
라우터는 `mistral-small-latest`을 사용해 소스의 내용을 분석하고 7가지 생성기 중 어떤 것이 가장 적합한지 추천합니다 — 학생이 수동으로 선택할 필요가 없도록. 인터페이스는 실시간 진행 상태를 표시합니다: 먼저 분석 단계, 그다음 개별 생성이 진행되며 취소 가능.

### 적응 학습

- **퀴즈 통계**: 시도 횟수 및 질문별 정확도 추적
- **퀴즈 복습**: 약한 개념을 겨냥한 5-10개의 새 질문 생성
- **지시 검출**: 복습 지시("...을 알면 내 수업을 안다" 같은)를 감지하여 모든 생성기에서 우선순위를 줌

### 보안 및 부모 통제

- **4개 연령 그룹**: 어린이 (≤10세), 청소년 (11-15), 학생 (16-25), 성인 (26+)
- **콘텐츠 중재**: `mistral-moderation-2603` 사용, 어린이/청소년에 대해 5개 카테고리 차단(sexual, hate, violence, selfharm, jailbreaking), 학생/성인에는 제한 없음
- **부모 PIN**: SHA-256 해시, 15세 미만 프로필에 필요
- **채팅 제한**: 16세 미만은 기본적으로 AI 채팅이 비활성화되며 부모가 활성화 가능

### 다중 프로필 시스템

- 이름, 나이, 아바타, 언어 선호도를 가진 다중 프로필
- 프로필에 연결된 프로젝트는 `profileId` 통해 관리
- 연쇄 삭제: 프로필 삭제 시 모든 프로젝트가 함께 삭제됨
...
- 인터페이스는 프랑스어와 영어로 전체 제공
- AI 프롬프트는 현재 2개 언어(FR, EN)를 지원하며 아키텍처는 15개 언어(es, de, it, pt, nl, ja, zh, ko, ar, hi, pl, ro, sv) 대응 준비
- 프로필별로 언어 설정 가능
...
| **AI** | Mistral AI SDK 2.1 | 채팅, OCR, STT, TTS, 에이전트, 중재 |
...
| 모델 | 사용처 | 이유 |
|---|---|---|
| `mistral-large-latest` | 노트, 플래시카드, 팟캐스트, 퀴즈, 빈칸 텍스트, 채팅, 음성 퀴즈 검증, 이미지 에이전트, 웹 검색 에이전트, 지시 검출 | 다국어 성능 우수 + 지시 따르기 능력 |
| `mistral-ocr-latest` | 문서 OCR | 인쇄 텍스트, 표, 필기체 |
| `voxtral-mini-latest` | 음성 인식 (STT) | 다국어 STT, `language="fr"`로 최적화 |
| `voxtral-mini-tts-latest` | 음성 합성 (TTS) | 팟캐스트, 음성 퀴즈, 낭독 |
| `mistral-moderation-2603` | 콘텐츠 중재 | 어린이/청소년용 5개 카테고리 차단 (+ jailbreaking) |
| `mistral-small-latest` | 스마트 라우터 | 라우팅 결정을 위한 빠른 콘텐츠 분석 |
| `eleven_v3` (ElevenLabs) | 음성 합성 (대안 TTS) | 자연스러운 음성, 구성 가능한 대안 |
...
> **참고** : Mistral Voxtral TTS가 기본 제공자입니다 — `MISTRAL_API_KEY` 외에 추가 키가 필요하지 않습니다. ElevenLabs는 설정에서 구성 가능한 대체 TTS 제공자입니다.
...
| `POST` | `/api/projects/:pid/moderate` | 중재 `{text}` |
...
| `POST` | `/api/projects/:pid/generate/auto` | 자동 백엔드 생성 (라우팅 + 5종: summary, flashcards, quiz, fill-blank, podcast) |
...
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | 빈칸 응답 제출 `{answers}` |
| `POST` | `/api/projects/:pid/generations/:gid/vocal-answer` | 구두 응답 검증 (오디오 + questionIndex) |
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS 낭독 (노트/플래시카드) |
...
| 결정 | 근거 |
|---|---|
| **Alpine.js를 React/Vue 대신 선택** | 작은 풋프린트, Vite로 컴파일된 TypeScript와 함께 경량 반응성 제공. 해커톤처럼 속도가 중요한 상황에 적합. |
| **JSON 파일로 영속성 관리** | 의존성 제로, 즉시 시작 가능. 데이터베이스 설정 불필요 — 바로 시작할 수 있음. |
| **Vite + Handlebars** | 개발을 위한 빠른 HMR과 코드 구성용 HTML partials의 장점을 결합, Tailwind JIT와 호환. |
| **프롬프트 중앙화** | 모든 AI 프롬프트를 `prompts.ts`에 모아두어 언어/연령대별로 쉽게 반복, 테스트 및 조정 가능. | **다중 세대 시스템** | 각 세대는 고유 ID를 가진 독립 객체 — 코스당 여러 학습지, 퀴즈 등을 허용합니다. |
| **연령별 맞춤 프롬프트** | 어휘, 복잡도 및 톤이 다른 4개의 연령 그룹 — 같은 콘텐츠가 학습자에 따라 다르게 전달됩니다. |
| **에이전트 기반 기능** | 이미지 생성 및 웹 검색은 임시 Mistral 에이전트를 사용 — 고유 수명 주기와 자동 정리 기능을 갖춥니다. |
...
- **[Mistral AI](https://mistral.ai)** — AI 모델( Large, OCR, Voxtral STT, Voxtral TTS, Moderation, Small ) + 전세계 해커톤
...
2026년 3월 Mistral AI Worldwide Hackathon에서 정성스럽게 제작되었습니다.
...
**이 문서는 gpt-5-mini 모델을 사용하여 프랑스어(fr) 버전에서 한국어(ko)로 번역되었습니다. 번역 과정에 대한 자세한 정보는 https://gitlab.com/jls42/ai-powered-markdown-translator 을 참조하세요.**
