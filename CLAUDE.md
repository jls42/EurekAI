...
- **Retraits datés OCR 3** : `mistral-ocr-2512` est **déprécié 2026-06-30** et **retiré 2026-09-30** (source autoritaire https://docs.mistral.ai/models/overview — colonnes Deprecation/Retirement/Alternative ; alternative = OCR 4). Le défaut est volontairement **OCR 4** pour ne PAS utiliser un modèle en retrait par défaut. Détails datés dans le JSDoc `DEFAULT_OCR_MODEL` (**source autoritaire — ne pas dupliquer la date ailleurs**) ; rappel release-time dans `scripts/check-deps.sh` (table `known_deprecations`, format `model|dep|retirement|alt`). Pas de tripwire auto.
