...
- **Race promote/cancel** : `store.promoteToGeneration` retourne `PromoteResult = { kind: 'promoted', generation } | { kind: 'cancelled' } | { kind: 'failed', code } | { kind: 'missing' }`. Le handler dispatche : `200 generation` si promoted, `409 PromoteErrorResponse` (`types.ts`, `{error: 'cancelled' | 'failed' | 'missing', gid}`) sinon. Le type HTTP est découplé du discriminant store pour qu'un futur rename de `kind` ne casse pas le client. `kind: 'missing'` est loggué `logger.error` (Sentry) — c'est un bug observabilité, pas un cancel user-initié. Plus de réponse 200 fantôme quand un cancel arrive pendant que Mistral travaille.
- **Boot sweep** : `store.cancelAllPendingsAtBoot()` au démarrage marque tous les `pendingTracker[].status === 'pending'` comme `cancelled`. Pas de TTL — par construction, tout pending sur disque vient d'un process mort. Si `getProject(meta.id)` retourne null (project.json corrompu), `console.warn` dédié pour signaler les pendings ghost laissés en place. Le tracker est aussi pruné à chaque save au-delà de 50 entrées : `pruneTrackerIfNeeded` garde max 50 entrées AND ≤ 7 jours d'âge (intersection — les terminaux gardés satisfont à la fois la fenêtre de fraîcheur ET le quota ; les pendings actifs ne sont jamais prunés).
- **`pendingTracker` séparé de `generations[]`** dans `ProjectData.results` (`types.ts`). Évite de polluer `generations: Generation[]` avec des entrées sans `data`. Promotion = remove du tracker + push dans generations atomique dans le même `saveProject`. Les terminaux ne suivent pas ce chemin : `markPendingFailed`/`markPendingCancelled` remplacent l'entrée in-place par l'arm terminale et la gardent jusqu'au pruning.
- **`PendingTrackerEntry` est une discriminated union sur `status`** (`types.ts`) : `PendingTrackerEntryActive` (status: 'pending') | `PendingTrackerEntryTerminal` (status: 'failed' | 'cancelled', `failureCode` + `completedAt` OBLIGATOIRES). Conséquence : `terminatePending`/`cancelAllPendingsAtBoot` ne peuvent PAS muter `entry.status` in-place — ils construisent un nouvel objet terminal et remplacent par index dans le tracker. Ferme à compile-time l'état impossible `{status: 'pending', failureCode: 'cancelled'}`.
