...
- **Dépréciation OCR 3 (choix temporaire assumé)** : `mistral-ocr-2512` est annoncé déprécié au 2026-06-30 (docs Mistral, remplacé par OCR 4) — défaut conservé **volontairement** pour le coût, pas un oubli. Rationale complète + date de revue dans le JSDoc de `DEFAULT_OCR_MODEL` (`helpers/ocr-models.ts`, **source autoritaire** — ne pas dupliquer la date ailleurs). Rappel daté au release-time dans `scripts/check-deps.sh` (table `known_deprecations`). Pas de tripwire auto : `check-deps` ne fait qu'un smoke-test d'existence et `/v1/models` renvoie `deprecation: None` (lag docs).
