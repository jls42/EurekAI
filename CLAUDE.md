...
- **`PendingTrackerEntry` est une discriminated union sur `status`** (`types.ts`) : `PendingTrackerEntryActive` (status: 'pending') | `PendingTrackerEntryTerminal` (status: 'failed' | 'cancelled', `failureCode` + `completedAt` OBLIGATOIRES). Conséquence : `terminatePending`/`cancelAllPendingsAtBoot` ne peuvent PAS muter `entry.status` in-place — ils construisent un nouvel objet terminal et remplacent par index dans le tracker. Ferme à compile-time l'état impossible `{status: 'pending', failureCode: 'cancelled'}` qui était type-valide pré-fix #16.
