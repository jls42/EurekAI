...
### OCR model selection (OCR 4 défaut / OCR 3 opt-in)
- **Source unique** : `helpers/ocr-models.ts` exporte `OCR_MODELS` (`['mistral-ocr-4-0', 'mistral-ocr-2512']`), le type `OcrModel`, `OCR_MODEL_LABELS` (noms produit UI : "OCR 4"/"OCR 3"), `DEFAULT_OCR_MODEL = 'mistral-ocr-4-0'` (OCR 4, $4/1000, **modèle courant**) et `normalizeOcrModel(v: unknown): OcrModel`. OCR 3 (`mistral-ocr-2512`, $2/1000) reste opt-in (moins cher). L'API ne change pas (`client.ocr.process` + `confidenceScoresGranularity: 'page'` compatible OCR 3 et 4).
- **Retraits datés OCR 3** : `mistral-ocr-2512` est **déprécié 2026-06-30** et **retiré 2026-09-30** (source amont https://docs.mistral.ai/models/overview — colonnes Deprecation/Retirement/Alternative ; alternative = OCR 4). Le défaut est volontairement **OCR 4** pour ne PAS utiliser un modèle en retrait par défaut. **Source unique de la date dans le code** : la const `OCR_MODEL_RETIREMENT` (`helpers/ocr-models.ts`) — **ne pas dupliquer la date ailleurs**. Elle est **affichée à l'utilisateur** dans le sélecteur (suffixe « retiré le 2026-09-30 » sur l'option OCR 3, via `modelOptionLabel` + i18n `settings.ocrRetiredOn`) pour qu'il sache que le modèle est en fin de vie. Rappel release-time dans `scripts/check-deps.sh` (table `known_deprecations`, format `model|dep|retirement|alt`, copie bash assumée). Pas de tripwire auto.
- **Câblage** : `config.models.ocr` propagé — `routes/sources.ts` passe `normalizeOcrModel(getConfig().models.ocr)` à `ocrFile(client, path, name, model: OcrModel)`. Sélecteur dans `dialog-settings.html` : 2 `<option>` montrant le **nom produit** (`modelOptionLabel` → "OCR 4"/"OCR 3" via `OCR_MODEL_LABELS`) + l'**ID technique réel en italique discret** sous le `<select>` (`x-text="configDraft._ocrModel"`).
- **Anti-piège legacy / alias mouvant** : `mergeSafe` préserve la valeur disque → un `config.json` avec l'alias `mistral-ocr-latest` suivrait silencieusement les bascules Mistral. Parade à 3 niveaux : migration one-time `normalizeLegacyOcrModel` dans `initConfig` (fige l'alias → défaut explicite OCR 4 + persiste via `persistConfig`), rejet symétrique dans `saveConfig`, et `normalizeOcrModel` défensif au call site. `normalizeOcrModel(v: unknown)` : signature `unknown` + guard `typeof` OBLIGATOIRE sous `strict` (sinon `.includes(string | undefined)` casse le typecheck).
- **Vigilance retraits modèles (leçon 2026-06)** : un alias `-latest` peut pointer sur une version **en retrait** sans repointage Mistral. Vérifié : `mistral-moderation-latest` → `mistral-moderation-2411` (retiré 2026-06-30) alors que `mistral-moderation-2603` ("Moderation 2") est le courant → `generators/moderation.ts` est pinné sur `mistral-moderation-2603`. Avant release, croiser les modèles utilisés avec https://docs.mistral.ai/models/overview. Le coût étant calculé sur `response.model` **daté**, `MODEL_PRICING` doit suivre la version réelle résolue (ex. Moderation 2 facturée $0.1/M tokens input, **pas** gratuite).
