...
| 🌐 | **Web / URL** | Paste a URL (direct scraping via Readability + Lightpanda) or type a search (Agent Mistral web_search) |
...
| ❓ | **Multiple-choice quizzes** | Multiple-choice questions with adaptive error review (configurable number) |
...
| 🗣️ | **Spoken quiz** | Questions read aloud (custom voice possible), oral answer, AI verification |
| 💬 | **AI tutor** | Contextual chat with your course documents, with tool calls |
| 🧠 | **Automatic router** | A router based on `mistral-small-latest` analyzes the content and proposes a combination of generators among the 7 available |
| 🔒 | **Parental control** | Configurable moderation per profile (customizable categories), parental PIN, chat restrictions |
| 🌍 | **Multilingual** | Interface available in 9 languages; AI generation controllable in 15 languages via prompts |
| 🔊 | **Text-to-speech** | Listen to notes and flashcards (question/answer dialogue) via Mistral Voxtral TTS or ElevenLabs |
...
- **Web / URL** — Paste one or more URLs to scrape content directly (Readability + Lightpanda for JS pages), or type keywords for a web search via Agent Mistral. The single field accepts both — URLs and keywords are automatically separated, each result creates an independent source.
...
The router uses `mistral-small-latest` to analyze source content and propose the most relevant generators among the 7 available. The interface displays real-time progress: first an analysis phase, then individual generations with possible cancellation.
...
- Instruction detection: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (note, flashcards, quiz, fill-in-the-blank texts)
...
| **Web scraping** | Readability + linkedom | Extraction of main content from web pages (Firefox Reader View tech) |
| **Headless browser** | Lightpanda | Ultra-light headless browser (Zig + V8) for JS/SPA pages — fallback scraping |
| **Markdown** | Marked | Markdown rendering in the chat |
...
| **Persistence** | JSON files | Storage without dependency |
...
| Model | Usage | Why |
|---|---|---|
| `mistral-large-latest` | Note, Flashcards, Podcast, Quiz, Fill-in-the-blank texts, Chat, Spoken quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `voxtral-mini-tts-latest` | Text-to-speech (TTS) | Podcasts, spoken quiz, text-to-speech playback |
...
| `DELETE` | `/api/projects/:pid` | Delete project | ### Sources
| Method | Endpoint | Description |
|---|---|---|
...
| **Persistence in JSON files** | Zero dependencies, instant startup. No database to configure — it's ready to go. |
...
Initiated during the Mistral AI Worldwide Hackathon (March 2026), fully developed by AI with [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
...
**This document was translated from the fr version into the en language by using the gpt-5-mini model. For more information on the translation process, see https://gitlab.com/jls42/ai-powered-markdown-translator**
