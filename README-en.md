...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI"><img src="https://img.shields.io/badge/▶️_See_the_demo-YouTube-red?style=for-the-badge&logo=youtube" alt="YouTube Demo"></a>
</p>
...
**EurekAI** was born during the [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([official site](https://worldwide-hackathon.mistral.ai/)) (March 2026). I needed a topic — and the idea came from something very concrete: I regularly prepare tests with my daughter, and I thought it had to be possible to make that more fun and interactive using AI.

The goal: take **any input** — a photo of the lesson, copied-and-pasted text, a voice recording, a web search — and turn it into **revision sheets, flashcards, quizzes, podcasts, fill-in-the-blanks, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited to French-speaking students.
...
| | Feature | Description |
|---|---|---|
| 📷 | **File import** | Import your lessons — photo, PDF (via Mistral OCR with confidence score per page, tier `high`/`medium`/`low`) or text file (TXT, MD). Upload sessions with per-file retry and individual progress |
...
The router uses `mistral-small-latest` to analyze source content and suggest the most relevant generators among the 7 available. The interface shows progress in real time: first an analysis phase, then individual generations with cancellation possible.
...
- **Endpoints**: `/generate/*` and `/sources/*` responses include `costDelta: { estimatedEuros, perModel }`. `GET /projects/:pid` returns `totalCost` + `costLog[]` history. `GET /api/config/status` exposes `costEstimateAvailable`
...
|---|---|---|
| `mistral-large-latest` | Study sheet, Flashcards, Podcast, Quiz, Fill-in-the-blank texts, Chat, Oral quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `PUT` | `/api/config` | Update config (models, voices, TTS provider) |
| `GET` | `/api/config/status` | API status: `mistralAvailable`, `elevenlabsAvailable`, `ttsAvailable` (active provider), `costEstimateAvailable` |
...
| `DELETE` | `/api/profiles/:id` | Delete profile + cascade projects `{pin?}` → `{ok, deletedProjects}` |
...
| `POST` | `/api/projects/:pid/generate/auto` | Backend auto-generation (routing + 7 types: summary, flashcards, quiz, fill-blank, podcast, voice-quiz, image). Runs in parallel — assumes a Mistral tier with rate limit ≥ 7 simultaneous requests; otherwise multiple 429s may be returned in `failedSteps`. |
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | Read TTS aloud (study sheets/flashcards) |
...
| **Persistence in JSON files** | Zero dependency, instant startup. No database to configure — you start it up and you're ready to go. |
