...
| 🃏 | **Flashcards** | Interactive Q/A cards, narrated audio playback |
...
| 🔊 | **Read aloud** | Listen to sheets and flashcards (Q/A dialogue) via Mistral Voxtral TTS or ElevenLabs |
...
| 🎨 | **Theme per profile** | Each profile chooses its `dark` or `light` theme — persists when switching profiles |
...
- **File import** — JPG, PNG, or PDF files processed by `mistral-ocr-latest` (printed text, tables, handwritten writing), or text files (TXT, MD) imported directly. Multi-file uploads use an **upload sessions** system: individual progress per file, retry of the failed file without resubmitting the others, dismiss the session when finished. OCR exposes an averaged **confidence score** (`average`, clamped in `[0,1]`, calculated from `averagePageConfidenceScore` returned by Mistral), displayed in the UI as a tier `high` / `medium` / `low` badge (thresholds ~0.9 / ~0.7) — warns without blocking if the scan is low quality.
- **Free text** — Type or paste any content. Moderated before storage if moderation is active.
- **Voice input** — Record audio in the browser. Transcribed by `voxtral-mini-latest`. The `language="fr"` parameter improves recognition.
- **Web / URL** — Paste one or more URLs to scrape the content directly (Readability + Lightpanda for JS pages), or type keywords for a web search via Mistral Agent. The single field accepts both — URLs and keywords are separated automatically, each result creates an independent source.
...
The router uses `mistral-small-latest` to analyze source content and suggest the most relevant generators among the 7 available. The interface shows progress in real time: first an analysis phase, then individual generations with possible cancellation.

### Adaptive learning

- **Quiz stats**: tracking of attempts and accuracy per question
...
- **Voice per profile** (`Profile.mistralVoices?: { host, guest }`) — each child can have their own podcast/voice quiz voice pair
- **Theme per profile** (`Profile.theme: 'dark' | 'light'`) — automatic switch when changing profile, persisted on the backend
...
- **UI**: cost badge per generation (`src/partials/cost-badge-gen.html`), per source (`cost-badge-src.html`), cumulative total in dashboard (`Project.totalCost`)
...
|---|---|---|
| `mistral-large-latest` | Study sheet, Flashcards, Podcast, Quiz, Fill-in-the-blanks, Chat, Voice quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
**Git hooks (Husky)**: `pre-commit` runs `npm test`, `pre-push` runs `npm run security`. Both block commit/push if they fail.
...
| `GET` | `/api/config/status` | API status: `mistral` (Mistral key set), `elevenlabs` (ElevenLabs key set), `ttsAvailable` (true if the configured TTS provider key is present) |
...
| `POST` | `/api/projects/:pid/generate/auto` | Backend auto-generation (routing + 7 types: summary, flashcards, quiz, fill-blank, podcast, voice quiz, image). Parallel execution — assumes a Mistral tier with rate limit ≥ 7 simultaneous requests; otherwise multiple 429s may surface in `failedSteps`. |
...
| **Persistence in JSON files** | Zero dependencies, instant startup. No database to configure — just launch and go. |
...
| **Multi-generation system** | Each generation is an independent object with its own ID — enables multiple sheets, quizzes, etc. per course. |
