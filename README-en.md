...
**EurekAI** was born during the [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([official site](https://worldwide-hackathon.mistral.ai/)) (March 2026). I needed a topic — and the idea came from something very concrete: I regularly prepare tests with my daughter, and I thought it should be possible to make that more playful and interactive thanks to AI.

The goal: take **any input** — a photo of the lesson, copied text, a voice recording, a web search — and turn it into **revision notes, flashcards, quizzes, podcasts, fill-in-the-blank texts, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited for French-speaking students.

The [initial prototype](https://github.com/jls42/worldwide-hackathon.mistral.ai) was built in 48 hours during the hackathon as a proof of concept around Mistral services — functional but limited. Since then, EurekAI has become a full project: fill-in-the-blank texts, exercise navigation, web scraping, configurable parental moderation, in-depth code review, and much more. The entire codebase is generated by AI — mainly [Claude Code](https://code.claude.com/), with some contributions via [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
...
| 🃏 | **Flashcards** | Interactive Q/A cards, dialogued audio playback |
| ❓ | **Multiple-choice quiz (MCQ)** | Multiple-choice questions with adaptive review of mistakes (configurable number) |
| ✏️ | **Fill-in-the-blank texts** | Exercises to complete with hints and tolerant validation |
| 🎙️ | **Podcast** | Mini 2-voice podcast audio — Mistral voices by default or custom voices (parents!) |
| 🖼️ | **Illustrations** | Educational images generated by a Mistral Agent |
| 🗣️ | **Vocal quiz** | Questions read aloud (custom voices possible), oral answer, AI verification |
...
| 🔊 | **Read aloud** | Listen to notes and flashcards (question/answer dialogue) via Mistral Voxtral TTS or ElevenLabs |
...
EurekAI accepts 4 source types, moderated per profile (enabled by default for child and teen):

- **File import** — JPG, PNG or PDF files processed by `mistral-ocr-latest` (printed text, tables, handwritten text), or text files (TXT, MD) imported directly.
...
- **Web / URL** — Paste one or more URLs to scrape content directly (Readability + Lightpanda for JS pages), or type keywords for a web search via the Mistral Agent. The single field accepts both — URLs and keywords are automatically separated, each result creates an independent source.
...
|---|---|---|
| **Revision note** | `mistral-large-latest` | Title, summary, key points, vocabulary, quotes, anecdote |
...
| **Vocal quiz** | `mistral-large-latest` + Voxtral TTS + STT | TTS questions → STT answer → AI verification |
...
- **Quiz revision**: generates 5-10 new questions targeting weak concepts
- **Instruction detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (notes, flashcards, quiz, fill-in-the-blank texts)

### Security & parental control

- **4 age groups**: child (≤10 years), teen (11-15), student (16-25), adult (26+)
- **Content moderation**: `mistral-moderation-latest` with 10 categories available, 5 blocked by default for child/teen (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Categories customizable per profile in settings.
...
- **Custom voices**: parents can create their own voices via the Mistral Voices API (from an audio sample) and assign them to host/guest roles — podcasts and vocal quizzes are then played with a parent's voice, making the experience more immersive for the child
...
| Model | Usage | Why |
|---|---|---|
| `mistral-large-latest` | Note, Flashcards, Podcast, Quiz, Fill-in-the-blank texts, Chat, Vocal quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction-following |
| `mistral-ocr-latest` | Document OCR | Printed text, tables, handwritten text |
| `voxtral-mini-latest` | Speech recognition (STT) | Multilingual STT, optimized with `language="fr"` |
| `voxtral-mini-tts-latest` | Speech synthesis (TTS) | Podcasts, vocal quiz, read aloud |
...
| `DELETE` | `/api/profiles/:id` | Delete a profile + cascade projects `{pin?}` → `{ok, deletedProjects}` ### Projects
...
| `POST` | `/api/projects/:pid/generate/quiz` | Multiple-choice quiz (MCQ) |
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS read aloud (revision sheets/flashcards) |
...
| **Smart URL scraping** | A single field accepts mixed URLs and keywords — URLs are scraped via Readability (static pages) with a Lightpanda fallback (JS/SPA pages), keywords trigger a Mistral Agent web_search. Each result creates an independent source. |
...
Initiated during the Mistral AI Worldwide Hackathon (March 2026), developed entirely by AI with [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
...
**This document was translated from the fr version into the en en language using the gpt-5-mini model. For more information on the translation process, see https://gitlab.com/jls42/ai-powered-markdown-translator**
