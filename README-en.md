...
| 🃏 | **Flashcards** | Interactive Q&A cards, dialog-style audio playback |
...
| 🔤 | **Dictation** | Words dictated in audio (Voxtral TTS) from an imported list, keyboard input, strict letter-by-letter correction with an explained spelling rule |
...
| 🔒 | **Parental Controls** | Configurable moderation by profile (customizable categories), parental PIN, chat restrictions |
| 🌍 | **Multilingual** | Interface available in 9 languages; AI generation controllable in 15 languages via prompts |
| 🔊 | **Read Aloud** | Listen to sheets and flashcards (question/answer dialog) via Mistral Voxtral TTS |
...
The router uses `mistral-small-latest` to analyze the content of the sources and suggest the most relevant generators among the 8 available. The interface shows progress in real time: first an analysis phase, then individual generations with cancellation possible.
...
- **Prompt detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (sheet, flashcards, quiz, fill-in-the-blank texts)

### Security & Parental Controls

- **4 age groups**: child (≤10), teen (11-15), student (16-25), adult (26+)
- **Content moderation**: `mistral-moderation-2603` (Mistral Moderation 2) with 10 available categories, 5 blocked by default for child/teen (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Categories are customizable per profile in settings. The `-latest` alias is intentionally avoided (it still points to a deprecated version).
...
- **Per-profile voices** (`Profile.mistralVoices?: { host, guest }`) — each child can have their own pair of podcast/voice-quiz voices
...
Every Mistral call (chat, OCR, STT, TTS, moderation, agents) is instrumented to provide a **transparent** € estimate to the user — no surprises in billing.
...
- **Endpoints**: the `/generate/*` and `/sources/*` responses decorate the returned object (Generation / Source) with `estimatedCost`, `usage` and `costBreakdown`. `POST /generate/route` adds a `costDelta: number` field for routing cost alone. `GET /projects/:pid` returns the enriched project with `totalCost` (sum calculated from `costLog[]`) + the full history
...
| Model | Use | Why |
|---|---|---|
| `mistral-large-latest` | Sheet, Flashcards, Podcast, Quiz, Fill-in-the-blank texts, Chat, Voice quiz verification, Image Agent, Web Search Agent, Prompt detection | Best multilingual support + instruction following |
...
| `mistral-moderation-2603` | Content moderation | 5 blocked categories for child/teen (including `jailbreaking`) |
...
|---|---|---|---|
| `MISTRAL_API_KEY` | optional | — | Mistral API key (chat, OCR, STT, Voxtral TTS, agents, moderation). If absent, the user enters their key in the app (stored in browser, never on server) |
...
| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/projects/:pid/sources/upload` | Import multipart files (OCR for JPG/PNG/PDF, direct read for TXT/MD) |
...
| `POST` | `/api/projects/:pid/generate/remediation-summary` | Targeted recap sheet for the questions missed in a quiz `{generationId, weakQuestions}` — called in parallel with `quiz-review` by the “Train me on my mistakes” button |
| `POST` | `/api/projects/:pid/generate/route` | Routing analysis (plan of generators to run) — returns `{plan, costDelta}` (routing cost only) |
| `POST` | `/api/projects/:pid/generate/auto` | Backend auto-generation (routing + 8 types: summary, flashcards, quiz, fill-blank, podcast, voice-quiz, image, dictation). Runs in parallel — assumes a Mistral tier with rate limit ≥ 8 simultaneous requests; otherwise several 429s may bubble up in `failedSteps`. |
...
| **JSON file persistence** | Zero dependency, instant startup. No database to configure — you just start it and go. |
...
| **100% Mistral TTS** | Mistral Voxtral TTS (no extra key beyond `MISTRAL_API_KEY`) — voice synthesis integrated into the cost-tracking pipeline and language-based voice resolution. |
...
- **[Luciole](https://luciole-vision.com)** — Font designed for visually impaired readers, © Laurent Bourcellier & Jonathan Perez, [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) (profiles’ “Reading comfort” option)
