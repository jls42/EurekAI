...
| 🎙️ | **Podcast** | Two-voice mini podcast in audio — default Mistral voices or custom voices (parents!) |
...
| 🎨 | **Theme by profile** | Each profile chooses its `dark` or `light` theme — persists when switching profiles |
...
|---|---|---|
| **Revision sheet** | `mistral-large-latest` | Title, summary, key points, vocabulary, quote, anecdote |
...
- **Content moderation**: `mistral-moderation-latest` with 10 available categories, 5 blocked by default for child/teen (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Customizable categories per profile in settings.
- **Parental PIN**: SHA-256 hash, required for profiles under 15 years old. For production deployment, use a slow salted hash (Argon2id, bcrypt).
...
- **Source of truth**: `helpers/pricing.ts` — `MODEL_PRICING` per model prefix (e.g. `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), `PRICING_SOURCES` with Mistral doc URLs for periodic re-scraping
...
| **TTS** | Mistral Voxtral TTS | `voxtral-mini-tts-latest`, integrated text-to-speech |
...
|---|---|---|
| `mistral-large-latest` | Summary sheet, Flashcards, Podcast, Quiz, Fill-in-the-blanks, Chat, Voice quiz checking, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `mistral-moderation-latest` | Content moderation | 5 blocked categories for children/teens (+ jailbreaking) |
| `mistral-small-latest` | Automatic router | Quick content analysis for routing decisions |
...
| `POST` | `/api/projects/:pid/sources/websearch` | Scrape URL or web search `{query}` — returns an array of sources |
...
| `POST` | `/api/projects/:pid/generate/auto` | Automatic backend generation (routing + 7 types: summary, flashcards, quiz, fill-blank, podcast, voice-quiz, image). Parallel execution — assumes a Mistral tier with rate limit ≥ 7 simultaneous requests; otherwise multiple 429s may surface in `failedSteps`. |
...
| **JSON file persistence** | Zero dependency, instant startup. No database to configure — just start and go. |
...
| **Multi-generation system** | Each generation is an independent object with its own ID — allows multiple sheets, quizzes, etc. per lesson. |
...
| **100% Mistral TTS** | Mistral Voxtral TTS (no extra key beyond `MISTRAL_API_KEY`) — voice synthesis integrated into the cost chain and language voice resolution. |
...
Started during the Mistral AI Worldwide Hackathon (March 2026), fully developed by AI with [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
