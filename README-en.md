...
| 📄 | **Study notes** | Structured notes with key points, vocabulary, quotations, anecdotes |
...
| 🗣️ | **Spoken quiz** | Questions read aloud (custom voice possible), oral response, AI verification |
...
| 🔊 | **Read aloud** | Listen to study notes and flashcards via Mistral Voxtral TTS or ElevenLabs |
...
EurekAI accepts 4 types of sources, moderated per profile (enabled by default for child and teen):
...
Seven types of learning materials generated:

| Generator | Model | Output |
|---|---|---|
| **Study sheet** | `mistral-large-latest` | Title, summary, key points, vocabulary, quotations, anecdote |
...
| **Spoken quiz** | `mistral-large-latest` + Voxtral TTS + STT | Questions TTS → response STT → AI verification |
...
- **Tool calls**: can generate study sheets, flashcards, quizzes or fill-in-the-blank texts during the conversation
- 50-message history per course
- Content moderation if enabled for the profile

### Automatic router

The router uses `mistral-small-latest` to analyze the content of sources and propose the most relevant generators among the 7 available. The UI displays real-time progress: first an analysis phase, then individual generations with possible cancellation.
...
- **Instruction detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible textual generators (study sheet, flashcards, quiz, fill-in-the-blank texts)
...
- Full Mistral voice catalogue available in settings, filterable by language
...
| Model | Use | Why |
|---|---|---|
| `mistral-large-latest` | Study sheet, Flashcards, Podcast, Quiz, Fill-in-the-blank texts, Chat, Spoken quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `POST` | `/api/projects/:pid/detect-consigne` | Detect revision instructions | ### Generation
| Method | Endpoint | Description |
|---|---|---|
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS playback aloud (cards/flashcards) |
...
| **Vite + Handlebars** | The best of both worlds: fast HMR for development, HTML partials for organizing code, Tailwind JIT. |
...
| **Agent-based features** | Image generation and web search use temporary Mistral Agents — proper lifecycle with automatic cleanup. |
