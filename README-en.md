...
The goal: take **any input** — a photo of the textbook, copied text, a voice recording, a web search — and transform it into **study guides, flashcards, quizzes, podcasts, cloze exercises, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited for French-speaking students.

The project was started during the hackathon, then continued and expanded afterward. All of the code is generated by AI — mainly [Claude Code](https://docs.anthropic.com/en/docs/claude-code), with some contributions from [Codex](https://openai.com/index/introducing-codex/).
...
| | Feature | Description |
|---|---|---|
| 📷 | **Upload OCR** | Photograph your textbook or notes — Mistral OCR extracts the content |
...
| 📄 | **Study guides** | Structured notes with key points, vocabulary, quotes, anecdotes |
| 🃏 | **Flashcards** | 5-50 Q/A cards with source references for active recall |
| ❓ | **Multiple-choice quizzes** | 5-50 MCQs with adaptive review of mistakes |
...
| 🔊 | **Read aloud** | Listen to study guides and flashcards via Mistral Voxtral TTS or ElevenLabs |
...
- **Web search** — Enter a query. A temporary Mistral Agent with the `web_search` tool fetches and summarizes results.
...
|---|---|---|
| **Study guide** | `mistral-large-latest` | Title, summary, 10-25 key points, vocabulary, quotes, anecdote |
...
- **Tool calls**: can generate study guides, flashcards, quizzes or cloze exercises during the conversation
- 50-message history per course
- Content moderation if enabled for the profile

### Automatic router

The router uses `mistral-small-latest` to analyze source content and propose the most relevant generators among the 7 available. The interface shows live progress: first an analysis phase, then individual generations with possible cancellation.
...
- **Instruction detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (study guide, flashcards, quiz, cloze exercises)

### Security & parental control

- **4 age groups**: child (≤10), teen (11-15), student (16-25), adult (26+)
- **Content moderation**: `mistral-moderation-2603` with 5 blocked categories for child/teen (sexual, hate, violence, selfharm, jailbreaking), no restrictions for student/adult
...
|---|---|---|
| **Runtime** | Node.js + TypeScript 5.x | Server and type safety |
...
| **File upload** | Multer 1.4 LTS | Multipart form handling |
...
| Model | Usage | Why |
|---|---|---|
| `mistral-large-latest` | Study guide, Flashcards, Podcast, Quiz, Cloze exercises, Chat, Voice quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `POST` | `/api/projects/:pid/detect-consigne` | Detect `POST` |
...
| `POST` | `/api/projects/:pid/generate/fill-blank` | Cloze exercises |
...
| **Agent-based features** | Image generation and web search use temporary Mistral Agents — self-contained lifecycle with automatic cleanup. |
...
Initiated during the Mistral AI Worldwide Hackathon (March 2026), developed entirely by AI with Claude Code and Codex.
