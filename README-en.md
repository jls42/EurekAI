...
**EurekAI** was born during the [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([official site](https://worldwide-hackathon.mistral.ai/)) (March 2026). I needed a project idea — and it came from something very concrete: I regularly prepare tests with my daughter, and I thought it should be possible to make that more playful and interactive with AI.

The goal: take **any input** — a photo of the lesson, copied text, a voice recording, a web search — and transform it into **study sheets, flashcards, quizzes, podcasts, fill-in-the-blank texts, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited for French-speaking students.

The project was developed during the hackathon on the [original repo](https://github.com/jls42/worldwide-hackathon.mistral.ai), then continued and expanded here. All code is AI-generated — mainly with [Claude Code](https://code.claude.com/), with some contributions via [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
...
| 🃏 | **Flashcards** | Q/A cards with source references for active recall (configurable count) |
| ❓ | **Multiple-choice quizzes** | Multiple-choice questions with adaptive review of mistakes (configurable count) |
...
| 🗣️ | **Voice quiz** | Questions read aloud, oral answer, AI verifies the response |
| 💬 | **AI tutor** | Contextual chat with your course documents, with tool calls |
| 🧠 | **Automatic router** | A router based on `mistral-small-latest` analyzes content and suggests a combination of generators among the 7 available types |
| 🔒 | **Parental control** | Moderation configurable per profile (customizable categories), parental PIN, chat restrictions |
| 🌍 | **Multilingual** | Interface available in 9 languages; AI generation can be driven in 15 languages via prompts |
| 🔊 | **Text-to-speech** | Listen to sheets and flashcards via Mistral Voxtral TTS or ElevenLabs |
...
- **Web search** — Enter a query. A temporary Mistral Agent with the `web_search` tool retrieves and summarizes results.

### AI content generation

Seven types of generated learning materials:
...
- **Tool calls**: can generate sheets, flashcards, quizzes or fill-in-the-blank texts during the conversation
- 50-message history per course
- Content moderation if enabled for the profile

### Automatic router

The router uses `mistral-small-latest` to analyze source content and suggest the most relevant generators among the 7 available. The UI shows real-time progress: first an analysis phase, then individual generations with cancel option.
...
- **Instruction detection**: detects study instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (sheet, flashcards, quiz, fill-in-the-blank)
...
- **Parental PIN**: SHA-256 hash, required for profiles under 15. For production deployment, use a slow hash with salt (Argon2id, bcrypt).
...
| Model | Use | Why |
|---|---|---|
| `mistral-large-latest` | Sheet, Flashcards, Podcast, Quiz, Fill-in-the-blank, Chat, Voice quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `voxtral-mini-tts-latest` | Speech synthesis (TTS) | Podcasts, voice quiz, text-to-speech |
...
> **Note**: Mistral Voxtral TTS is the default provider — no extra key required beyond `MISTRAL_API_KEY`. ElevenLabs is an alternative TTS provider configurable in settings.
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS playback (sheets/flashcards) |
...
## Architectural decisions | Decision | Justification |
|---|---|
...
| **Multi-generation system** | Each generation is an independent object with its own ID — allows multiple worksheets, quizzes, etc. per lesson. |
...
Initiated during the Mistral AI Worldwide Hackathon (March 2026), entirely developed by AI with [Claude Code](https://code.claude.com/), [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
