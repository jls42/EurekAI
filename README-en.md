...
**EurekAI** was born during the [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([official site](https://worldwide-hackathon.mistral.ai/)) (March 2026). I needed a project — and the idea came from something concrete: I regularly prepare tests with my daughter, and I thought it should be possible to make that more fun and interactive with AI.

The goal: take **any input** — a photo of the lesson, copied text, a voice recording, a web search — and turn it into **study sheets, flashcards, quizzes, podcasts, fill-in-the-blank exercises, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited for French-speaking students.

The [initial prototype](https://github.com/jls42/worldwide-hackathon.mistral.ai) was built in 48 hours during the hackathon as a proof of concept around Mistral services — already functional, but limited. Since then, EurekAI has become a full project: cloze texts, exercise navigation, web scraping, configurable parental moderation, deep code review, and more. All the code was generated by AI — mainly [Claude Code](https://code.claude.com/), with some contributions via [Codex](https://openai.com/codex/) and [Gemini CLI](https://geminicli.com/).
...
| ❓ | **Multiple-choice quizzes** | Multiple-choice questions with adaptive revision of mistakes (configurable number) |
...
| 🧠 | **Automatic router** | A router based on `mistral-small-latest` analyzes content and proposes a combination of generators among the 7 available types |
...
| **Multiple-choice quiz** | `mistral-large-latest` | Multiple-choice questions, explanations, adaptive revision (configurable number) |
...
The router uses `mistral-small-latest` to analyze source content and propose the most relevant generators among the 7 available. The interface shows real-time progress: first an analysis phase, then individual generations with possible cancellation.
...
- **Instruction detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible textual generators (sheet, flashcards, quiz, fill-in-the-blank)
...
- **Custom voices**: parents can create their own voices via the Mistral Voices API (from an audio sample) and assign them to host/guest roles — podcasts and spoken quizzes are then read with a parent's voice, making the experience more immersive for the child
- Two configurable voice roles: **host** (main narrator) and **guest** (second podcast voice)
- Full catalogue of Mistral voices available in settings, filterable by language
...
| Model | Usage | Why |
|---|---|---|
| `mistral-large-latest` | Sheet, Flashcards, Podcast, Quiz, Fill-in-the-blank, Chat, Spoken quiz verification, Image Agent, Web Search Agent, Instruction detection | Best multilingual + instruction following |
...
| `POST` | `/api/projects/:pid/generate/auto` | Backend auto-generation (routing + 5 types: summary, flashcards, quiz, fill-in-the-blank, podcast) |
...
| `POST` | `/api/projects/:pid/generations/:gid/vocal-answer` | Verify an oral response (audio + questionIndex) |
...
| **Persistence in JSON files** | Zero dependencies, instant startup. No database to configure — you start and you're ready. |
...
| **Multi-generation system** | Each generation is an independent object with its own ID — allows multiple cards, quizzes, etc. per course. |
...
| **Smart URL scraping** | A single field accepts mixed URLs and keywords — URLs are scraped via Readability (static pages) with a Lightpanda fallback (JS/SPA pages), keywords trigger a Mistral web_search Agent. Each result creates an independent source. |
