...
The goal: take **any input** — a photo of the textbook, copied text, a voice recording, a web search — and transform it into **study sheets, flashcards, quizzes, podcasts, fill-in-the-blank texts, illustrations, and more**. All powered by Mistral AI's French models, making it naturally suited to French-speaking students.

The project was started during the hackathon and then picked up and expanded afterward. All the code was generated by AI — mainly [Claude Code](https://docs.anthropic.com/en/docs/claude-code), with some contributions via [Codex](https://openai.com/index/introducing-codex/).
...
The router uses `mistral-small-latest` to analyze source content and propose the most relevant generators among the 7 available. The interface shows real-time progress: first an analysis phase, then individual generations with cancel option.
...
- **Instruction detection**: detects revision instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (study sheet, flashcards, quiz, fill-in-the-blank)

### Security & parental control

- **4 age groups**: child (≤10 years), teen (11-15), student (16-25), adult (26+)
- **Content moderation**: `mistral-moderation-latest` with 5 categories blocked for child/teen (sexual, hate, violence, selfharm, jailbreaking), no restriction for student/adult
...
| **Prompts centralisés** | All AI prompts in `prompts.ts` — easy to iterate, test and adapt by language/age group. |
| **Système multi-générations** | Each generation is an independent object with its own ID — allows multiple worksheets, quizzes, etc. per course. |
| **Prompts adaptés par âge** | 4 age groups with different vocabulary, complexity and tone — the same content teaches differently depending on the learner. |
| **Fonctionnalités basées sur les Agents** | Image generation and web search use temporary Mistral Agents — clean lifecycle with automatic cleanup. |
| **TTS multi-provider** | Mistral Voxtral TTS by default (no additional key), ElevenLabs as an alternative — configurable without restart. |
