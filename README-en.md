...
| ❓ | **MCQ Quiz** | Multiple-choice questions with adaptive review of mistakes (configurable count) |
...
| 🔊 | **Read Aloud** | Listen to study sheets and flashcards (question/answer dialogue) via Mistral Voxtral TTS |
...
- **File Import** — JPG, PNG, or PDF files processed by Mistral OCR — **OCR 3 (`mistral-ocr-2512`) by default**, **OCR 4 (`mistral-ocr-4-0`) optional** in Settings (better quality, but 2× the cost) — for printed text, tables, and handwriting; or text files (TXT, MD) imported directly. Multi-file uploads use an **upload session** system: individual progress per file, retry the failed file without resubmitting the others, dismiss the session when finished. OCR exposes an averaged **confidence score** (`average`, clamped to `[0,1]`, computed from `averagePageConfidenceScore` returned by Mistral), displayed in the UI as a tier `high` / `medium` / `low` badge (thresholds ~0.9 / ~0.7) — warns without blocking if the scan is low quality.
- **Free Text** — Type or paste any content. Moderated before storage if moderation is enabled.
- **Voice Input** — Record audio in the browser. Transcribed by `voxtral-mini-latest`. The `language="fr"` setting improves recognition.
...
- **Tool calling**: can generate study sheets, flashcards, quizzes, or fill-in-the-blank texts during the conversation
- 50-message history per course
- Content moderation if enabled for the profile

### Automatic Router

The router uses `mistral-small-latest` to analyze source content and suggest the most relevant generators among the 7 available. The interface shows real-time progress: first an analysis phase, then individual generations with cancellation possible.
...
- **Instruction detection**: detects review instructions ("I know my lesson if I know...") and prioritizes them in compatible text generators (study sheet, flashcards, quiz, fill-in-the-blank texts)

### Safety & Parental Controls

- **4 age groups**: child (≤10), teen (11-15), student (16-25), adult (26+)
- **Content moderation**: `mistral-moderation-latest` with 10 available categories, 5 blocked by default for child/teen (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Categories can be customized by profile in settings.
...
- **Profile voices** (`Profile.mistralVoices?: { host, guest }`) — each child can have their own podcast/voice quiz voice pair
- **Profile theme** (`Profile.theme: 'dark' | 'light'`) — automatic switch when changing profile, persisted on the backend
...
- **Mistral Voxtral TTS**: `voxtral-mini-tts-latest`, 100% Mistral text-to-speech, no extra key needed
- **Custom voices**: parents can create their own voices via the Mistral Voices API (from an audio sample) and assign them to the host/guest roles — podcasts and voice quizzes are then read using a parent's voice, making the experience even more immersive for the child
...
| **Web scraping** | Readability + linkedom | Main content extraction from web pages (Firefox Reader View tech) |
...
| Model | Use | Why |
|---|---|---|
| `mistral-large-latest` | Study Sheet, Flashcards, Podcast, Quiz, Fill-in-the-Blank Texts, Chat, Voice Quiz Verification, Image Agent, Web Search Agent, Instruction Detection | Best multilingual + instruction following |
...
> **Note**: Mistral Voxtral TTS is the only TTS provider — no extra key needed beyond `MISTRAL_API_KEY`.
...
> **For AI contributors**: see [`CLAUDE.md`](CLAUDE.md) for detailed architectural context, mandatory rules (anti-leak prompts, error codes, cost tracking), and known pitfalls (Lizard CCN, Opengrep, Codacy/Semgrep migration).
...
| `PUT` | `/api/config` | Modify the config (models, voice, TTS model) |
...
| **JSON file persistence** | Zero dependencies, instant startup. No database to configure — we just start and go. |
...
| **100% Mistral TTS** | Mistral Voxtral TTS (no additional key beyond `MISTRAL_API_KEY`) — speech synthesis integrated into cost tracking and language-based voice resolution. |
...
- **[Lightpanda](https://lightpanda.io)** — Ultra-light headless browser for JS/SPA page scraping
