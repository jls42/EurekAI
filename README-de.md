...
<p align="center">
  <strong>Verwandle beliebige Inhalte in interaktive Lernerfahrungen — angetrieben durch KI.</strong>
</p>
...
**EurekAI** entstand während des [Mistral AI Worldwide Hackathon](https://worldwide-hackathon.mistral.ai/) (März 2026). Ich brauchte ein Thema — und die Idee kam aus etwas sehr Konkretem: Ich bereite regelmäßig Klassenarbeiten mit meiner Tochter vor und dachte, es müsste möglich sein, das mithilfe von KI spielerischer und interaktiver zu gestalten.

Ziel: Beliebige Eingaben — ein Foto aus dem Lehrbuch, ein kopierter Text, eine Sprachaufnahme, eine Websuche — in **Lernzettel, Karteikarten, Quiz, Podcasts, Lückentexte, Illustrationen und mehr** zu verwandeln. Alles angetrieben von den französischen Modellen von Mistral AI, wodurch die Lösung von Natur aus gut für frankophone Schüler geeignet ist.

Jede Codezeile wurde während des Hackathons geschrieben. Alle eingesetzten APIs und Open-Source-Bibliotheken werden gemäß den Regeln des Hackathons verwendet.
...
| 🃏 | **Karteikarten** | 5–50 Q/A-Karten mit Quellenangaben für aktives Erinnern |
| ❓ | **Multiple-Choice-Quiz** | 5–50 Multiple-Choice-Fragen mit adaptiver Fehlerüberprüfung |
...
| 🖼️ | **Illustrationen** | Lehrreiche Bilder generiert von einem Mistral-Agent |
| 🗣️ | **Sprachquiz** | Fragen werden vorgelesen, Antwort mündlich — KI prüft die Antwort |
| 💬 | **KI-Tutor** | Kontextueller Chat mit Ihren Kursdokumenten, mit Tool-Aufrufen |
| 🧠 | **Intelligenter Router** | Die KI analysiert Ihren Inhalt und empfiehlt die relevantesten Generatoren aus den 7 verfügbaren |
| 🔒 | **Elterliche Kontrolle** | Altersmoderation, Eltern-PIN, Chat-Einschränkungen |
| 🌍 | **Mehrsprachig** | Interface und KI-Inhalte vollständig auf Französisch und Englisch |
...
EurekAI akzeptiert 4 Quellentypen, moderiert je nach Profil (standardmäßig für Kind und Teen aktiviert):

- **OCR-Upload** — JPG-, PNG- oder PDF-Dateien, verarbeitet durch `mistral-ocr-latest`. Verarbeitet gedruckten Text, Tabellen und Handschrift.
- **Freitext** — Tippen oder Einfügen beliebiger Inhalte. Wird vor der Speicherung moderiert, wenn Moderation aktiviert ist.
- **Sprachaufnahme** — Audio direkt im Browser aufnehmen. Transkribiert durch `voxtral-mini-latest`. Der Parameter `language="fr"` optimiert die Erkennung.
- **Websuche** — Anfrage eingeben. Ein temporärer Mistral-Agent mit dem Tool `web_search` holt und fasst die Ergebnisse zusammen.

### KI-Inhaltserzeugung

Sieben Typen von generiertem Lernmaterial:

| Generator | Modell | Ausgabe |
|---|---|---|
| **Lernzettel** | `mistral-large-latest` | Titel, Zusammenfassung, 10–25 Kernpunkte, Vokabular, Zitate, Anekdote |
| **Karteikarten** | `mistral-large-latest` | 5–50 Q/A-Karten mit Quellenangaben für aktives Erinnern |
| **Multiple-Choice-Quiz** | `mistral-large-latest` | 5–50 Fragen, je 4 Antwortoptionen, Erklärungen, adaptive Wiederholung |
...
| **Sprachquiz** | `mistral-large-latest` + Voxtral TTS + STT | Fragen per TTS → Antwort per STT → KI-Prüfung |
...
### Automatischer intelligenter Router

Der Router verwendet `mistral-small-latest`, um den Inhalt der Quellen zu analysieren und zu empfehlen, welche Generatoren unter den 7 verfügbaren am passendsten sind — damit Lernende nicht manuell auswählen müssen. Die Oberfläche zeigt den Echtzeitfortschritt: zuerst eine Analysephase, dann die einzelnen Generierungen mit Möglichkeit zum Abbrechen.

### Adaptives Lernen

- **Quiz-Statistiken**: Verfolgung von Versuchen und Genauigkeit pro Frage
- **Quiz-Wiederholung**: Generiert 5–10 neue Fragen, die gezielt auf Schwächen eingehen
- **Anweisungs-Erkennung**: Erkennt Lernanweisungen ("Ich kann meine Lektion, wenn ich ... kann") und priorisiert diese in allen Generatoren

### Sicherheit & elterliche Kontrolle

- **4 Altersgruppen**: Kind (≤10 Jahre), Teenager (11–15), Student (16–25), Erwachsener (26+)
- **Inhaltsmoderation**: `mistral-moderation-2603` mit 5 für Kind/Teen gesperrten Kategorien (Sexualität, Hass, Gewalt, Selbstverletzung, Jailbreaking), keine Einschränkungen für Student/Erwachsener
...
- **Mistral Voxtral TTS** (Standard): `voxtral-mini-tts-latest`, kein zusätzlicher Schlüssel notwendig
...
- Oberfläche vollständig auf Französisch und Englisch verfügbar
- KI-Prompts unterstützen heute 2 Sprachen (FR, EN) mit Architektur, die für 15 Sprachen bereit ist (es, de, it, pt, nl, ja, zh, ko, ar, hi, pl, ro, sv)
...
| **Templating** | vite-plugin-handlebars | HTML-Zusammensetzung mittels Partials |
...
| **TTS (Alternative)** | ElevenLabs SDK 2.36 | `eleven_v3`, natürliche Stimmen |
| **Icons** | Lucide 0.575 | SVG-Icon-Bibliothek |
| **Markdown** | Marked 17 | Markdown-Rendering im Chat |
| **Datei-Upload** | Multer 1.4 | Multipart-Formularverarbeitung |
| **Audio** | ffmpeg-static | Zusammenfügen von Audio-Segmenten |
| **Tests** | Vitest 4 | Unit-Tests — Coverage gemessen via SonarCloud |
| **Persistenz** | JSON-Dateien | Speicherung ohne zusätzliche Abhängigkeiten |
...
|---|---|---|
| `mistral-large-latest` | Lernzettel, Karteikarten, Podcast, Quiz, Lückentexte, Chat, Prüfung Sprachquiz, Image-Agent, Web-Search-Agent, Anweisungs-Erkennung | Beste Multilingualität + Befolgen von Anweisungen |
...
| `mistral-moderation-2603` | Inhaltsmoderation | 5 für Kind/Teen gesperrte Kategorien (+ Jailbreaking) |
| `mistral-small-latest` | Intelligenter Router | Schnelle Inhaltsanalyse zur Routing-Entscheidung |
| `eleven_v3` (ElevenLabs) | Sprachsynthese (alternativ TTS) | Natürliche Stimmen, konfigurierbare Alternative |
...
> **Hinweis** : Mistral Voxtral TTS ist der Standardprovider — kein zusätzlicher Schlüssel erforderlich außer `MISTRAL_API_KEY`. ElevenLabs ist ein alternativer TTS-Provider, konfigurierbar in den Einstellungen.
...
| `PUT` | `/api/config` | Config ändern (Modelle, Stimmen, TTS-Provider) |
| `GET` | `/api/config/status` | API-Status (Mistral, ElevenLabs, TTS) |
| `POST` | `/api/config/reset` | Config auf Standard zurücksetzen |
...
| `DELETE` | `/api/profiles/:id` | Profil löschen + kaskadierende Projekte |
...
| `POST` | `/api/projects/:pid/detect-consigne` | Anweisungen erkennen |
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS-Vorlese-Funktion (Lernzettel/Karteikarten) |
...
| **Persistenz in JSON-Dateien** | Keine Abhängigkeiten, sofortiger Start. Keine Datenbankeinrichtung — einfach starten und loslegen. |
| **Vite + Handlebars** | Das Beste aus beiden Welten: schnelles HMR für Entwicklung, HTML-Partials zur Code-Organisation, Tailwind JIT. |
| **Zentralisierte Prompts** | Alle KI-Prompts in `prompts.ts` — leicht zu iterieren, zu testen und pro Sprache/Altersgruppe anzupassen. |
| **Multi-Generationen-System** | Jede Generation ist ein eigenständiges Objekt mit eigener ID — ermöglicht mehrere Arbeitsblätter, Quiz usw. pro Kurs. |
| **Altersangepasste Prompts** | 4 Altersgruppen mit unterschiedlichem Wortschatz, Komplexität und Ton — derselbe Inhalt wird je nach Lernendem anders vermittelt. |
