...
|---|---|---|
| 📷 | **Importación de archivos** | Importa tus lecciones — foto, PDF (mediante Mistral OCR con puntuación de confianza) o archivo de texto (TXT, MD) |
...
| 🃏 | **Flashcards** | Tarjetas de preguntas y respuestas interactivas, lectura de audio dialogada |
| ❓ | **Cuestionarios tipo test** | Preguntas de opción múltiple con repaso adaptativo de errores (número configurable) |
...
| 🔊 | **Lectura en voz alta** | Escucha las fichas y flashcards (diálogo pregunta/respuesta) mediante Mistral Voxtral TTS o ElevenLabs |
...
- **Web / URL** — Pega una o varias URLs para scrapear el contenido directamente (Readability + Lightpanda para las páginas JS), o escribe palabras clave para una búsqueda web mediante Agent Mistral. El campo único acepta ambos — URLs y palabras clave se separan automáticamente, y cada resultado crea una fuente independiente.
...
| **Cuestionario tipo test** | `mistral-large-latest` | Preguntas de opción múltiple, explicaciones, repaso adaptativo (número configurable) |
...
- **Detección de instrucciones**: detecta las instrucciones de repaso ("Sé mi lección si sé...") y las prioriza en los generadores textuales compatibles (ficha, flashcards, cuestionario, textos con huecos)
...
- **Mistral Voxtral TTS** (predeterminado) : `voxtral-mini-tts-latest`, no se necesita ninguna clave adicional
...
- **Voces personalizadas**: los padres pueden crear sus propias voces mediante la API de Mistral Voices (a partir de una muestra de audio) y asignarlas a los roles de anfitrión/invitado — los podcasts y cuestionarios por voz se reproducen entonces con la voz de un padre, haciendo que la experiencia sea aún más inmersiva para el niño
...
| **TTS (predeterminado)** | Mistral Voxtral TTS | `voxtral-mini-tts-latest`, síntesis de voz integrada |
...
| Modelo | Uso | Por qué |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos con huecos, Chat, Verificación de cuestionario por voz, Agent Imagen, Agent Web Search, Detección de instrucciones | Mejor multilingüe + seguimiento de instrucciones |
...
| `voxtral-mini-tts-latest` | Síntesis de voz (TTS) | Podcasts, cuestionario por voz, lectura en voz alta |
...
> **Nota** : Mistral Voxtral TTS es el proveedor predeterminado — no se necesita ninguna clave adicional más allá de `MISTRAL_API_KEY`. ElevenLabs es un proveedor TTS alternativo configurable en los ajustes.
...
> **`ELEVENLABS_API_KEY`** es opcional (TTS alternativo).
...
| `GET` | `/api/config/voices` | Listar las voces de Mistral TTS (opcional `?lang=fr`) |
...
| `DELETE` | `/api/profiles/:id` | Eliminar un perfil + cascada de proyectos `{pin?}` → `{ok, deletedProjects}` | ### Proyectos
| Método | Endpoint | Descripción |
|---|---|---|
...
| `POST` | `/api/projects/:pid/sources/websearch` | Scraping de URL o búsqueda web `{query}` — devuelve una matriz de fuentes |
...
| `POST` | `/api/projects/:pid/generate/route` | Análisis de enrutamiento (plan de los generadores a ejecutar) |
| `POST` | `/api/projects/:pid/generate/auto` | Generación automática backend (enrutamiento + 7 tipos: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Ejecución en paralelo — supone un tier Mistral con rate-limit ≥ 7 solicitudes simultáneas; de lo contrario, varios 429 pueden remontarse en `failedSteps`. |
...
| **Scraping inteligente de URL** | Un campo único acepta URLs y palabras clave mezcladas — las URLs se scrapean mediante Readability (páginas estáticas) con fallback Lightpanda (páginas JS/SPA), las palabras clave activan un Agent Mistral web_search. Cada resultado crea una fuente independiente. |
| **TTS multi-provider** | Mistral Voxtral TTS por defecto (sin clave adicional), ElevenLabs como alternativa — configurable sin reinicio. |
