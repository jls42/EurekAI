...
|---|---|---|
| 📷 | **Importación de archivos** | Importa tus lecciones — foto, PDF (mediante Mistral OCR con puntuación de confianza promediada, niveles `high`/`medium`/`low`) o archivo de texto (TXT, MD). Sesiones de carga con reintento por archivo y progreso individual |
| 📝 | **Entrada de texto** | Escribe o pega cualquier texto directamente |
| 🎤 | **Entrada de voz** | Grábate — Voxtral STT transcribe tu voz |
| 🌐 | **Web / URL** | Pega una URL (scraping directo mediante Readability + Lightpanda) o escribe una búsqueda (agente web_search de Mistral) |
...
| 🎙️ | **Podcast** | Minipodcast de 2 voces en audio — voz Mistral por defecto o voces personalizadas (¡de los padres!) |
| 🖼️ | **Ilustraciones** | Imágenes educativas generadas por un agente Mistral |
| 🗣️ | **Quiz por voz** | Preguntas leídas en voz alta (voz personalizada posible), respuesta oral, verificación IA |
| 💬 | **Tutor de IA** | Chat contextual con tus documentos de clase, con llamada de herramientas |
...
- **Importación de archivos** — Archivos JPG, PNG o PDF tratados por OCR Mistral — **OCR 3 (`mistral-ocr-2512`) por defecto**, **OCR 4 (`mistral-ocr-4-0`) opcional** en los Ajustes (mejor calidad, pero 2× el coste) — para texto impreso, tablas y escritura manuscrita; o archivos de texto (TXT, MD) importados directamente. Las cargas multiarchivo usan un sistema de **sesiones de carga**: progreso individual por archivo, reintento del archivo fallido sin volver a enviar los demás, cierre de la sesión cuando termina. El OCR expone una **puntuación de confianza** promediada (`average`, limitada en `[0,1]`, calculada a partir de `averagePageConfidenceScore` devueltos por Mistral), mostrada en la UI como insignia de nivel `high` / `medium` / `low` (umbrales ~0.9 / ~0.7) — avisa sin bloquear si el escaneo es de mala calidad.
- **Texto libre** — Escribe o pega cualquier contenido. Moderado antes de guardarlo si la moderación está activa.
...
| **Quiz por voz** | `mistral-large-latest` + Voxtral TTS + STT | Preguntas TTS → respuesta STT → verificación IA |
...
- **Llamada de herramientas**: puede generar fichas, flashcards, quiz o textos con huecos durante la conversación
- Historial de 50 mensajes por curso
- Moderación del contenido si está activada para el perfil

### Enrutador automático

El enrutador usa `mistral-small-latest` para analizar el contenido de las fuentes y proponer los generadores más pertinentes entre los 7 disponibles. La interfaz muestra el progreso en tiempo real: primero una fase de análisis, luego las generaciones individuales con posibilidad de cancelación.
...
- **Detección de consigna**: detecta las instrucciones de repaso ("Sé la lección si sé...") y las prioriza en los generadores textuales compatibles (ficha, flashcards, quiz, textos con huecos)
...
- **Restricciones del chat**: el chat de IA está desactivado por defecto para los menores de 16 años, activable por los padres
...
- Eliminación en cascada: borrar un perfil elimina todos sus proyectos
...
- **Unidades admitidas**: `tokens`, `characters` (TTS), `pages` (OCR), `audio-seconds` (STT) — conversión pilotada por `helpers/cost-calc.ts`
...
- **Endpoints**: las respuestas `/generate/*` y `/sources/*` decoran el objeto devuelto (Generation / Source) con `estimatedCost`, `usage` y `costBreakdown`. `POST /generate/auto/route` añade un campo `costDelta: number` para el coste del enrutamiento solo. `GET /projects/:pid` devuelve el proyecto enriquecido con `totalCost` (suma calculada desde `costLog[]`) + el historial completo

### TTS multproveedor y voces personalizadas

- **Mistral Voxtral TTS**: `voxtral-mini-tts-latest`, síntesis de voz 100% Mistral, no se necesita ninguna clave adicional
- **Voces personalizadas**: los padres pueden crear sus propias voces mediante la API Mistral Voices (a partir de una muestra de audio) y asignarlas a los roles de anfitrión/invitado — los podcasts y los quiz por voz se reproducen entonces con la voz de un padre, haciendo la experiencia todavía más inmersiva para el niño
- Dos roles de voz configurables: **anfitrión** (narrador principal) e **invitado** (segunda voz del podcast)
- Catálogo completo de las voces Mistral disponible en los ajustes, filtrable por idioma
...
| **Navegador headless** | Lightpanda | Navegador headless ultraligero (Zig + V8) para las páginas JS/SPA — scraping de respaldo |
...
| Modelo | Uso | Por qué |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos con huecos, Chat, Verificación del quiz por voz, Agente de imagen, Agente de búsqueda web, Detección de consigna | Mejor multilingüe + seguimiento de instrucciones |
| `mistral-ocr-2512` (OCR 3, por defecto) | OCR de documentos | Texto impreso, tablas, escritura manuscrita ($2 / 1000 pages) |
| `mistral-ocr-4-0` (OCR 4, opcional) | OCR de documentos — calidad superior | Seleccionable en Ajustes, 2× el coste ($4 / 1000 pages) |
...
> **Clave API ingresada por el usuario** : `MISTRAL_API_KEY` es ahora **opcional**. Si falta, la app arranca igualmente y pide a cada usuario que introduzca **su propia clave Mistral** en la interfaz. La clave se **almacena en el navegador** (cifrada mediante Web Crypto + IndexedDB en un contexto seguro) y se envía por solicitud — **nunca se persiste en el servidor**. Precedencia: clave del perfil > clave global del navegador > `MISTRAL_API_KEY` (env). Definir `EUREKAI_REQUIRE_USER_KEY=true` obliga a cada usuario a proporcionar su clave (la clave de env solo sirve ya para las precargas).

> **HTTPS local (tableta/LAN)** : `localhost` ya es un contexto seguro. Para un acceso por LAN (tableta), genera un certificado local y activa HTTPS para desbloquear el cifrado del navegador + cifrar la clave en tránsito :
> ```bash
...
| `SONAR_TOKEN` | opcional CI | — | Usado únicamente por el workflow de GitHub Actions SonarCloud |
...
**Hooks de Git (Husky)**: `pre-commit` ejecuta `npm test`, `pre-push` ejecuta `npm run security`. Ambos bloquean el commit/push en caso de error.
...
Sin estas herramientas, `npm test` falla en `pretest` (lizard ausente) y `npm run security` falla (opengrep ausente). Entonces los hooks de husky bloquean el commit/push.
...
> **`:U`** es una bandera de Podman rootless que ajusta automáticamente los permisos del volumen.
...
> **Para los contribuyentes de IA**: consultar [`CLAUDE.md`](CLAUDE.md) para el contexto arquitectónico detallado, las reglas obligatorias (anti-leak prompts, códigos de error, seguimiento de costos) y los problemas conocidos (Lizard CCN, Opengrep, migración Codacy/Semgrep).
...
| `POST` | `/api/projects/:pid/sources/websearch` | Scraping de URL o búsqueda web `{query}` — devuelve un arreglo de fuentes |
...
| `POST` | `/api/projects/:pid/generate/remediation-summary` | Ficha de refuerzo centrada en las preguntas falladas de un quiz `{generationId, weakQuestions}` — llamada en paralelo de `quiz-review` por el botón «Entrenarme en mis errores» |
...
| `POST` | `/api/projects/:pid/generate/auto` | Generación automática backend (enrutamiento + 7 tipos: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Ejecución en paralelo — supone un nivel Mistral con rate-limit ≥ 7 solicitudes simultáneas; si no, varios 429 pueden aparecer en `failedSteps`. |
...
| **Sistema multigeneración** | Cada generación es un objeto independiente con su propio ID — permite varias fichas, quiz, etc. por curso. |
...
| **TTS 100% Mistral** | Mistral Voxtral TTS (sin clave adicional más allá de `MISTRAL_API_KEY`) — síntesis de voz integrada en la cadena de costo y en la resolución de voz por idioma. |
