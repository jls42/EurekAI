...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI"><img src="https://img.shields.io/badge/▶️_Bekijk_de_demo-YouTube-red?style=for-the-badge&logo=youtube" alt="YouTube-demo"></a>
</p>
...
|---|---|---|
| 📷 | **Bestandsimport** | Importeer je lessen — foto, PDF (via Mistral OCR met gemiddeld vertrouwensscore, niveau `high`/`medium`/`low`) of tekstbestand (TXT, MD). Uploadsessies met retry per bestand en individuele voortgang |
| 📝 | **Tekstinvoer** | Typ of plak rechtstreeks eender welke tekst |
| 🎤 | **Spraakinvoer** | Neem jezelf op — Voxtral STT transcribeert je stem |
| 🌐 | **Web / URL** | Plak een URL (rechtstreekse scraping via Readability + Lightpanda) of typ een zoekopdracht (Agent Mistral web_search) |
| 📄 | **Samenvattingen** | Gestructureerde notities met kernpunten, vocabulaire, citaten, anekdotes |
| 🃏 | **Flashcards** | Interactieve vraag/antwoord-kaarten, gesproken audio-lezing |
...
| 🗣️ | **Spraakquiz** | Vragen hardop voorgelezen (aangepaste stem mogelijk), mondeling antwoord, AI-controle |
| 💬 | **AI-tutor** | Contextuele chat met je lesdocumenten, met toolgebruik |
...
| 🔊 | **Voorlezen** | Luister naar samenvattingen en flashcards (vraag/antwoord-dialogen) via Mistral Voxtral TTS |
| 💶 | **API-kosten volgen** | Transparante schatting van de €-kost van elke generatie en bron (tokens / tekens / pagina's / audioconcessies). Badge per kaart + totaal per project, zichtbaar in het dashboard |
...
- **Bestandsimport** — JPG-, PNG- of PDF-bestanden verwerkt door `mistral-ocr-latest` (gedrukte tekst, tabellen, handschrift), of tekstbestanden (TXT, MD) rechtstreeks geïmporteerd. Multi-bestandsuploads gebruiken een systeem van **uploadsessies**: individuele voortgang per bestand, retry van een mislukt bestand zonder de andere opnieuw te verzenden, sessie sluiten wanneer klaar. De OCR geeft een **gemiddelde vertrouwensscore** weer (`average`, begrensd in `[0,1]`, berekend op basis van `averagePageConfidenceScore` teruggegeven door Mistral), weergegeven in de UI als badge niveau `high` / `medium` / `low` (drempels ~0.9 / ~0.7) — waarschuwt zonder te blokkeren als de scan van slechte kwaliteit is.
...
- **Web / URL** — Plak een of meerdere URL's om de inhoud rechtstreeks te scrapen (Readability + Lightpanda voor JS-pagina's), of typ trefwoorden voor een webzoekopdracht via Agent Mistral. Het ene veld accepteert beide — URL's en trefwoorden worden automatisch gescheiden, elk resultaat creëert een onafhankelijke bron.
...
Een gesprekstutor met volledige toegang tot de lesdocumenten:

- Gebruikt `mistral-large-latest`
- **Toolgebruik**: kan tijdens het gesprek samenvattingen, flashcards, quizzen of invuloefeningen genereren
- Geschiedenis van 50 berichten per les
- Inhoudsmoderatie indien geactiveerd voor het profiel

### Automatische router

De router gebruikt `mistral-small-latest` om de inhoud van de bronnen te analyseren en de meest relevante generatoren voor te stellen uit de 7 beschikbare. De interface toont de voortgang in realtime: eerst een analysefase, daarna de afzonderlijke generaties met mogelijkheid tot annuleren.
...
- **Signaaldetectie**: detecteert revisie-instructies ("Ik ken mijn les als ik weet...") en geeft ze prioriteit in compatibele tekstgeneratoren (samenvatting, flashcards, quiz, invuloefeningen)

### Beveiliging & ouderlijk toezicht

- **4 leeftijdsgroepen**: kind (≤10 jaar), tiener (11-15), student (16-25), volwassene (26+)
- **Inhoudsmoderatie**: `mistral-moderation-latest` met 10 beschikbare categorieën, 5 standaard geblokkeerd voor kind/tiener (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Per profiel aanpasbare categorieën in de instellingen.
...
- **Thema per profiel** (`Profile.theme: 'dark' | 'light'`) — automatische omschakeling bij profielwissel, persistent aan backendzijde
...
Elke Mistral-aanroep (chat, OCR, STT, TTS, moderatie, agents) is geïnstrumenteerd om de gebruiker een **transparante** €-schatting te bieden — geen verrassingen op de factuur.

- **Bron van waarheid**: `helpers/pricing.ts` — `MODEL_PRICING` per modelprefix (bv. `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), `PRICING_SOURCES` met Mistral-doc-URL's voor periodiek herscrapen
...
- **Endpoints**: de antwoorden `/generate/*` en `/sources/*` decoreren het geretourneerde object (Generation / Source) met `estimatedCost`, `usage` en `costBreakdown`. `POST /generate/auto/route` voegt een veld `costDelta: number` toe voor de kost van alleen de routering. `GET /projects/:pid` retourneert het verrijkte project met `totalCost` (som berekend uit `costLog[]`) + de volledige geschiedenis
...
- **Aangepaste stemmen**: ouders kunnen hun eigen stemmen creëren via de Mistral Voices API (op basis van een audiofragment) en die toewijzen aan de rollen host/gast — podcasts en spraakquizzen worden dan voorgelezen met de stem van een ouder, wat de ervaring voor het kind nog meeslepender maakt
...
- Taal per profiel configureerbaar
...
| **Persistente opslag** | JSON-bestanden | Opslag zonder afhankelijkheid |

---

## Modelreferentie | Model | Gebruik | Waarom |
|---|---|---|
| `mistral-large-latest` | Overzicht, Flashcards, Podcast, Quiz, Tekst met open plekken, Chat, Controle van gesproken quiz, Afbeeldingsagent, Webzoekagent, Instructiedetectie | Beste multilingual + instructieopvolging |
| `mistral-ocr-latest` | OCR van documenten | Gedrukte tekst, tabellen, handschrift |
| `voxtral-mini-latest` | Spraakherkenning (STT) | Meertalige STT, geoptimaliseerd met `language="fr"` |
| `voxtral-mini-tts-latest` | Tekst-naar-spraak (TTS) | Podcasts, gesproken quiz, hardop voorlezen |
| `mistral-moderation-latest` | Contentmoderatie | 5 geblokkeerde categorieën voor kind/tiener (+ jailbreaking) |
...
> **Opmerking**: Mistral Voxtral TTS is de enige TTS-provider — geen extra sleutel nodig bovenop `MISTRAL_API_KEY`.
...
**Git-hooks (Husky)**: `pre-commit` voert `npm test` uit, `pre-push` voert `npm run security` uit. Beide blokkeren commit/push bij falen.
...
Zonder deze tools faalt `npm test` bij `pretest` (lizard ontbreekt) en `npm run security` faalt (opengrep ontbreekt). De husky-hooks blokkeren dan commit/push.
...
> **`:U`** is een rootless Podman-flag die automatisch de volume-permissies aanpast.
...
| `PUT` | `/api/config` | Config wijzigen (modellen, stemmen, TTS-model) |
...
| Methode | Endpoint | Beschrijving |
|---|---|---|
| `POST` | `/api/projects/:pid/generate/summary` | Overzicht voor revisie |
| `POST` | `/api/projects/:pid/generate/flashcards` | Flashcards |
| `POST` | `/api/projects/:pid/generate/quiz` | Meerkeuzequiz |
| `POST` | `/api/projects/:pid/generate/fill-blank` | Tekst met open plekken |
...
| `POST` | `/api/projects/:pid/generate/route` | Routeringsanalyse (plan van de te starten generatoren) — geeft `{plan, costDelta}` terug (alleen routeringskosten) |
| `POST` | `/api/projects/:pid/generate/auto` | Backend automatische generatie (routing + 7 types: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Parallelle uitvoering — veronderstelt een Mistral-tier met rate limit ≥ 7 gelijktijdige verzoeken; anders kunnen meerdere 429's in `failedSteps` terugkomen. |

Alle generatie-routes accepteren `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}`. `quiz-review` vereist bovendien `{generationId, weakQuestions}`.

### CRUD-generaties
| Methode | Endpoint | Beschrijving |
|---|---|---|
| `POST` | `/api/projects/:pid/generations/:gid/quiz-attempt` | Quizantwoorden indienen `{answers}` |
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | Antwoorden voor teksten met open plekken indienen `{answers}` |
| `POST` | `/api/projects/:pid/generations/:gid/vocal-answer` | Een gesproken antwoord controleren (audio + questionIndex) |
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS hardop voorlezen (overzichten/flashcards) |
...
| **Persistentie in JSON-bestanden** | Geen afhankelijkheden, directe opstart. Geen database om te configureren — opstarten en gaan. |
| **Vite + Handlebars** | Het beste van twee werelden: snelle HMR voor ontwikkeling, HTML-partials voor codeorganisatie, Tailwind JIT. |
| **Gecentraliseerde prompts** | Alle AI-prompts in `prompts.ts` — eenvoudig itereren, testen en aanpassen per taal/leeftijdsgroep. |
| **Multi-generatiesysteem** | Elke generatie is een onafhankelijk object met een eigen ID — maakt meerdere overzichten, quizzen, enz. per cursus mogelijk. |
| **Op leeftijd aangepaste prompts** | 4 leeftijdsgroepen met verschillende woordenschat, complexiteit en toon — dezelfde inhoud wordt anders aangeleerd afhankelijk van de leerling. |
| **Op Agents gebaseerde functionaliteiten** | Afbeeldingsgeneratie en websearch gebruiken tijdelijke Mistral Agents — nette levenscyclus met automatische cleanup. |
| **Slimme URL-scraping** | Eén veld accepteert gemengde URL's en trefwoorden — URL's worden gescrapet via Readability (statische pagina's) met fallback Lightpanda (JS-/SPA-pagina's), trefwoorden starten een Mistral web_search-agent. Elk resultaat maakt een onafhankelijke bron aan. |
| **100% Mistral TTS** | Mistral Voxtral TTS (geen extra sleutel nodig bovenop `MISTRAL_API_KEY`) — spraaksynthese geïntegreerd in de kostenketen en in de stemresolutie per taal. |
