...
<p align="center">
  <strong>Transformeer elke inhoud in een interactieve leerervaring — aangedreven door AI.</strong>
</p>
...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI">▶️ Bekijk de demo op YouTube</a> · <a href="README-en.md">🇬🇧 Read in English</a>
</p>
...
Elke regel code werd tijdens de hackathon geschreven. Alle open-source APIs en bibliotheken worden gebruikt in overeenstemming met de hackathonregels.
...
|---|---|---|
| 📷 | **OCR-upload** | Maak een foto van je handboek of notities — Mistral OCR extraheert de inhoud |
| 📝 | **Tekstinvoer** | Typ of plak elk gewenste tekst rechtstreeks |
...
| ❓ | **Meerkeuzequiz** | 5-50 meerkeuzevragen met adaptieve review van fouten |
| ✏️ | **Invuloefeningen** | Oefeningen om in te vullen met hints en tolerantiewaardering bij controle |
...
| 💬 | **AI-tutor** | Contextuele chat met je lesdocumenten, met tool-aanroepen |
| 🧠 | **Slimme router** | De AI analyseert je inhoud en raadt de meest relevante generators aan uit de 7 beschikbare |
| 🔒 | **Ouderlijk toezicht** | Leeftijdsmoderatie, ouder-PIN, chatbeperkingen |
...
- **OCR-upload** — JPG-, PNG- of PDF-bestanden verwerkt door `mistral-ocr-latest`. Behandelt gedrukte tekst, tabellen en handschrift.
...
| **Meerkeuzequiz** | `mistral-large-latest` | 5-50 vragen, 4 keuzes elk, uitleg, adaptieve review |
| **Invuloefeningen** | `mistral-large-latest` | Te vullen zinnen met hints, tolerantiewaardering bij controle (Levenshtein) |
...
De router gebruikt `mistral-small-latest` om de inhoud van bronnen te analyseren en aan te bevelen welke generators het meest relevant zijn uit de 7 beschikbare — zodat leerlingen niet handmatig hoeven te kiezen. De interface toont realtime voortgang: eerst een analysetap, daarna individuele generaties met annuleeroptie.
...
- **Detectie van instructie** : detecteert herzieningsinstructies ("Ik ken mijn les als ik weet...") en geeft ze prioriteit in alle generators

### Beveiliging & ouderlijk toezicht

- **4 leeftijdsgroepen** : kind (≤10 jaar), tiener (11-15), student (16-25), volwassene (26+)
- **Inhoudsmoderatie** : `mistral-moderation-2603` met 5 geblokkeerde categorieën voor kind/tiener (sexual, hate, violence, selfharm, jailbreaking), geen beperkingen voor student/volwassene
- **Ouderlijk PIN** : SHA-256-hash, vereist voor profielen jonger dan 15 jaar
- **Chatbeperkingen** : AI-chat standaard uitgeschakeld voor onder 16, kan door ouders geactiveerd worden
...
- Volledige interface beschikbaar in Frans en Engels
...
| **Bestandsupload** | Multer 1.4 | Afhandeling multipart-formulieren |
| **Audio** | ffmpeg-static | Samenvoegen van audiodelen |
| **Tests** | Vitest 4 | Unittests — coverage gemeten door SonarCloud |
...
|---|---|---|
| `mistral-large-latest` | Samenvatting, Flashcards, Podcast, Quiz, Invuloefeningen, Chat, Spraakquiz-verificatie, Image Agent, Web Search Agent, Detectie van instructies | Beste multilingual + instructieopvolging |
...
| `mistral-moderation-2603` | Inhoudsmoderatie | 5 geblokkeerde categorieën voor kind/tiener (+ jailbreaking) |
...
> **Opmerking** : Mistral Voxtral TTS is de standaardprovider — geen extra sleutel nodig behalve `MISTRAL_API_KEY`. ElevenLabs is een configureerbare alternatieve TTS-provider in de instellingen.
...
| `PUT` | `/api/profiles/:id` | Een profiel aanpassen (PIN vereist voor < 15 jaar) |
...
| `POST` | `/api/projects/:pid/detect-consigne` | Detecteren van herzieningsinstructies |
...
| `POST` | `/api/projects/:pid/generate/route` | Routeringsanalyse (plan welke generators te starten) |
| `POST` | `/api/projects/:pid/generate/auto` | Auto-backendgeneratie (routering + 5 types: summary, flashcards, quiz, fill-blank, podcast) |
...
| **Persistentie in JSON-bestanden** | Geen afhankelijkheden, directe start. Geen databaseconfiguratie vereist — je kunt meteen beginnen. |
...
| **Multi-generatie systeem** | Elke generatie is een onafhankelijk object met een eigen ID — maakt meerdere lesfiches, quizzen, enz. per cursus mogelijk. |
...
| **Agent-gebaseerde functionaliteiten** | Beeldgeneratie en webzoekopdrachten gebruiken tijdelijke Mistral-agents — eigen levenscyclus met automatische opruiming. |
