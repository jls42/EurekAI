...
<p align="center">
  <strong>Transformeer elk type inhoud in een interactieve leerervaring — aangedreven door <a href="https://mistral.ai">Mistral AI</a>.</strong>
</p>
...
**EurekAI** is ontstaan tijdens de [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([officiële site](https://worldwide-hackathon.mistral.ai/)) (maart 2026). Ik had een onderwerp nodig — en het idee kwam voort uit iets heel concreets: ik bereid regelmatig toetsen voor met mijn dochter, en ik dacht dat het mogelijk moest zijn om dat speelser en interactiever te maken dankzij AI.

Het doel: elke invoer nemen — een foto van het handboek, gekopieerde tekst, een geluidsopname, een webzoekopdracht — en die omzetten in samenvattingen, flashcards, quizzen, podcasts, invuloefeningen, illustraties en meer. Alles aangedreven door de Franse modellen van Mistral AI, wat het van nature geschikt maakt voor Franstalige leerlingen.

Het project is begonnen tijdens de hackathon en daarna verder uitgewerkt. De volledige code is gegenereerd door AI — voornamelijk [Claude Code](https://docs.anthropic.com/en/docs/claude-code), met enkele bijdragen via [Codex](https://openai.com/index/introducing-codex/).
...
| 📝 | **Saisie texte** | Typ of plak direct elke tekst |
| 🎤 | **Entrée vocale** | Neem jezelf op — Voxtral STT transcribeert je stem |
| 🌐 | **Recherche web** | Stel een vraag — een Mistral-agent zoekt de antwoorden op het web |
| 📄 | **Fiches de révision** | Gestructureerde samenvattingen met kernpunten, vocabulaire, citaten, anekdotes |
| 🃏 | **Flashcards** | 5-50 Q/A-kaarten met bronvermeldingen voor actief onthouden |
| ❓ | **Quiz QCM** | 5-50 meerkeuzevragen met adaptieve foutherziening |
| ✏️ | **Textes à trous** | Invuloefeningen met hints en tolerante validatie |
| 🎙️ | **Podcast** | Mini-podcast met 2 stemmen omgezet naar audio via Mistral Voxtral TTS |
| 🖼️ | **Illustrations** | Educatieve afbeeldingen gegenereerd door een Mistral-agent |
| 🗣️ | **Quiz vocal** | Vragen voorgelezen, mondeling antwoord, AI controleert het antwoord |
| 💬 | **Tuteur IA** | Contextuele chat met je lesdocumenten, met mogelijkheid om tools aan te roepen |
| 🧠 | **Routeur automatique** | Een router gebaseerd op `mistral-small-latest` analyseert de inhoud en stelt een combinatie van generators voor uit de 7 beschikbare types |
| 🔒 | **Contrôle parental** | Ouderlijk toezicht: leeftijdsmoderatie, ouderlijke PIN, chatbeperkingen |
| 🌍 | **Multilingue** | Meertalig: interface beschikbaar in 9 talen; AI-generatie te sturen in 15 talen via prompts |
| 🔊 | **Lecture à voix haute** | Voorlezen: luister naar samenvattingen en flashcards via Mistral Voxtral TTS of ElevenLabs |
...
EurekAI accepteert 4 soorten bronnen, gemodereerd afhankelijk van het profiel (standaard ingeschakeld voor kind en tiener):

- **Upload OCR** — Bestanden JPG, PNG of PDF verwerkt door `mistral-ocr-latest`. Ondersteunt gedrukte tekst, tabellen en handschrift.
- **Texte libre** — Typ of plak elke inhoud. Gemodereerd voor opslag als moderatie is ingeschakeld.
- **Entrée vocale** — Spraakinvoer: neem audio in de browser op. Getranscribeerd door `voxtral-mini-latest`. De parameter `language="fr"` optimaliseert herkenning.
- **Recherche web** — Webzoekopdracht: voer een zoekopdracht in. Een tijdelijke Mistral-agent met de tool `web_search` haalt resultaten op en vat ze samen.

### AI-inhoudsgeneratie

Zeven typen gegenereerd lesmateriaal:

| Generator | Model | Uitvoer |
|---|---|---|
| **Fiche de révision** | `mistral-large-latest` | Titel, samenvatting, 10-25 kernpunten, vocabulaire, citaten, anekdote |
| **Flashcards** | `mistral-large-latest` | 5-50 Q/A-kaarten met bronvermeldingen voor actief onthouden |
| **Quiz QCM** | `mistral-large-latest` | 5-50 vragen, 4 keuzes elk, uitleg, adaptieve herziening |
| **Textes à trous** | `mistral-large-latest` | Zinnen om in te vullen met hints, tolerante validatie (Levenshtein) |
| **Podcast** | `mistral-large-latest` + Voxtral TTS | Script 2 stemmen → MP3-audio |
| **Illustration** | Agent `mistral-large-latest` | Educatieve afbeelding via de tool `image_generation` |
| **Quiz vocal** | `mistral-large-latest` + Voxtral TTS + STT | Vragen TTS → antwoord STT → AI-verificatie |

### AI-tutor via chat

Een AI-tutor met volledig toegang tot lesdocumenten:

- Gebruikt `mistral-large-latest`
- **Gereedschapsoproepen**: kan tijdens het gesprek samenvattingen, flashcards, quizzen of invuloefeningen genereren
- Geschiedenis van 50 berichten per cursus
- Inhoudsmoderatie indien ingeschakeld voor het profiel

### Automatische router

De router gebruikt `mistral-small-latest` om de inhoud van bronnen te analyseren en de meest geschikte generators voor te stellen uit de 7 beschikbare types. De interface toont realtime voortgang: eerst een analysefase, daarna individuele generaties met mogelijkheid tot annuleren.
...
- Detectie van instructies: detecteert herzieningsinstructies ("Je sais ma leçon si je sais...") en geeft prioriteit aan compatibele tekstgenerators (samenvatting, flashcards, quiz, invuloefeningen)

### Beveiliging & ouderlijk toezicht

- 4 leeftijdsgroepen: kind (≤10 jaar), tiener (11-15), student (16-25), volwassene (26+)
- Inhoudsmoderatie: `mistral-moderation-2603` met 5 categorieën geblokkeerd voor kind/tiener (sexual, hate, violence, selfharm, jailbreaking), geen beperkingen voor student/volwassene
- Ouderlijk PIN: SHA-256-hash, vereist voor profielen jonger dan 15 jaar. Voor productie-implementatie gebruik een langzaam gehashte met zout (Argon2id, bcrypt).
...
- Provider configureerbaar in de app-instellingen
...
| **Serveur de dev** | Vite 7.x + tsx | HMR, Handlebars-partials, proxy |
...
| **TTS (défaut)** | Mistral Voxtral TTS | `voxtral-mini-tts-latest`, ingebouwde spraaksynthese |
| **TTS (alternatif)** | ElevenLabs SDK 2.x | `eleven_v3`, natuurlijke stemmen |
| **Icônes** | Lucide | SVG-icoonbibliotheek |
| **Markdown** | Marked | Markdown-rendering in de chat |
| **Upload fichiers** | Multer 1.4 LTS | Multipart-formulierenbeheer |
| **Audio** | ffmpeg-static | Samenvoegen van audiopartjes |
| **Tests** | Vitest | Unittests — dekking gemeten door SonarCloud |
| **Persistance** | Fichiers JSON | Opslag zonder afhankelijkheden |

---

## Referentie van modellen

| Model | Gebruik | Waarom |
|---|---|---|
| `mistral-large-latest` | Samenvatting, Flashcards, Podcast, Quiz, Invuloefeningen, Chat, Verificatie spraakquiz, Image-agent, Web Search-agent, Detectie consigne | Beste meertaligheid + instructieopvolging |
...
> **Opmerking** : Mistral Voxtral TTS is de standaardprovider — geen extra sleutel nodig behalve `MISTRAL_API_KEY`. ElevenLabs is een alternatieve TTS-provider configureerbaar in de instellingen.
...
| `PUT` | `/api/config` | Wijzig de config (modellen, stemmen, TTS-provider) |
| `GET` | `/api/config/status` | Status van APIs (Mistral, ElevenLabs, TTS) |
| `POST` | `/api/config/reset` | Reset de configuratie naar standaard |
| `GET` | `/api/config/voices` | Lijst Mistral TTS-stemmen (optioneel `?lang=fr`) |

### Profielen
| Methode | Endpoint | Beschrijving |
|---|---|---|
| `GET` | `/api/profiles` | Alle profielen tonen |
...
| `PUT` | `/api/projects/:pid` | Naam wijzigen `{name}` |
...
| `POST` | `/api/projects/:pid/sources/voice` | STT-audio (multipart audio) |
...
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | Invuloefening-antwoorden indienen `{answers}` |
...
| `PUT` | `/api/projects/:pid/generations/:gid` | Naam wijzigen `{title}` |
...
| **Persistentie in JSON-bestanden** | Geen afhankelijkheden, onmiddellijke start. Geen database te configureren — je kunt meteen beginnen. |
| **Vite + Handlebars** | Het beste van twee werelden: snelle HMR voor ontwikkeling, HTML-partials voor de organisatie van code, Tailwind JIT. |
| **Gecentraliseerde prompts** | Alle AI-prompts in `prompts.ts` — eenvoudig te itereren, testen en aanpassen per taal/leeftijdsgroep. |
| **Multi-generatiesysteem** | Elke generatie is een onafhankelijk object met een eigen ID — maakt meerdere kaarten, quizzen, enz. per cursus mogelijk. |
| **Leeftijdsgestuurde prompts** | 4 leeftijdsgroepen met verschillend vocabulaire, complexiteit en toon — dezelfde inhoud onderwijst anders afhankelijk van de leerling. |
| **Agent-gebaseerde functionaliteiten** | Afbeeldingsgeneratie en webzoekopdrachten gebruiken tijdelijke Mistral-agents — eigen levenscyclus met automatische opruiming. |
