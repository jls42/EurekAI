...
<p align="center">
  <strong>将任何内容转变为互动学习体验 — 由 <a href="https://mistral.ai">Mistral AI</a> 提供动力。</strong>
</p>
...
<p align="center">
  <a href="https://sonarcloud.io/summary/new_code?id=jls42_EurekAI"><img src="https://sonarcloud.io/api/project_badges/measure?project=jls42_EurekAI&metric=alert_status&token=c8f6207cabba0e2841b2ec04126d7fd6687397e8" alt="质量关"></a>
...
**EurekAI** 诞生于 [Mistral AI 全球黑客松](https://luma.com/mistralhack-online)（[官网](https://worldwide-hackathon.mistral.ai/)）（2026 年 3 月）。我需要一个项目题材——灵感来自一件很实际的事：我经常和女儿一起准备测验，我想到可以借助 AI 把这件事做得更有趣、更互动。

目标：将 **任何输入** —— 课程照片、复制粘贴的文本、语音录音、网页搜索 —— 转换为 **复习笔记、抽认卡、测验、播客、填空题、插图** 等内容。全部由法国的 Mistral AI 模型驱动，因此天然适合法语学生使用。

最初的 [原型](https://github.com/jls42/worldwide-hackathon.mistral.ai) 在黑客松的 48 小时内作为针对 Mistral 服务的概念验证构建——已能工作但功能有限。此后，EurekAI 发展为真正的项目：填空题、练习导航、网页抓取、可配置的家长审核、深入的代码审查等。全部代码由 AI 生成——主要用到 [Claude Code](https://code.claude.com/)，并有少量通过 [Codex](https://openai.com/codex/) 和 [Gemini CLI](https://geminicli.com/) 的贡献。
...
| | 功能 | 描述 |
|---|---|---|
| 📷 | **导入文件** | 导入课件：照片、PDF（通过 Mistral OCR）或文本文件（TXT、MD） |
| 📝 | **文本输入** | 直接键入或粘贴任意文本 |
| 🎤 | **语音输入** | 在浏览器中录音 — Voxtral STT 转录你的语音 |
| 🌐 | **网页 / URL** | 粘贴 URL（通过 Readability + Lightpanda 直接抓取）或输入搜索词（Agent Mistral web_search） |
| 📄 | **复习笔记** | 结构化笔记，含要点、词汇、引用、轶事 |
| 🃏 | **抽认卡** | 问答卡片并带有来源引用以便主动记忆（数量可配置） |
| ❓ | **多选测验** | 多项选择题，带错题自适应复习（数量可配置） |
| ✏️ | **填空题** | 带提示和容错验证的填空练习 |
| 🎙️ | **播客** | 双声道迷你播客音频 — 默认 Mistral 声音或自定义语音（家长可用！） |
| 🖼️ | **插图** | 由 Agent Mistral 生成的教育插图 |
| 🗣️ | **语音测验** | 题目朗读（可自定义语音）、口头回答、AI 验证 |
| 💬 | **智能辅导** | 可访问你的课程文档的上下文聊天，支持调用工具 |
| 🧠 | **自动路由器** | 基于 `mistral-small-latest` 的路由器分析内容并在 7 种生成器间推荐组合 |
| 🔒 | **家长控制** | 可按档案配置的审核（可自定义类别）、家长 PIN、聊天限制 |
| 🌍 | **多语言** | 界面支持 9 种语言；通过提示可控制 AI 在 15 种语言中生成内容 |
| 🔊 | **朗读** | 使用 Mistral Voxtral TTS 或 ElevenLabs 收听笔记和抽认卡 |
...
<p align="center">
  <img src="public/assets/model-map.webp" alt="模型到任务映射" width="800" />
</p>
...
EurekAI 接受 4 种来源类型，根据档案进行审核（儿童和青少年默认启用）：

- **导入文件** — JPG、PNG 或 PDF 文件由 `mistral-ocr-latest` 处理（印刷文本、表格、手写笔记），或直接导入文本文件（TXT、MD）。
- **自由文本** — 键入或粘贴任意内容。若启用审核则在存储前进行审核。
- **语音输入** — 在浏览器中录音。由 `voxtral-mini-latest` 转录。参数 `language="fr"` 优化识别效果。
- **网页 / URL** — 粘贴一个或多个 URL 以直接抓取内容（Readability + Lightpanda 处理 JS 页面），或输入关键词通过 Agent Mistral 搜索网页。唯一输入栏同时接受 URL 与关键词，两者会自动分离，每个结果创建一个独立来源。
...
| **多选测验** | `mistral-large-latest` | 多项选择题、解释、错题自适应复习（数量可配置） |
| **填空题** | `mistral-large-latest` | 带提示的待填句子，容错验证（Levenshtein） |
...
- **工具调用**：可在对话中生成复习笔记、抽认卡、测验或填空题
- 每门课程保留 50 条消息的历史
- 若为启用审核的档案，则对话内容会被审核

### 自动路由器

路由器使用 `mistral-small-latest` 分析来源内容，并在 7 种生成器中推荐最相关的生成策略。界面实时显示进度：先是分析阶段，然后逐个生成，支持取消单项生成。

### 自适应学习

- **测验统计**：跟踪每题的尝试次数与正确率
- **测验复习**：生成 5-10 道新题，针对薄弱概念
- **指令检测**：识别复习指令（如：“如果我知道……，说明我会这章”），并在兼容的文本生成器（复习笔记、抽认卡、测验、填空题）中优先处理
...
- **家长 PIN**：采用 SHA-256 哈希，15 岁以下档案需要输入。生产部署建议使用带盐的慢哈希（Argon2id、bcrypt）。
- **聊天限制**：16 岁以下默认禁用 AI 聊天，家长可启用

### 多档案系统

- 支持多个档案，包含姓名、年龄、头像、语言偏好
- 项目可与档案关联，通过 `profileId` 管理
- 级联删除：删除档案会删除其所有项目
...
- 可在应用设置中选择提供者
- **自定义语音**：家长可通过 Mistral Voices API（基于音频样本）创建专属语音并分配给主讲/嘉宾角色——播客与语音测验即可使用家长录制的声音，增强沉浸感
- 支持两个配置的语音角色：**主讲**（主要叙述者）与 **嘉宾**（播客第二声道）
- 可在设置中查看 Mistral 全量语音目录，并按语言过滤
...
| **无头浏览器** | Lightpanda | 面向 JS/SPA 页面的超轻量无头浏览器（Zig + V8）——作为抓取回退方案 |
...
| 模型 | 使用场景 | 为什么 |
|---|---|---|
| `mistral-large-latest` | 复习笔记、抽认卡、播客、测验、填空题、聊天、语音测验验证、图像 Agent、网页搜索 Agent、指令检测 | 最佳的多语种能力 + 指令跟随能力 |
| `mistral-ocr-latest` | 文档 OCR | 印刷文本、表格、手写文字 |
| `voxtral-mini-latest` | 语音识别（STT） | 多语种 STT，可与 `language="fr"` 优化配合 |
| `voxtral-mini-tts-latest` | 语音合成（TTS） | 播客、语音测验、朗读 |
| `mistral-moderation-latest` | 内容审核 | 儿童/青少年默认屏蔽 5 大类（+ 越狱检测） |
| `mistral-small-latest` | 自动路由器 | 快速分析内容以做出路由决策 |
| `eleven_v3` (ElevenLabs) | 语音合成（可选 TTS） | 自然人声，可配置替代方案 |
...
> **注意**：Mistral Voxtral TTS 是默认提供者——除 `MISTRAL_API_KEY` 外无需额外密钥。ElevenLabs 是可在设置中配置的替代 TTS 提供者。

---

## 容器部署

镜像已发布至 **GitHub Container Registry**：
...
> **`:U`** 是 Podman 无 root 模式的标志，会自动调整卷权限。  
> **`ELEVENLABS_API_KEY`** 为可选项（用于替代 TTS）。
...
|---|---|---|
| `GET` | `/api/config` | 当前配置 |
| `PUT` | `/api/config` | 修改配置（模型、语音、TTS 提供者） |
...
| 方法 | 端点 | 描述 |
|---|---|---|
| `POST` | `/api/projects/:pid/sources/upload` | 导入 multipart 文件（针对 JPG/PNG/PDF 的 OCR，针对 TXT/MD 的直接读取） |
| `POST` | `/api/projects/:pid/sources/text` | 自由文本 `{text}` |
| `POST` | `/api/projects/:pid/sources/voice` | 语音 STT（音频 multipart） |
| `POST` | `/api/projects/:pid/sources/websearch` | 抓取 URL 或 网络搜索 `{query}` — 返回一个来源数组 |
...
| 方法 | 端点 | 描述 |
|---|---|---|
| `POST` | `/api/projects/:pid/generate/summary` | 复习讲义 |
| `POST` | `/api/projects/:pid/generate/flashcards` | 抽认卡 |
| `POST` | `/api/projects/:pid/generate/quiz` | 多项选择测验（QCM） |
...
| `POST` | `/api/projects/:pid/generations/:gid/read-aloud` | TTS 朗读（讲义/抽认卡） |
...
| 决策 | 理由 |
|---|---|
| **选择 Alpine.js 而非 React/Vue** | 占用小，配合由 Vite 编译的 TypeScript 提供轻量响应性。非常适合重视速度的黑客马拉松。 |
| **使用 JSON 文件持久化** | 零依赖，瞬时启动。无需配置数据库 —— 启动即可使用。 |
| **Vite + Handlebars** | 两者兼得：开发时快速 HMR，使用 HTML partials 组织代码，Tailwind JIT。 |
| **集中式提示（Prompts）** | 所有 AI 提示放在 `prompts.ts` — 易于迭代、测试并按语言/年龄组进行调整。 |
| **多生成系统** | 每个生成都是一个具有独立 ID 的对象 —— 允许每门课程拥有多份讲义、测验等。 |
| **按年龄调整的提示** | 4 个年龄组，词汇、复杂度和语气各异 —— 相同内容可根据学习者不同方式教授。 |
| **基于 Agents 的功能** | 图像生成和网页搜索使用临时的 Mistral Agents —— 生命周期独立并自动清理。 |
| **智能 URL 抓取** | 单一输入字段同时接受 URL 和关键字混合输入 —— URL 使用 Readability 抓取（静态页面），回退到 Lightpanda（JS/SPA 页面），关键字触发 Mistral Agent 的 web_search。每个结果都会创建一个独立来源。 |
| **多提供商 TTS** | 默认使用 Mistral Voxtral TTS（无需额外密钥），可选 ElevenLabs —— 可在不重启的情况下配置。 |
...
于 Mistral AI 全球黑客松（2026 年 3 月）期间启动，完全由 AI 开发，使用了 [Claude Code](https://code.claude.com/)、[Codex](https://openai.com/codex/) 和 [Gemini CLI](https://geminicli.com/)。
