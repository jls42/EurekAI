...
<p align="center">
  <strong>Transforme qualquer conteúdo em uma experiência de aprendizagem interativa — impulsionada por IA.</strong>
</p>
...
**EurekAI** nasceu durante o [Hackathon Mundial Mistral AI](https://luma.com/mistralhack-online) ([site oficial](https://worldwide-hackathon.mistral.ai/)) (março de 2026). Eu precisava de um tema — e a ideia veio de algo bem concreto: eu costumo preparar provas com minha filha, e pensei que seria possível tornar isso mais divertido e interativo com IA.

O objetivo: pegar **qualquer entrada** — uma foto do manual, um texto copiado, uma gravação de voz, uma busca na web — e transformá-la em **fichas de revisão, flashcards, quizzes, podcasts, textos com lacunas, ilustrações e muito mais**. Tudo alimentado pelos modelos franceses da Mistral AI, tornando a solução naturalmente adaptada aos alunos francófonos.

Cada linha de código foi escrita durante o hackathon. Todas as APIs e bibliotecas open-source são usadas em conformidade com as regras do hackathon.
...
|---|---|---|
| 📷 | **Upload OCR** | Tire uma foto do seu manual ou das suas notas — Mistral OCR extrai o conteúdo |
...
| 🎙️ | **Podcast** | Mini-podcast 2 vozes convertido em áudio via Mistral Voxtral TTS |
...
| 🧠 | **Roteador inteligente** | A IA analisa seu conteúdo e recomenda os geradores mais relevantes entre os 7 disponíveis |
...
- **Texto livre** — Digite ou cole qualquer conteúdo. Moderado antes do armazenamento se a moderação estiver ativada.
...
O roteador usa `mistral-small-latest` para analisar o conteúdo das fontes e recomendar quais geradores são mais pertinentes entre os 7 disponíveis — para que os alunos não tenham que escolher manualmente. A interface mostra o progresso em tempo real: primeiro uma fase de análise, depois as gerações individuais com possibilidade de cancelamento.
...
- **Detecção de instrução** : detecta instruções de revisão ("Se eu sei minha lição se eu sei...") e as prioriza em todos os geradores
...
- Prompts de IA suportam 2 idiomas hoje (FR, EN) com arquitetura preparada para 15 (es, de, it, pt, nl, ja, zh, ko, ar, hi, pl, ro, sv)
...
| Modelo | Uso | Por quê |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos com lacunas, Chat, Verificação de quiz por voz, Agent Image, Agent Web Search, Detecção de instrução | Melhor multilíngue + seguimento de instruções |
...
| `eleven_v3` (ElevenLabs) | Síntese de voz (TTS alternativo) | Vozes naturais, alternativa configurável |
...
| `POST` | `/api/config/reset` | Resetar a config padrão |
...
| `DELETE` | `/api/profiles/:id` | Excluir um perfil + projetos em cascata |
...
| **Persistência em arquivos JSON** | Zero dependência, início instantâneo. Nenhum banco de dados para configurar — você inicia e está pronto. |
...
| **Sistema multi-gerações** | Cada geração é um objeto independente com seu próprio ID — permite várias fichas, questionários, etc. por curso. |
