...
<p align="center">
  <a href="https://app.codacy.com/gh/jls42/EurekAI/dashboard?utm_source=gh&utm_medium=referral&utm_content=&utm_campaign=Badge_grade"><img src="https://app.codacy.com/project/badge/Grade/e4e3a71712194157a90c2335f84ba7e4" alt="Badge do Codacy"></a>
...
O [protótipo inicial](https://github.com/jls42/worldwide-hackathon.mistral.ai) foi criado em 48h durante o hackathon como prova de conceito em torno dos serviços da Mistral — já funcional, mas limitado. Desde então, EurekAI se tornou um projeto de verdade: textos com lacunas, navegação nos exercícios, scraping web, moderação parental configurável, revisão de código aprofundada e muito mais. Todo o código é gerado por IA — principalmente por [Claude Code](https://code.claude.com/), com algumas contribuições via [Codex](https://openai.com/codex/) e [Gemini CLI](https://geminicli.com/).
...
| ❓ | **Quiz de múltipla escolha** | Perguntas de múltipla escolha com revisão adaptativa dos erros (quantidade configurável) |
| ✏️ | **Textos com lacunas** | Exercícios para completar com dicas e validação tolerante |
| 🎙️ | **Podcast** | Mini-podcast com 2 vozes em áudio — voz Mistral por padrão ou vozes personalizadas (dos pais!) |
| 🖼️ | **Ilustrações** | Imagens educacionais geradas por um Agent Mistral |
| 🗣️ | **Quiz oral** | Perguntas lidas em voz alta (voz custom possível), resposta oral, verificação por IA |
| 💬 | **Tutor de IA** | Chat contextual com seus documentos de curso, com chamadas de ferramentas |
...
- **Importação de arquivos** — Arquivos JPG, PNG ou PDF processados por OCR Mistral — **OCR 3 (`mistral-ocr-2512`) por padrão**, **OCR 4 (`mistral-ocr-4-0`) como opção** em Configurações (melhor qualidade, mas 2× o custo) — para texto impresso, tabelas e escrita manuscrita; ou arquivos de texto (TXT, MD) importados diretamente. Os uploads de vários arquivos usam um sistema de **sessões de upload**: progresso individual por arquivo, retry do arquivo com falha sem reenviar os outros, encerramento da sessão quando concluída. O OCR expõe um **score de confiança** médio (`average`, limitado em `[0,1]`, calculado a partir de `averagePageConfidenceScore` retornados pela Mistral), exibido na UI como badge de tier `high` / `medium` / `low` (limiares ~0.9 / ~0.7) — avisa sem bloquear se o scan estiver com baixa qualidade.
...
- **Web / URL** — Cole uma ou várias URLs para fazer scraping do conteúdo diretamente (Readability + Lightpanda para páginas JS), ou digite palavras-chave para uma pesquisa web via Agent Mistral. O campo único aceita os dois — URLs e palavras-chave são separados automaticamente, e cada resultado cria uma fonte independente.
...
| **Quiz de múltipla escolha** | `mistral-large-latest` | Perguntas de múltipla escolha, explicações, revisão adaptativa (quantidade configurável) |
...
- **Chamadas de ferramentas**: pode gerar fichas, flashcards, quizzes ou textos com lacunas durante a conversa
- Histórico de 50 mensagens por curso
- Moderação de conteúdo se ativada para o perfil

### Roteador automático

O roteador usa `mistral-small-latest` para analisar o conteúdo das fontes e propor os geradores mais relevantes entre os 7 disponíveis. A interface mostra o progresso em tempo real: primeiro uma fase de análise, depois as gerações individuais com cancelamento possível.
...
- **PIN parental**: hash SHA-256, obrigatório para perfis com menos de 15 anos. Para uma implantação em produção, preveja um hash lento com sal (Argon2id, bcrypt).
...
- **Voz por perfil** (`Profile.mistralVoices?: { host, guest }`) — cada criança pode ter sua dupla de vozes de podcast/quiz oral
...
- **Fonte da verdade**: `helpers/pricing.ts` — `MODEL_PRICING` por prefixo de modelo (ex: `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), `PRICING_SOURCES` com URLs de docs da Mistral para novo scraping periódico
...
- **Vozes personalizadas**: os pais podem criar suas próprias vozes via a API Mistral Voices (a partir de uma amostra de áudio) e atribuí-las aos papéis de host/convidado — os podcasts e quizzes orais passam então a ser lidos com a voz de um dos pais, tornando a experiência ainda mais imersiva para a criança
...
| **Frontend** | HTML + TailwindCSS 4.x + Alpine.js 3.x | Interface reativa, TypeScript compilado por Vite |
...
| **Navegador headless** | Lightpanda | Navegador headless ultraleve (Zig + V8) para páginas JS/SPA — fallback scraping |
...
| Modelo | Uso | Por quê |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos com lacunas, Chat, Verificação de quiz oral, Agent Image, Agent Web Search, Detecção de instrução | Melhor multilíngue + seguimento de instruções |
| `mistral-ocr-2512` (OCR 3, padrão) | OCR de documentos | Texto impresso, tabelas, escrita manuscrita ($2 / 1000 páginas) |
| `mistral-ocr-4-0` (OCR 4, opção) | OCR de documentos — qualidade superior | Selecionável em Configurações, 2× o custo ($4 / 1000 páginas) |
| `voxtral-mini-latest` | Reconhecimento de voz (STT) | STT multilíngue, otimizado com `language="fr"` |
| `voxtral-mini-tts-latest` | Síntese de voz (TTS) | Podcasts, quiz oral, leitura em voz alta |
...
> **Chave de API inserida pelo usuário**: `MISTRAL_API_KEY` agora é **opcional**. Se ela estiver ausente, o app ainda inicia e pede que cada usuário insira **sua própria chave Mistral** na interface. A chave é **armazenada no navegador** (criptografada via Web Crypto + IndexedDB em contexto seguro) e enviada por requisição — **nunca persistida no servidor**. Precedência: chave do perfil > chave global do navegador > `MISTRAL_API_KEY` (env). Definir `EUREKAI_REQUIRE_USER_KEY=true` força cada usuário a fornecer sua chave (a chave de env passa a servir apenas para pré-carregamentos).

> **HTTPS local (tablet/rede local)**: `localhost` já é um contexto seguro. Para acesso pela rede local (tablet), gere um certificado local e ative HTTPS para desbloquear a criptografia do navegador + criptografar a chave em trânsito:
> ```bash
> ./scripts/gen-cert.sh 192.168.1.42   # mkcert se dispo, senão openssl self-signed
...
> **Para os contribuidores de IA** : consultar [`CLAUDE.md`](CLAUDE.md) para o contexto arquitetural detalhado, as regras obrigatórias (anti-vazamento de prompts, códigos de erro, rastreamento de custos) e os problemas conhecidos (Lizard CCN, Opengrep, migração Codacy/Semgrep).
...
| `POST` | `/api/projects/:pid/generate/auto` | Geração automática no backend (roteamento + 7 tipos : summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Execução em paralelo — pressupõe um tier Mistral com rate-limit ≥ 7 requisições simultâneas ; caso contrário, vários 429 podem retornar em `failedSteps`. |
...
| **Scraping inteligente de URLs** | Um único campo aceita URLs e palavras-chave misturadas — as URLs são scrapeadas via Readability (páginas estáticas) com fallback Lightpanda (páginas JS/SPA), as palavras-chave disparam um Agent Mistral web_search. Cada resultado cria uma fonte independente. |
