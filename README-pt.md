...
<p align="center">
  <strong>Transforme qualquer conteúdo em uma experiência de aprendizado interativa — impulsionado por <a href="https://mistral.ai">Mistral AI</a>.</strong>
</p>
...
O [protótipo inicial](https://github.com/jls42/worldwide-hackathon.mistral.ai) foi concebido em 48h durante o hackathon como prova de conceito em torno dos serviços da Mistral — já funcional, mas limitado. Desde então, EurekAI se tornou um projeto de verdade: textos com lacunas, navegação nos exercícios, scraping web, moderação parental configurável, revisão aprofundada de código e muito mais. Todo o código é gerado por IA — principalmente [Claude Code](https://code.claude.com/), com algumas contribuições via [Codex](https://openai.com/codex/) e [Gemini CLI](https://geminicli.com/).
...
|---|---|---|
| 📷 | **Importação de arquivos** | Importe suas lições — foto, PDF (via Mistral OCR com score de confiança médio, tiers `high`/`medium`/`low`) ou arquivo de texto (TXT, MD). Sessões de upload com retry por arquivo e progresso individual |
...
| 🔊 | **Leitura em voz alta** | Ouça as fichas e flashcards (diálogo pergunta/resposta) via Mistral Voxtral TTS ou ElevenLabs |
| 💶 | **Monitoramento de custos de API** | Estimativa transparente do custo € de cada geração e fonte (tokens / caracteres / páginas / segundos de áudio). Badge por cartão + total por projeto, visível no dashboard |
...
- **Importação de arquivos** — Arquivos JPG, PNG ou PDF processados por `mistral-ocr-latest` (texto impresso, tabelas, escrita à mão), ou arquivos de texto (TXT, MD) importados diretamente. Os uploads de múltiplos arquivos usam um sistema de **sessões de upload**: progresso individual por arquivo, retry do arquivo com falha sem reenviar os outros, dismiss da sessão quando concluída. O OCR expõe uma **pontuação de confiança** média (`average`, limitada em `[0,1]`, calculada a partir de `averagePageConfidenceScore` retornados por Mistral), exibida na UI como badge tier `high` / `medium` / `low` (limiares ~0.9 / ~0.7) — avisa sem bloquear se o scan tiver baixa qualidade.
...
- **Web / URL** — Cole uma ou várias URLs para fazer scraping do conteúdo diretamente (Readability + Lightpanda para páginas JS), ou digite palavras-chave para uma busca web via Agent Mistral. O campo único aceita ambos — URLs e palavras-chave são separados automaticamente, cada resultado cria uma fonte independente.
...
O roteador usa `mistral-small-latest` para analisar o conteúdo das fontes e propor os geradores mais relevantes entre os 7 disponíveis. A interface mostra o progresso em tempo real: primeiro uma fase de análise, depois as gerações individuais com possibilidade de cancelamento.

### Aprendizagem adaptativa

- **Estatísticas de quiz**: acompanhamento das tentativas e da precisão por pergunta
- **Revisão de quiz**: gera 5-10 novas perguntas visando os conceitos fracos
- **Detecção de instrução**: detecta instruções de revisão ("Eu sei minha lição se sei...") e as prioriza nos geradores textuais compatíveis (ficha, flashcards, quiz, textos com lacunas)
...
- **PIN parental**: hash SHA-256, exigido para perfis com menos de 15 anos. Para uma implantação em produção, prever um hash lento com sal (Argon2id, bcrypt).
...
- **Voz por perfil** (`Profile.mistralVoices?: { host, guest }`) — cada criança pode ter seu par de vozes para podcast/quiz de voz
...
Cada chamada Mistral (chat, OCR, STT, TTS, moderação, agents) é instrumentada para fornecer uma estimativa € **transparente** ao usuário — sem surpresas na cobrança.
...
- **Unidades suportadas**: `tokens`, `characters` (TTS), `pages` (OCR), `audio-seconds` (STT) — conversão guiada por `helpers/cost-calc.ts`
...
- **Mistral Voxtral TTS** (padrão) : `voxtral-mini-tts-latest`, nenhuma chave adicional necessária
- **ElevenLabs** (alternativo) : `eleven_v3`, vozes naturais, requer `ELEVENLABS_API_KEY`
- Provedor configurável nas definições do aplicativo
- **Vozes personalizadas**: os pais podem criar suas próprias vozes via API Mistral Voices (a partir de uma amostra de áudio) e atribuí-las aos papéis de anfitrião/convidado — os podcasts e quizzes de voz são então lidos com a voz de um dos pais, tornando a experiência ainda mais imersiva para a criança
...
## Referência dos modelos | Modelo | Utilização | Porquê |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos com lacunas, Chat, Verificação de quiz vocal, Agente de Imagem, Agente de Pesquisa Web, Detecção de instruções | Melhor multilíngue + seguimento de instruções |
...
| `eleven_v3` (ElevenLabs) | Síntese vocal (TTS alternativa) | Vozes naturais, alternativa configurável |
...
> **Nota**: Mistral Voxtral TTS é o provider padrão — não é necessária nenhuma chave adicional além de `MISTRAL_API_KEY`. ElevenLabs é um provider TTS alternativo configurável nas definições.

### Variáveis de ambiente

| Variável | Requisito | Padrão | Função |
|---|---|---|---|
| `MISTRAL_API_KEY` | ✅ | — | Chave API Mistral (chat, OCR, STT, TTS Voxtral, agentes, moderação) |
| `ELEVENLABS_API_KEY` | ⚠ opcional | — | Chave ElevenLabs; necessária apenas se o provider TTS = ElevenLabs |
...
Sem essas ferramentas, `npm test` falha em `pretest` (lizard ausente) e `npm run security` falha (opengrep ausente). Os hooks husky bloqueiam então o commit/push.
...
> **`ELEVENLABS_API_KEY`** é opcional (TTS alternativo).
...
> **Para os contribuidores IA**: consultar [`CLAUDE.md`](CLAUDE.md) para o contexto arquitetural detalhado, as regras obrigatórias (anti-leak prompts, códigos de erro, acompanhamento de custos) e os problemas conhecidos (Lizard CCN, Opengrep, migração Codacy/Semgrep).
...
| `GET` | `/api/config/status` | Estado das APIs: `mistral` (chave Mistral definida), `elevenlabs` (chave ElevenLabs definida), `ttsAvailable` (true se a chave do provider TTS configurado está presente) |
| `POST` | `/api/config/reset` | Repor a config padrão |
...
| `DELETE` | `/api/profiles/:id` | Eliminar um perfil + cascata de projetos `{pin?}` → `{ok, deletedProjects}` |
...
| Método | Endpoint | Descrição |
|---|---|---|
| `POST` | `/api/projects/:pid/sources/upload` | Importar ficheiros multipart (OCR para JPG/PNG/PDF, leitura direta para TXT/MD) |
| `POST` | `/api/projects/:pid/sources/text` | Texto livre `{text}` |
| `POST` | `/api/projects/:pid/sources/voice` | Voz STT (áudio multipart) |
| `POST` | `/api/projects/:pid/sources/websearch` | Scraping de URL ou pesquisa web `{query}` — devolve uma tabela de fontes |
| `DELETE` | `/api/projects/:pid/sources/:sid` | Eliminar uma fonte |
| `POST` | `/api/projects/:pid/moderate` | Moderar `{text}` |
| `POST` | `/api/projects/:pid/detect-consigne` | Detetar instruções de revisão |
...
| `POST` | `/api/projects/:pid/generate/route` | Análise de roteamento (plano dos geradores a lançar) — devolve `{plan, costDelta}` (custo do roteamento apenas) |
| `POST` | `/api/projects/:pid/generate/auto` | Geração automática backend (roteamento + 7 tipos: summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Execução em paralelo — pressupõe um tier Mistral com rate-limit ≥ 7 pedidos simultâneos; caso contrário, vários 429 podem aparecer em `failedSteps`. |

Todas as rotas de geração aceitam `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}`. `quiz-review` exige ainda `{generationId, weakQuestions}`.
...
|---|---|
| **Alpine.js em vez de React/Vue** | Pegada mínima, reatividade leve com TypeScript compilado por Vite. Perfeito para um hackathon em que a velocidade conta. |
| **Persistência em ficheiros JSON** | Zero dependências, arranque instantâneo. Sem base de dados para configurar — arrancamos e já está. |
...
| **Prompts centralizados** | Todos os prompts IA em `prompts.ts` — fácil de iterar, testar e adaptar por língua/grupo etário. |
...
| **Prompts adaptados por idade** | 4 grupos etários com vocabulário, complexidade e tom diferentes — o mesmo conteúdo ensina de forma diferente consoante o aluno. |
| **Funcionalidades baseadas em Agents** | A geração de imagens e a pesquisa web usam Agents Mistral temporários — ciclo de vida limpo com limpeza automática. |
| **Scraping inteligente de URL** | Um único campo aceita URLs e palavras-chave misturadas — as URLs são scrapadas via Readability (páginas estáticas) com fallback Lightpanda (páginas JS/SPA), as palavras-chave desencadeiam um Agent Mistral web_search. Cada resultado cria uma fonte independente. |
| **TTS multi-provider** | Mistral Voxtral TTS por defeito (sem chave adicional), ElevenLabs como alternativa — configurável sem reiniciar. |
