<p align="center">
  <img src="public/assets/logo.webp" alt="Logotipo do EurekAI" width="120" />
</p>

<h1 align="center">EurekAI</h1>

<p align="center">
  <strong>Transforme qualquer conteúdo em uma experiência de aprendizagem interativa — alimentada por <a href="https://mistral.ai">Mistral AI</a>.</strong>
</p>
...
<p align="center">
  <a href="https://sonarcloud.io/summary/new_code?id=jls42_EurekAI"><img src="https://sonarcloud.io/api/project_badges/measure?project=jls42_EurekAI&metric=alert_status" alt="Critério de qualidade"></a>
...
**EurekAI** nasceu durante o [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online) ([site oficial](https://worldwide-hackathon.mistral.ai/)) (março de 2026). Eu precisava de um tema — e a ideia veio de algo muito concreto: eu preparo regularmente as provas com minha filha, e pensei que deveria ser possível tornar isso mais lúdico e interativo com IA.

O objetivo: pegar **qualquer entrada** — uma foto da lição, um texto copiado e colado, uma gravação de voz, uma busca na web — e transformá-la em **fichas de revisão, flashcards, quizzes, podcasts, textos com lacunas, ilustrações e muito mais**. Tudo alimentado pelos modelos franceses da Mistral AI, o que o torna uma solução naturalmente adaptada a estudantes francófonos.

O [protótipo inicial](https://github.com/jls42/worldwide-hackathon.mistral.ai) foi construído em 48h durante o hackathon como prova de conceito em torno dos serviços da Mistral — já funcional, mas limitado. Desde então, EurekAI virou um projeto de verdade: textos com lacunas, navegação entre exercícios, scraping da web, moderação parental configurável, revisão de código aprofundada e muito mais. Todo o código é gerado por IA — principalmente [Claude Code](https://code.claude.com/), com algumas contribuições via [Codex](https://openai.com/codex/) e [Gemini CLI](https://geminicli.com/).
...
|---|---|---|
| 📷 | **Importação de arquivos** | Importe suas lições — foto, PDF (via Mistral OCR com score de confiança médio, tier `high`/`medium`/`low`) ou arquivo de texto (TXT, MD). Sessões de upload com nova tentativa por arquivo e progresso individual |
| 📝 | **Entrada de texto** | Digite ou cole qualquer texto diretamente |
| 🎤 | **Entrada de voz** | Grave áudio no navegador — Voxtral STT transcreve sua voz |
| 🌐 | **Web / URL** | Cole uma URL (scraping direto via Readability + Lightpanda) ou digite uma busca (Agent Mistral web_search) |
| 📄 | **Ficha de revisão** | Notas estruturadas com pontos-chave, vocabulário, citações, anedotas |
...
| 🎙️ | **Podcast** | Mini-podcast de 2 vozes em áudio — voz Mistral padrão ou vozes personalizadas (pais!) |
...
| 💶 | **Acompanhamento dos custos da API** | Estimativa transparente do custo € de cada geração e fonte (tokens / caracteres / páginas / segundos de áudio). Badge por cartão + total por projeto, visível no dashboard |
...
- **Importação de arquivos** — Arquivos JPG, PNG ou PDF processados por OCR Mistral — **OCR 3 (`mistral-ocr-2512`) por padrão**, **OCR 4 (`mistral-ocr-4-0`) opcional** em Configurações (melhor qualidade, mas 2× o custo) — para texto impresso, tabelas e escrita manuscrita; ou arquivos de texto (TXT, MD) importados diretamente. Os uploads de múltiplos arquivos usam um sistema de **sessões de upload**: progresso individual por arquivo, nova tentativa do arquivo com falha sem reenviar os outros, encerramento da sessão quando concluída. O OCR expõe uma **pontuação média de confiança** (`average`, limitada em `[0,1]`, calculada a partir de `averagePageConfidenceScore` retornados pela Mistral), exibida na interface como um badge de tier `high` / `medium` / `low` (limiares ~0.9 / ~0.7) — avisa sem bloquear se a digitalização estiver com baixa qualidade.
...
- **Web / URL** — Cole uma ou várias URLs para fazer scraping do conteúdo diretamente (Readability + Lightpanda para páginas JS), ou digite palavras-chave para uma busca na web via Agent Mistral. O campo único aceita os dois — URLs e palavras-chave são separados automaticamente, e cada resultado cria uma fonte independente.

### Geração de conteúdo por IA

Sete tipos de material de aprendizagem gerados:
...
Um tutor conversacional com acesso total aos documentos de aula:
...
O roteador usa `mistral-small-latest` para analisar o conteúdo das fontes e propor os geradores mais relevantes entre os 7 disponíveis. A interface exibe o progresso em tempo real: primeiro uma fase de análise, depois as gerações individuais com possibilidade de cancelamento.
...
- **PIN parental**: hash SHA-256, exigido para perfis com menos de 15 anos. Para implantação em produção, use um hash lento com sal (Argon2id, bcrypt).
...
- **Voz por perfil** (`Profile.mistralVoices?: { host, guest }`) — cada criança pode ter sua própria dupla de vozes para podcast/quiz de voz
- **Tema por perfil** (`Profile.theme: 'dark' | 'light'`) — alterna automaticamente ao mudar de perfil, persistido no backend
...
Cada chamada da Mistral (chat, OCR, STT, TTS, moderação, agentes) é instrumentada para fornecer uma estimativa € **transparente** ao usuário — sem surpresas na cobrança.

- **Fonte da verdade**: `helpers/pricing.ts` — `MODEL_PRICING` por prefixo de modelo (ex: `mistral-large` → input 0.5 €/M tokens, output 1.5 €/M tokens), `PRICING_SOURCES` com URLs da documentação da Mistral para novo scraping periódico
...
- **Interface**: badge de custo por geração (`src/partials/cost-badge-gen.html`), por fonte (`cost-badge-src.html`), total acumulado no dashboard (`Project.totalCost`)
...
### TTS multi-provedor & vozes personalizadas

- **Mistral Voxtral TTS**: `voxtral-mini-tts-latest`, síntese de voz 100% Mistral, sem necessidade de chave adicional
- **Vozes personalizadas**: os pais podem criar suas próprias vozes via a API Mistral Voices (a partir de uma amostra de áudio) e atribuí-las aos papéis de anfitrião/convidado — os podcasts e quizzes de voz passam então a ser lidos com a voz de um dos pais, tornando a experiência ainda mais imersiva para a criança
...
| **Navegador headless** | Lightpanda | Navegador headless ultraleve (Zig + V8) para páginas JS/SPA — scraping de fallback |
...
| Modelo | Uso | Por quê |
|---|---|---|
| `mistral-large-latest` | Ficha, Flashcards, Podcast, Quiz, Textos com lacunas, Chat, Verificação de quiz de voz, Agent de imagem, Agent de busca na web, Detecção de instrução | Melhor suporte multilíngue + seguimento de instruções |
| `mistral-ocr-2512` (OCR 3, padrão) | OCR de documentos | Texto impresso, tabelas, escrita manuscrita ($2 / 1000 pages) |
| `mistral-ocr-4-0` (OCR 4, opcional) | OCR de documentos — qualidade superior | Selecionável em Configurações, 2× o custo ($4 / 1000 pages) |
...
|---|---|---|---|
| `MISTRAL_API_KEY` | opcional | — | Chave de API Mistral (chat, OCR, STT, TTS Voxtral, agents, moderação). Se estiver ausente, o usuário insere sua chave no app (armazenada no navegador, nunca no servidor) |
...
| `HTTPS_KEY` / `HTTPS_CERT` | opcional | — | Caminhos da chave/certificado TLS (cf. `scripts/gen-cert.sh`) → Express e Vite servem em HTTPS (secure context LAN/tablet) |
...
> **Para os contribuidores de IA** : consultar [`CLAUDE.md`](CLAUDE.md) para o contexto arquitetural detalhado, as regras obrigatórias (anti-leak prompts, códigos de erro, cost tracking) e as armadilhas conhecidas (Lizard CCN, Opengrep, migração Codacy/Semgrep).
...
| `GET` | `/api/config/status` | Status das APIs : `mistral` (chave Mistral definida), `ttsAvailable` (alias de `mistral`, Mistral Voxtral é o único provider TTS) |
...
| `POST` | `/api/projects/:pid/generate/remediation-summary` | Ficha de revisão focada nas perguntas erradas de um quiz `{generationId, weakQuestions}` — chamada em paralelo com `quiz-review` pelo botão « Treinar-me nos meus erros » |
| `POST` | `/api/projects/:pid/generate/route` | Análise de roteamento (plano dos geradores a executar) — retorna `{plan, costDelta}` (custo do roteamento apenas) |
| `POST` | `/api/projects/:pid/generate/auto` | Geração automática backend (roteamento + 7 tipos : summary, flashcards, quiz, fill-blank, podcast, quiz-vocal, image). Execução em paralelo — pressupõe um tier Mistral com rate-limit ≥ 7 requisições simultâneas ; caso contrário, vários 429 podem aparecer em `failedSteps`. |
...
| **Persistência em arquivos JSON** | Zero dependência, inicialização instantânea. Nenhum banco de dados para configurar — iniciamos e pronto. |
...
| **Prompts adaptados por idade** | 4 grupos etários com vocabulário, complexidade e tom diferentes — o mesmo conteúdo ensina de forma diferente conforme o aluno. |
| **Funcionalidades baseadas em Agents** | A geração de imagens e a busca web usam Agents Mistral temporários — ciclo de vida limpo com limpeza automática. |
| **Scraping inteligente de URL** | Um campo único aceita URLs e palavras-chave misturadas — as URLs são escrapeadas via Readability (páginas estáticas) com fallback Lightpanda (páginas JS/SPA), as palavras-chave acionam um Agent Mistral web_search. Cada resultado cria uma fonte independente. |
