...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI"><img src="https://img.shields.io/badge/▶️_Voir_la_démo-YouTube-red?style=for-the-badge&logo=youtube" alt="YouTube デモを見る"></a>
</p>
...
**EurekAI** は [Mistral AI Worldwide Hackathon](https://luma.com/mistralhack-online)（公式サイト：[https://worldwide-hackathon.mistral.ai/](https://worldwide-hackathon.mistral.ai/)）中（2026年3月）に生まれました。テーマが必要で、きっかけは非常に具体的なものから来ました：私は定期的に娘のテスト準備を手伝っており、AIによってそれをもっと楽しくインタラクティブにできるはずだと考えました。

目的：写真、コピー＆ペーストしたテキスト、音声録音、Web検索など、**あらゆる入力**を受け取り、**要点まとめ、フラッシュカード、クイズ、ポッドキャスト、穴埋め問題、イラストなど**の学習素材に変換すること。すべてが Mistral AI のモデルで動作するため、フランス語話者の生徒に自然に適したソリューションになっています。

プロジェクトはハッカソン中に [オリジナルのリポジトリ](https://github.com/jls42/worldwide-hackathon.mistral.ai) で開発され、その後ここで継続・拡張されました。コード全体は主に [Claude Code](https://code.claude.com/) によって生成され、一部は [Codex](https://openai.com/codex/) や [Gemini CLI](https://geminicli.com/) を通じた寄与があります。
...
| | 機能 | 説明 |
|---|---|---|
| 📷 | **ファイルインポート** | レッスンをインポート — 写真、PDF（Mistral OCR経由）またはテキストファイル（TXT、MD） |
| 📝 | **テキスト入力** | 任意のテキストを直接入力または貼り付け |
| 🎤 | **音声入力** | 録音 — Voxtral STT が音声を文字起こし |
| 🌐 | **Web検索** | 質問を入力 — 一時的な Mistral エージェントがウェブで回答を検索 |
| 📄 | **要点まとめ（学習ノート）** | キーポイント、語彙、引用、逸話を含む構造化ノート |
| 🃏 | **フラッシュカード** | 出典参照付きのQ/Aカード（枚数は設定可能） |
| ❓ | **多肢選択クイズ** | 誤答に応じた適応的復習付きの選択式問題（枚数は設定可能） |
| ✏️ | **穴埋め問題** | ヒントと寛容な検証付きの補充問題 |
| 🎙️ | **ポッドキャスト** | 2声のミニポッドキャストを Mistral Voxtral TTS で音声化 |
| 🖼️ | **イラスト** | 教育用イメージを Mistral エージェントが生成 |
| 🗣️ | **音声クイズ** | 問題を音声で読み上げ、口答で回答、AIが検証 |
| 💬 | **AI チューター** | コース文書にコンテキストアクセスする会話型チューター（ツール呼び出し可能） |
| 🧠 | **自動ルーティング** | `mistral-small-latest` ベースのルーターがコンテンツを解析し、利用可能な7種類のジェネレータを組み合わせて提案 |
| 🔒 | **ペアレンタルコントロール** | プロファイルごとに設定可能なモデレーション（カテゴリカスタマイズ可）、親用PIN、チャット制限 |
| 🌍 | **多言語対応** | インターフェイスは9言語対応；プロンプトでAI生成は15言語で制御可能 |
...
EurekAI はプロファイルに応じてモデレートされた4種類のソースを受け入れます（子どもとティーンはデフォルトで有効）：

- **ファイルインポート** — JPG、PNG、PDF を `mistral-ocr-latest` で処理（印刷文字、表、手書き文字）、またはテキストファイル（TXT、MD）を直接インポート。
- **自由テキスト** — 任意のコンテンツを入力または貼り付け。保存前にモデレーションが有効なら審査されます。
- **音声入力** — ブラウザで音声を録音。`voxtral-mini-latest` が文字起こし。`language="fr"` の設定で認識が最適化されます。
- **Web検索** — クエリを入力。`web_search` ツールを使う一時的な Mistral エージェントが結果を取得して要約します。
...
| ジェネレータ | モデル | 出力 |
|---|---|---|
| **要点まとめ（学習ノート）** | `mistral-large-latest` | タイトル、要約、キーポイント、語彙、引用、逸話 |
| **フラッシュカード** | `mistral-large-latest` | 出典参照付きQ/Aカード（枚数設定可） |
| **多肢選択クイズ** | `mistral-large-latest` | 選択式問題、解説、適応的復習（枚数設定可） |
| **穴埋め問題** | `mistral-large-latest` | ヒント付きの補充文、寛容な検証（Levenshtein） |
| **ポッドキャスト** | `mistral-large-latest` + Voxtral TTS | 2声のスクリプト → MP3 音声 |
| **イラスト** | Agent `mistral-large-latest` | ツール `image_generation` 経由で教育用画像 |
| **音声クイズ** | `mistral-large-latest` + Voxtral TTS + STT | TTSで出題 → STTで回答 → AIが検証 |

### チャットベースのAIチューター

コース文書にフルアクセスする会話型チューター：

- 使用モデル：`mistral-large-latest`
- **ツール呼び出し**：会話中にノート、フラッシュカード、クイズ、穴埋め問題を生成可能
- コースごとに50メッセージの履歴
- プロファイルでモデレーションが有効なら内容は検査されます

### 自動ルーター

ルーターは `mistral-small-latest` を使ってソースの内容を分析し、7つのジェネレータの中から最も適切なものを提案します。UIはリアルタイム進行状況を表示：最初に分析フェーズ、続いて個別生成が行われ、キャンセル可能です。

### 適応学習

- **クイズ統計**：各問題ごとの試行回数と正答率を追跡
- **クイズ復習**：弱点を狙った5〜10問の新規問題を生成
- **指示検出**：復習指示（「私はこの単元を〜と知っていれば理解したことになる」等）を検出し、対応するテキスト系ジェネレータ（ノート、フラッシュカード、クイズ、穴埋め）で優先的に扱います

### セキュリティ & ペアレンタルコントロール

- **年齢グループ4分類**：子ども（≤10歳）、ティーン（11–15）、学生（16–25）、成人（26+）
- **コンテンツモデレーション**：`mistral-moderation-latest`、10カテゴリ利用可能、子ども/ティーンではデフォルトで5カテゴリをブロック（`sexual`、`hate_and_discrimination`、`violence_and_threats`、`selfharm`、`jailbreaking`）。カテゴリはプロファイル設定でカスタマイズ可能。
- **親用PIN**：SHA-256 ハッシュ、15歳未満のプロファイルに必要。運用環境ではソルト付きの遅延ハッシュ（Argon2id、bcrypt）を検討。
...
- 階層削除：プロファイルを削除すると関連プロジェクトも削除

### マルチプロバイダTTS

- **Mistral Voxtral TTS**（デフォルト）：`voxtral-mini-tts-latest`、追加キー不要
- **ElevenLabs**（代替）：`eleven_v3`、より自然な音声、`ELEVENLABS_API_KEY` を要する
- プロバイダはアプリ設定で切替可能
...
| レイヤー | 技術 | 役割 |
|---|---|---|
| **ランタイム** | Node.js + TypeScript 6.x | サーバーおよび型安全性 |
...
| モデル | 使用箇所 | 理由 |
|---|---|---|
| `mistral-large-latest` | ノート、フラッシュカード、ポッドキャスト、クイズ、穴埋め、チャット、音声クイズ検証、画像エージェント、Web検索エージェント、指示検出 | 多言語対応と指示フォローが最良 |
...
| `mistral-moderation-latest` | コンテンツモデレーション | 子ども/ティーンで5カテゴリをブロック（＋ジャイルブレイキング対策） |
| `mistral-small-latest` | 自動ルーター | ルーティング判断のための高速分析 |
| `eleven_v3` (ElevenLabs) | 音声合成（TTS 代替） | 自然な声、設定可能な代替 |
...
> **注意**：Mistral Voxtral TTS はデフォルトのプロバイダです — `MISTRAL_API_KEY` 以外に追加のキーは不要です。ElevenLabs は設定で切り替え可能な代替TTSプロバイダです。
...
| メソッド | エンドポイント | 説明 |
|---|---|---|
| `GET` | `/api/projects` | プロジェクト一覧を取得（`?profileId=` オプション） |
...
| メソッド | エンドポイント | 説明 |
|---|---|---|
| `POST` | `/api/projects/:pid/sources/upload` | ファイルmultipartのインポート（JPG/PNG/PDFはOCR、TXT/MDは直接読み込み） |
...
| メソッド | エンドポイント | 説明 |
|---|---|---|
| `POST` | `/api/projects/:pid/generate/summary` | 要点まとめ（学習ノート） |
...
| `POST` | `/api/projects/:pid/generate/route` | ルーティング解析（起動するジェネレータのプラン） |
| `POST` | `/api/projects/:pid/generate/auto` | 自動バックエンド生成（ルーティング + 5タイプ：summary, flashcards, quiz, fill-blank, podcast） |

すべての生成ルートは `{sourceIds?, lang?, ageGroup?, count?, useConsigne?}` を受け入れます。`quiz-review` は追加で `{generationId, weakQuestions}` を要求します。
...
| `POST` | `/api/projects/:pid/generations/:gid/vocal-answer` | 口答応答を検証（オーディオ + questionIndex） |
...
## アーキテクチャ上の決定 | 決定 | 理由 |
|---|---|
| **React/VueではなくAlpine.js** | 最小のフットプリント、ViteでコンパイルされたTypeScriptによる軽量なリアクティビティ。速度が重要なハッカソンに最適。 |
| **JSONファイルでの永続化** | 依存なし、即起動。データベースの設定不要 — すぐに始められる。 |
| **Vite + Handlebars** | いいとこ取り：開発向けの高速HMR、コード整理のためのHTMLパーシャル、Tailwind JIT。 |
| **プロンプトの集中管理** | すべてのAIプロンプトを `prompts.ts` に集約 — 言語や年齢層ごとに繰り返し編集、テスト、適応が容易。 |
| **マルチ生成システム** | 各生成は独立したオブジェクトで固有のIDを持つ — コースごとに複数のシート、クイズなどを可能にする。 |
| **年齢別に最適化されたプロンプト** | 語彙、複雑さ、トーンが異なる4つの年齢グループ — 同じ内容でも学習者によって教え方を変える。 |
| **エージェントベースの機能** | 画像生成とウェブ検索は一時的なMistralエージェントを使用 — ライフサイクルが管理され、自動クリーンアップされる。 |
| **複数プロバイダー対応TTS** | デフォルトはMistral Voxtral TTS（追加キー不要）、代替にElevenLabs — 再起動なしで設定可能。 |
...
Mistral AI Worldwide Hackathon（2026年3月）で開始され、[Claude Code](https://code.claude.com/)、[Codex](https://openai.com/codex/)、[Gemini CLI](https://geminicli.com/) を用いたAIによって全工程が開発されました。
...
**この文書はgpt-5-miniモデルを使用してfr版からja版へ翻訳されました。翻訳プロセスの詳細についてはhttps://gitlab.com/jls42/ai-powered-markdown-translatorを参照してください。**
