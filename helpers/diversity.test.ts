...
describe('buildExclusionContext', () => {
  ...
  it('extracts podcast first host lines', () => {
    const gens = [{
      type: 'podcast',
      data: { script: [{ speaker: 'host', text: 'Bienvenue!' }, { speaker: 'guest', text: 'Merci' }] },
    }] as unknown as Generation[];
  ...
  it('truncates at maxChars', () => {
    const questions = Array.from({ length: 100 }, (_, i) => ({ question: `Question numero ${i} sur un sujet tres important et detaille` }));
