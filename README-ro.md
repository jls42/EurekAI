...
Obiectivul: să iau **orice intrare** — o fotografie a lecției, un text copiat-lipit, o înregistrare vocală, o căutare web — și să o transform în **fișe de recapitulare, flashcards, quiz-uri, podcasturi, texte cu spații libere, ilustrații și multe altele**. Totul alimentat de modelele franceze Mistral AI, ceea ce îl face o soluție natural adaptată elevilor francofoni.
...
|---|---|---|
| 📷 | **Import fișiere** | Importați lecțiile — fotografie, PDF (prin Mistral OCR cu scor de încredere mediu, trepte `high`/`medium`/`low`) sau fișier text (TXT, MD). Sesiuni de upload cu retry per fișier și progres individual |
| 📝 | **Intrare text** | Tastați sau lipiți direct orice text |
...
| 🃏 | **Flashcards** | Cartonașe Q/R interactive, citire audio dialogată |
| ❓ | **Quiz QCM** | Întrebări cu alegere multiplă cu revizuirea adaptivă a erorilor (număr configurabil) |
...
| 🧠 | **Rutare automată** | Un router bazat pe `mistral-small-latest` analizează conținutul și propune o combinație de generatoare dintre cele 7 tipuri disponibile |
| 🔒 | **Control parental** | Moderare configurabilă per profil (categorii personalizabile), PIN parental, restricții ale chatului |
| 🌍 | **Multilingv** | Interfață disponibilă în 9 limbi; generare IA pilotabilă în 15 limbi prin prompturi |
| 🔊 | **Citire cu voce tare** | Ascultați fișele și flashcards (dialog între întrebare/răspuns) prin Mistral Voxtral TTS |
| 💶 | **Urmărirea costurilor API** | Estimare transparentă a costului € al fiecărei generări și surse (tokeni / caractere / pagini / secunde audio). Insignă per card + total pe proiect, vizibil în dashboard |
...
- **Import fișiere** — Fișiere JPG, PNG sau PDF procesate prin OCR Mistral — **OCR 3 (`mistral-ocr-2512`) implicit**, **OCR 4 (`mistral-ocr-4-0`) opțional** în Setări (calitate mai bună, dar cost de 2×) — pentru text tipărit, tabele și scris de mână; sau fișiere text (TXT, MD) importate direct. Încărcările multi-fișier folosesc un sistem de **sesiuni de upload**: progres individual per fișier, retry pentru fișierul eșuat fără a retrimite celelalte, dismiss al sesiunii când este terminată. OCR-ul expune un **scor de încredere** mediu (`average`, limitat în `[0,1]`, calculat pe baza `averagePageConfidenceScore` returnate de Mistral), afișat în UI sub forma unui badge de nivel `high` / `medium` / `low` (praguri ~0.9 / ~0.7) — avertizează fără a bloca dacă scanarea este de calitate slabă.
...
| **Flashcards** | `mistral-large-latest` | Cartonașe Q/R cu referințe la surse (număr configurabil) |
...
Routerul folosește `mistral-small-latest` pentru a analiza conținutul surselor și a propune cele mai relevante generatoare dintre cele 7 disponibile. Interfața afișează progresul în timp real : mai întâi o fază de analiză, apoi generările individuale cu posibilitate de anulare.
...
- **Detectarea consignei** : detectează instrucțiunile de revizuire ("Știu lecția mea dacă știu...") și le prioritizează în generatoarele textuale compatibile (fișă, flashcards, quiz, texte cu spații libere)
...
- **Sursa de adevăr** : `helpers/pricing.ts` — `MODEL_PRICING` per prefix de model (de ex. `mistral-large` → input 0.5 €/M tokeni, output 1.5 €/M tokeni), `PRICING_SOURCES` cu URL-urile documentației Mistral pentru re-scraping periodic
- **Unități suportate** : `tokens`, `characters` (TTS), `pages` (OCR), `audio-seconds` (STT) — conversie pilotată de `helpers/cost-calc.ts`
- **Lanțul de instrumentare** : `helpers/tracked-client.ts` (wrap client Mistral) → `helpers/usage-context.ts` (AsyncLocalStorage) → `helpers/cost-calc.ts` → `helpers/cost-persist.ts` → `helpers/cost-middleware.ts` (injecție în răspunsul HTTP)
...
- **Voci personalizate** : părinții pot crea propriile voci prin API-ul Mistral Voices (pe baza unui eșantion audio) și le pot asigna rolurilor gazdă/guest — podcasturile și quiz-urile vocale sunt apoi citite cu vocea unui părinte, făcând experiența și mai captivantă pentru copil
...
|---|---|---|
| `mistral-large-latest` | Fișă, Flashcards, Podcast, Quiz, Texte cu spații libere, Chat, Verificare quiz vocal, Agent Imagine, Agent Web Search, Detectarea consignei | Cel mai bun multilingv + urmărirea instrucțiunilor |
| `mistral-ocr-2512` (OCR 3, implicit) | OCR de documente | Text tipărit, tabele, scris de mână ($2 / 1000 pages) |
| `mistral-ocr-4-0` (OCR 4, opțiune) | OCR de documente — calitate superioară | Selectabil în Setări, cost de 2× ($4 / 1000 pages) |
...
> **Notă** : Mistral Voxtral TTS este singurul provider TTS — nu este necesară nicio cheie suplimentară în plus față de `MISTRAL_API_KEY`.
...
| `SONAR_TOKEN` | opțional CI | — | Utilizat doar de workflow-ul GitHub Actions SonarCloud |
...
**Hook-uri Git (Husky)** : `pre-commit` lansează `npm test`, `pre-push` lansează `npm run security`. Ambele blochează commitul/push-ul în caz de eșec.
...
Fără aceste instrumente, `npm test` eșuează la `pretest` (lizard absent) și `npm run security` eșuează (opengrep absent). Hook-urile husky blochează atunci commitul/push-ul.
...
> **Pentru contribuitorii IA**: consultați [`CLAUDE.md`](CLAUDE.md) pentru contextul arhitectural detaliat, regulile obligatorii (anti-leak prompts, coduri de eroare, urmărirea costurilor) și capcanele cunoscute (Lizard CCN, Opengrep, migrarea Codacy/Semgrep).
...
| `GET` | `/api/config/status` | Starea API-urilor: `mistral` (cheie Mistral definită), `ttsAvailable` (alias al `mistral`, Mistral Voxtral este singurul furnizor TTS) |
...
| `POST` | `/api/projects/:pid/generate/quiz` | Quiz cu alegere multiplă |
...
| **Vite + Handlebars** | Ce e mai bun din două lumi: HMR rapid pentru dezvoltare, partials HTML pentru organizarea codului, Tailwind JIT. |
| **Prompts centralizate** | Toate prompturile IA în `prompts.ts` — ușor de iterat, testat și adaptat după limbă/grupă de vârstă. |
...
| **Prompts adaptate pe vârstă** | 4 grupuri de vârstă cu vocabular, complexitate și ton diferite — același conținut predă diferit în funcție de cursant. |
...
| **TTS 100% Mistral** | Mistral Voxtral TTS (fără cheie suplimentară dincolo de `MISTRAL_API_KEY`) — sinteză vocală integrată în lanțul de cost și în rezolvarea vocii în funcție de limbă. |
