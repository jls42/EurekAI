...
<p align="center">
  <a href="https://www.youtube.com/watch?v=_b1TQz2leoI"><img src="https://img.shields.io/badge/▶️_Vezi_demonstrația-YouTube-red?style=for-the-badge&logo=youtube" alt="Demonstrație YouTube"></a>
</p>
...
|---|---|---|
| 📷 | **Import de fișiere** | Importați lecțiile — fotografie, PDF (prin Mistral OCR cu scor de încredere mediat, praguri `high`/`medium`/`low`) sau fișier text (TXT, MD). Sesiuni de upload cu retry per fișier și progres individual |
...
| ❓ | **Quiz-uri grilă** | Întrebări cu alegere multiplă cu recapitulare adaptivă a greșelilor (număr configurabil) |
...
| 💬 | **Tutor IA** | Chat contextual cu documentele dvs. de curs, cu apelare de unelte |
...
| 💶 | **Urmărirea costurilor API** | Estimare transparentă a costului € al fiecărei generări și surse (tokeni / caractere / pagini / secunde audio). Badge pe card + total pe proiect, vizibil în dashboard |
...
## În profunzime — Funcționalități

### Intrare multi-modală

EurekAI acceptă 4 tipuri de surse, moderare în funcție de profil (activată implicit pentru copii și adolescenți):

- **Import de fișiere** — Fișiere JPG, PNG sau PDF procesate prin `mistral-ocr-latest` (text imprimat, tabele, scriere de mână), sau fișiere text (TXT, MD) importate direct. Uploadurile cu mai multe fișiere folosesc un sistem de **sesiuni de upload**: progres individual pentru fiecare fișier, retry pentru fișierul eșuat fără a retrimite celelalte, închiderea sesiunii când este finalizată. OCR-ul expune un **scor de încredere** mediat (`average`, limitat în `[0,1]`, calculat pe baza `averagePageConfidenceScore` returnate de Mistral), afișat în UI sub forma unui badge de nivel `high` / `medium` / `low` (praguri ~0.9 / ~0.7) — avertizează fără să blocheze dacă scanarea este de calitate slabă.
...
| **Quiz grilă** | `mistral-large-latest` | Întrebări cu alegere multiplă, explicații, recapitulare adaptivă (număr configurabil) |
| **Texte cu spații lipsă** | `mistral-large-latest` | Fraze de completat cu indicii, validare tolerantă (Levenshtein) |
...
- **Apelare de unelte**: poate genera fișe, flashcards, quiz-uri sau texte cu spații lipsă în timpul conversației
- Istoric de 50 de mesaje per curs
- Moderarea conținutului dacă este activată pentru profil

### Router automat

Routerul folosește `mistral-small-latest` pentru a analiza conținutul surselor și pentru a propune cele mai relevante generatoare dintre cele 7 disponibile. Interfața afișează progresul în timp real: mai întâi o fază de analiză, apoi generările individuale cu posibilitate de anulare.

### Învățare adaptativă

- **Statistici ale quiz-urilor**: urmărirea tentativelor și a acurateței pe întrebare
- **Recapitulare quiz**: generează 5-10 întrebări noi care vizează conceptele slabe
- **Detectarea instrucțiunii**: detectează instrucțiunile de recapitulare („Știu lecția dacă știu...”) și le prioritizează în generatoarele textuale compatibile (fișă, flashcards, quiz, texte cu spații lipsă)
...
- **PIN parental**: hash SHA-256, necesar pentru profilurile sub 15 ani. Pentru o implementare de producție, prevedeați un hash lent cu salt (Argon2id, bcrypt).
- **Restricții ale chatului**: chat-ul IA dezactivat implicit pentru cei sub 16 ani, activabil de către părinți
...
- **Temă per profil** (`Profile.theme: 'dark' | 'light'`) — comutare automată la schimbarea profilului, persistentă în backend
...
- **Sursa de adevăr**: `helpers/pricing.ts` — `MODEL_PRICING` per prefix de model (ex: `mistral-large` → input 0.5 €/M tokeni, output 1.5 €/M tokeni), `PRICING_SOURCES` cu URL-uri din documentația Mistral pentru re-scraping periodic
...
- **End-pointuri**: răspunsurile `/generate/*` și `/sources/*` decorează obiectul returnat (Generation / Source) cu `estimatedCost`, `usage` și `costBreakdown`. `POST /generate/auto/route` adaugă un câmp `costDelta: number` pentru costul exclusiv al rutării. `GET /projects/:pid` returnează proiectul îmbogățit cu `totalCost` (sumă calculată din `costLog[]`) + istoricul complet
...
- **Voci personalizate**: părinții își pot crea propriile voci prin API-ul Mistral Voices (pornind de la un eșantion audio) și le pot atribui rolurilor gazdă/invitat — podcasturile și quiz-urile vocale sunt apoi redate cu vocea unui părinte, făcând experiența și mai captivantă pentru copil
...
|---|---|---|
| `mistral-large-latest` | Foaie de revizuire, flashcards, podcast, quiz, texte cu spații libere, chat, verificare quiz vocal, agent imagine, agent căutare web, detectare instrucțiuni | Cel mai bun multilingual + respectarea instrucțiunilor |
...
|---|---|---|---|
| `MISTRAL_API_KEY` | ✅ | — | Cheie API Mistral (chat, OCR, STT, TTS Voxtral, agenți, moderare) |
...
| `GET` | `/api/config/status` | Starea API-urilor: `mistral` (cheia Mistral definită), `ttsAvailable` (alias pentru `mistral`, Mistral Voxtral este singurul provider TTS) |
...
| Metodă | Endpoint | Descriere |
|---|---|---|
| `POST` | `/api/projects/:pid/generate/summary` | Foaie de revizuire |
...
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | Trimite răspunsurile la textele cu spații libere `{answers}` |
...
| **Prompturi centralizate** | Toate prompturile AI în `prompts.ts` — ușor de iterat, testat și adaptat după limbă/grupă de vârstă. |
...
| **Scraping inteligent de URL-uri** | Un singur câmp acceptă URL-uri și cuvinte-cheie amestecate — URL-urile sunt scrapate prin Readability (pagini statice) cu fallback Lightpanda (pagini JS/SPA), iar cuvintele-cheie declanșează un agent Mistral web_search. Fiecare rezultat creează o sursă independentă. |
| **TTS 100% Mistral** | Mistral Voxtral TTS (fără cheie suplimentară dincolo de `MISTRAL_API_KEY`) — sinteză vocală integrată în lanțul de cost și în rezoluția vocală pe limbă. |
