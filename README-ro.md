...
**EurekAI** s-a născut în timpul [Hackathonului mondial Mistral AI](https://luma.com/mistralhack-online) ([site-ul oficial](https://worldwide-hackathon.mistral.ai/)) (martie 2026). Aveam nevoie de un subiect — și ideea a venit din ceva foarte concret: îmi pregătesc regulat testele cu fiica mea, și m-am gândit că ar trebui să fie posibil să fac asta mai distractiv și interactiv cu ajutorul IA.

Scopul: să preia **orice intrare** — o fotografie a lecției, un text copiat-lipit, o înregistrare vocală, o căutare web — și să o transforme în **fișe de recapitulare, flashcarduri, quiz-uri, podcasturi, texte de completat, ilustrații și altele**. Toate acestea sunt alimentate de modelele franceze Mistral AI, ceea ce face soluția în mod natural adaptată elevilor francofoni.

Proiectul a fost dezvoltat în timpul hackathonului pe [repo-ul inițial](https://github.com/jls42/worldwide-hackathon.mistral.ai), apoi continuat și îmbogățit aici. Întregul cod este generat de IA — în principal [Claude Code](https://code.claude.com/), cu câteva contribuții prin [Codex](https://openai.com/codex/) și [Gemini CLI](https://geminicli.com/).
...
|---|---|---|
| 📷 | **Import fișiere** | Importă lecțiile — fotografie, PDF (prin Mistral OCR) sau fișier text (TXT, MD) |
...
| 📄 | **Fișe de recapitulare** | Note structurate cu puncte cheie, vocabular, citate, anecdotă |
| 🃏 | **Flashcarduri** | Carduri Q/R cu referințe la surse pentru memorarea activă (număr configurabil) |
| ❓ | **Chestionar tip grilă (QCM)** | Întrebări cu alegere multiplă cu revizuire adaptativă a greșelilor (număr configurabil) |
| ✏️ | **Texte de completat** | Exerciții de completare cu indicii și validare tolerantă |
| 🎙️ | **Podcast** | Mini-podcast cu 2 voci convertit în audio prin Mistral Voxtral TTS |
...
| 🔒 | **Control parental** | Moderație configurabilă pe profil (categorii personalizabile), PIN parental, restricții ale chat-ului |
| 🌍 | **Multilingv** | Interfață disponibilă în 9 limbi; generarea IA controlabilă în 15 limbi prin prompts |
| 🔊 | **Citire cu voce tare** | Ascultă fișele și flashcardurile prin Mistral Voxtral TTS sau ElevenLabs |
...
- **Text liber** — Tastează sau lipește orice conținut. Moderat înainte de stocare dacă moderarea este activată.
...
- **Căutare web** — Introdu o interogare. Un Agent Mistral temporar cu instrumentul `web_search` recuperează și rezumă rezultatele.
...
| **Chestionar QCM** | `mistral-large-latest` | Întrebări cu alegere multiplă, explicații, revizuire adaptativă (număr configurabil) |
| **Texte de completat** | `mistral-large-latest` | Propoziții de completat cu indicii, validare tolerantă (Levenshtein) |
...
- **Apel la instrumente** : poate genera fișe, flashcarduri, quiz-uri sau texte de completat în timpul conversației
- Istoric de 50 de mesaje pe curs
- Moderație a conținutului dacă este activată pentru profil

### Rutator automat

Rutatorul folosește `mistral-small-latest` pentru a analiza conținutul surselor și a propune generatoarele cele mai potrivite dintre cele 7 disponibile. Interfața afișează progresul în timp real: mai întâi o fază de analiză, apoi generările individuale cu posibilitate de anulare.
...
- **Detectarea instrucțiunilor**: detectează indicațiile de revizuire ("Știu lecția dacă știu...") și le prioritizează în generatoarele textuale compatibile (fișă, flashcarduri, quiz, texte de completat)

### Securitate & control parental

- **4 grupe de vârstă**: copil (≤10 ani), adolescent (11-15), student (16-25), adult (26+)
- **Moderație a conținutului**: `mistral-moderation-latest` cu 10 categorii disponibile, 5 blocate implicit pentru copil/adolescent (`sexual`, `hate_and_discrimination`, `violence_and_threats`, `selfharm`, `jailbreaking`). Categorii personalizabile pe profil în setări.
- **PIN parental**: hash SHA-256, obligatoriu pentru profiluri sub 15 ani. Pentru un mediu de producție, folosiți un hash lent cu salt (Argon2id, bcrypt).
- **Restricții ale chat-ului**: chat IA dezactivat implicit pentru cei sub 16 ani, activabil de părinți
...
|---|---|---|
| **Runtime** | Node.js + TypeScript 6.x | Server și asigurarea tipurilor |
...
| **IA** | Mistral AI SDK 2.x | Chat, OCR, STT, TTS, Agenti, Moderație |
...
|---|---|---|
| `mistral-large-latest` | Fișă, Flashcards, Podcast, Quiz, Texte de completat, Chat, Verificare quiz vocal, Agent Imagine, Agent Web Search, Detectare instrucțiuni | Cel mai bun multilingv + urmărire a instrucțiunilor |
...
| `mistral-moderation-latest` | Moderație de conținut | 5 categorii blocate pentru copil/adolescent (+ jailbreaking) |
...
| `POST` | `/api/config/reset` | Resetează config la implicit |
| `GET` | `/api/config/voices` | Listează vocile Mistral TTS (opțional `?lang=fr`) |
| `GET` | `/api/moderation-categories` | Categorii de moderație disponibile + implicit pe vârstă |
...
| `POST` | `/api/projects/:pid/generations/:gid/fill-blank-attempt` | Trimitere răspunsuri texte de completat `{answers}` |
...
## Decizii arhitecturale | Decizie | Justificare |
|---|---|
...
| **Sistem multi-generații** | Fiecare generație este un obiect independent cu propriul ID — permite mai multe fișe, quiz-uri etc. pe curs. |
| **Prompts adaptate pe vârstă** | 4 grupe de vârstă cu vocabular, complexitate și tonuri diferite — același conținut predă diferit în funcție de cursant. |
...
- **[Mistral AI](https://mistral.ai)** — Modele IA (Large, OCR, Voxtral STT, Voxtral TTS, Moderation, Small) + Hackathon mondial
...
- **[TailwindCSS](https://tailwindcss.com)** — Framework utilitar CSS
